...
public class TopicSelectionApplyAction extends ProjectBaseAction{
	...
	private static final String HQL2 = " from KeyTopicSelection tops left outer join tops.university uni left outer join tops.expUniversity expUni where 1=1 ";
	...
	public Object[] advSearchCondition(){
		...
		if(accountType.equals(AccountType.EXPERT) || accountType.equals(AccountType.TEACHER) || accountType.equals(AccountType.ADMINISTRATOR) || accountType.equals(AccountType.MINISTRY_UNIVERSITY) || accountType.equals(AccountType.LOCAL_UNIVERSITY)) {
			hql.append(" and (tops.status >= 1 or tops.isImported = 1) ");
			...
			hql.append(" and (tops.status >= 2 or tops.isImported=1) ");
		}
		else if(accountType.equals(AccountType.MINISTRY_UNIVERSITY) || accountType.equals(AccountType.LOCAL_UNIVERSITY)) {
			hql.append(" and (tops.status >= 3 or tops.isImported=1) ");
		}
		else if(accountType.equals(AccountType.PROVINCE)) {
			hql.append(" and (tops.status >= 4 or tops.isImported=1) ");
		}
		else if(accountType.equals(AccountType.MINISTRY)) {
			hql.append(" and (tops.status >= 5 or tops.isImported=1) ");
		}
		session.put("topicSelectionMap", map);
		//处理查询范围
		String addHql = this.keyService.topicSelectionInSearch(account);
	...
	public String toAdd(){
		...
		if(accountType.equals(AccountType.MINISTRY_UNIVERSITY) || accountType.equals(AccountType.LOCAL_UNIVERSITY)){//高校推荐
			topicSelection = new KeyTopicSelection();
	...
	public String add(){
		...
		if(topicSelection != null){
			...
			if(accountType.equals(AccountType.EXPERT) || accountType.equals(AccountType.TEACHER)){//专家推荐
				...
				/* 以下代码为跳过部门审核*/
				if(topsApplicantSubmitStatus == 3){//提交申报
				...
				if(topsUniversitySubmitStatus == 3){//提交申报
		...
		topicSelection.setIsImported(0);
...
//		}
	...
	public String toModify(){
		...
		topicSelection = (KeyTopicSelection)dao.query(KeyTopicSelection.class, topsId);//entityId为项目申请id
		if(topicSelection==null){
			addActionError("选题申报对象为空");
			return ERROR;
		}
		if(accountType.equals(AccountType.MINISTRY_UNIVERSITY) || accountType.equals(AccountType.LOCAL_UNIVERSITY)){//高校申报选题
			...
			setAppFlag(2);
		}else if(accountType.equals(AccountType.EXPERT) || accountType.equals(AccountType.TEACHER)){//专家申报选题
			setAppFlag(1);
		}else{
			return INPUT;
		}
	...
	public String modify(){
		Account account = loginer.getAccount();
		AccountType accountType = account.getType();
		KeyTopicSelection oldTopicSelection = (KeyTopicSelection) this.dao.query(KeyTopicSelection.class, topsId);
		if(topicSelection != null){
			if(topicSelection.getName() != null && !topicSelection.getName().isEmpty()){//中文名
	...
	public String addResult(){
		if(topicSelection != null){
			...
			if(topicSelection.getApplicantType() != -1){//申报者类型
				topicSelection.setApplicantType(topicSelection.getApplicantType());
			}
			if(topicSelection.getApplicantId() != null && !topicSelection.getApplicantId().isEmpty()){//申报者id
				topicSelection = this.keyService.setApplicantIdFromTopicSelection(topicSelection, topicSelection);
			}
			if(topicSelection.getApplicantName() != null && !topicSelection.getApplicantName().isEmpty()){//申报者姓名
		...
		topicSelection.setIsImported(1);
	...
	public String modifyResult(){
		...
		if(topicSelection != null){
			...
			if(topicSelection.getUniversity().getId() != null){//申报者id
				Agency university = (Agency) this.dao.query(Agency.class, topicSelection.getUniversity().getId());
	...
	public String delete(){
		for(int i = 0; i < topsIds.size(); i++){
			KeyTopicSelection topicSelection = (KeyTopicSelection)(dao.query(KeyTopicSelection.class, topsIds.get(i)));
	...
	public String toView() {
		...
		topicSelection = (KeyTopicSelection) this.dao.query(KeyTopicSelection.class, topsId);
	...
	public String viewTopic(){
		...
		if(null != topsId){//重大攻关项目选题
			...
			jsonMap.put("topsInfo", topsInfo);//选题申报业务信息
			jsonMap.put("application", null);//项目申报
		}
