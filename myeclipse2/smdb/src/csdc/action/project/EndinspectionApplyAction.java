...
public abstract class EndinspectionApplyAction extends ProjectBaseAction {
	...
	protected String projectName,projectNumber,researchType,projectSubtype,projectTopic,applicant,university,productType,productTypeOther,divisionName,provinceName,memberName;//高级检索条件
	...
	public String publish() {
		...
		for(int i=0;i<entityIds.size();i++){
			...
			if(endinspection != null){
				endinspection.setIsPublished(1);
	...
	public String notPublish() {
		...
		for(int i=0;i<entityIds.size();i++){
			ProjectEndinspection endinspection = (ProjectEndinspection)this.projectService.getCurrentEndinspectionByGrantedId(entityIds.get(i));
			if(endinspection != null){
				endinspection.setIsPublished(0);
				dao.modify(endinspection);
			}else{
				jsonMap.put(GlobalInfo.ERROR_INFO, ProjectInfo.ERROR_END_NOT_EXIST);
			}
	...
	public Object[] advSearchCondition(){
		...
		if(accountType.within(AccountType.EXPERT, AccountType.STUDENT)) {
			hql.append(" and (endi.status >= 1 or endi.isImported=1) ");
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public ProjectEndinspection doWithAdd(ProjectEndinspection endinspection){
		 //保存上传的文件
	...
	public String submit(){
		...
		File endWordFile = new File(ServletActionContext.getServletContext().getRealPath("") + "/" + endinspection.getFile());
...
//				curFile = new File(ServletActionContext.getServletContext().getRealPath("") + "/" + endinspection.getFile());
//			}
//			if(curFile != null) {
...
//		}
	...
	public ProjectEndinspection doWithAddAndModifyResult(ProjectEndinspection endinspection, ProjectGranted granted, int flag){
...
//		return (type == 1) ? SUCCESS : "exportOverView";//打印/导出
	...
	public String confirmPrintOverView(){
		...
		sb.append(" and (endi.status >= 5 or endi.isImported=1) ");
