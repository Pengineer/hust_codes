...
public class SubAction extends AccountAction {
	...
	private static final long serialVersionUID = -8093488243878187869L;
	private static final String HQL = "select a.id, pp.name, p.id, p.name, a.startDate, a.expireDate, a.status, a.lastLoginDate from Account a left join a.passport pp, Officer o left join o.person p left join o.agency ag where a.officer.id = o.id and (a.type = 'MINISTRY_UNIVERSITY' or a.type = 'LOCAL_UNIVERSITY') and (ag.type = 3 or ag.type = 4) ";
	...
	private static final String PAGE_NAME = "accountUniversitySubPage";
	...
	public Object[] simpleSearchCondition() {
		...
		hql.append(HQL);
		hql.append(" and ");
		if (loginer.getCurrentType().equals(AccountType.ADMINISTRATOR) || loginer.getCurrentType().equals(AccountType.MINISTRY)) {// 系统管理员或部级账号查询所有校级子账号
			hql.append(" 1=1 ");
		} else if (loginer.getCurrentType().equals(AccountType.PROVINCE)) {// 省级账号查询本省地方高校所有校级子账号
			hql.append(" ag.subjection.id = :currentUnitId and ag.type = 4 ");
			map.put("currentUnitId", loginer.getCurrentBelongUnitId());
		} else if ((loginer.getCurrentType().equals(AccountType.MINISTRY_UNIVERSITY) || loginer.getCurrentType().equals(AccountType.LOCAL_UNIVERSITY)) && loginer.getIsPrincipal() == 1) {// 校级主账号查询本校所有校级子账号
			hql.append(" o.agency.id = :currentUnitId ");
			map.put("currentUnitId", loginer.getCurrentBelongUnitId());
		} else {// 其它账号无法查询任何校级子账号
			hql.append(" 1=0 ");
		}
		hql.append(" and ");
