...
//		System.out.println("院系名称"+department.getName());
		...
		if(null != department.getAddress() && department.getAddress().trim().length()>100){
			this.addFieldError("department.address", "院系地址最长为100个字符");
		}
		...
		if(null != department.getPostcode() && !inputValidate.checkPostcode(department.getPostcode().trim())){
			this.addFieldError("agency.postcode","邮政编码输入错误，请重新输入");
		}
//		if(!department.getPostcode().isEmpty()){// 邮政编码格式校验
//			System.out.println("输入的邮政编码为："+department.getPostcode());
//			Pattern p = Pattern.compile("[0-9]{6}");
//			Matcher m = p.matcher(department.getPostcode().trim());
//			if(!m.matches()){
//				this.addFieldError("department.postcode","邮政编码输入不正确，请重新输入");
//			}
//		}
//
//		if (!department.getPhone().isEmpty()){// 电话号码校验
...
//	        boolean b = m.matches();
//	        if(!b){
//	        	this.addFieldError("department.phone","电话号码输入不正确,请重新输入");
//	        }
//		}
//
//		if(!department.getFax().isEmpty()){// 传真格式校验
//			Pattern p = Pattern.compile("^([0-9]{2,3})?(-)?[0-9]{7,8}$");
//			Matcher m = p.matcher(department.getFax().trim());
//			if(!m.matches()){
//				this.addFieldError("department.fax","传真输入不正确，请重新输入");
//			}
//		}
//
//		if(!department.getEmail().isEmpty()){// 邮箱格式校验
//			Pattern p = Pattern.compile("^([a-z0-9A-Z]+[-|\\.]?)+[a-z0-9A-Z]@([a-z0-9A-Z]+(-[a-z0-9A-Z]+)?\\.)+[a-zA-Z]{2,}$");
//			Matcher m = p.matcher(department.getEmail().trim());
//			if(!m.matches()){
//				this.addFieldError("department.email","邮箱输入错误，请重新输入");
//			}
//		}
