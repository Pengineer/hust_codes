...
public class StudentAction extends PersonAction {
	...
	public void viewInner(){
		...
		academic = (Academic) ((Object[]) result.get(0))[6];
		...
		tempAgency.setId(agency.getId());
		tempAgency.setName(agency.getName());
		jsonMap.put("agency", tempAgency);
		if (department != null){
			Department tempDepartment = new Department();
			tempDepartment.setId(department.getId());
			tempDepartment.setName(department.getName());
			jsonMap.put("department", tempDepartment);
			jsonMap.put("institute", null);
		} else if (institute != null) {
			Institute tempInstitute = new Institute();
			tempInstitute.setId(institute.getId());
			tempInstitute.setName(institute.getName());
			jsonMap.put("institute", tempInstitute);
			jsonMap.put("department", null);
		}
	}
	...
	public String toMerge() throws IllegalArgumentException, IntrospectionException, IllegalAccessException, InvocationTargetException{		
		...
		studentMerger.mergeStudent(targetId, new ArrayList<Serializable>(incomeIds));
		//合并结果返回至前台
