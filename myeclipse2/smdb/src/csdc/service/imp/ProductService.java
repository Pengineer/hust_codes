...
public class ProductService extends BaseService implements IProductService {
	...
	public List<Map> getAnnProducts(String grantedId, List<String> annIds) {
		...
		if(null != annIds && !annIds.isEmpty()) {
			List annInfos = dao.query("select ann.applicantSubmitStatus, ann.finalAuditStatus, ann.finalAuditResult, ann.isImported from " +
				"ProjectAnninspection ann where ann.id = ?", annIds.get(0));
			if(null != annInfos && annInfos.size() > 0) {
				Object[] obj = (Object[]) annInfos.get(0);
				int status = (Integer)obj[1], result = (Integer)obj[2], isImported = (Integer)obj[3];
				if(status == 3 && result == 1){//当前年检没通过，能再添加年检成果
					submitStatus = 0;
				} else if(status == 3 && result == 2){//当前年检已通过，不能再添加年检成果
					submitStatus = 3;
				} else if(status != 3 && isImported == 1){//当前年检是录入的，且正在处理，不能添加年检成果
					submitStatus = 3;
				}
			} 
		}
	...
	public List<Map> getMidProducts(String grantedId, List<String> midIds) {
		...
		if(null != midIds && !midIds.isEmpty()) {
			List midInfos = dao.query("select pm.applicantSubmitStatus, pm.finalAuditStatus, pm.finalAuditResult, pm.isImported from " +
			...
			if(null != midInfos && midInfos.size() > 0) {
				...
				if(status == 3 && result == 1){//当前中检没通过，能再添加中检成果
					...
					submitStatus = 3;
				} else if(status != 3 && isImported == 1){//当前中检是录入的，且正在处理，不能添加中检成果
					submitStatus = 3;
				}
			} 
		}
	...
	public List<Map> getEndProducts(String grantedId, List<String> endIds) {
		...
		if(null != endIds && !endIds.isEmpty()) {
			List endInfos = dao.query("select pe.applicantSubmitStatus, pe.finalAuditStatus, pe.finalAuditResultEnd, pe.isImported " +
			...
			if(null != endInfos && endInfos.size() > 0) {
				...
				if(status == 3 && result == 1){//当前结项没通过，能再添加结项成果
					...
					submitStatus = 3;
				} else if(status != 3 && isImported == 1){//当前结项是录入的，且正在处理，不能添加结项成果
					submitStatus = 3;
				}
			} 
		}
	...
	public Boolean canAddAnnProduct(String grantedId){
		ProjectGranted projectGranted = (ProjectGranted)dao.query(ProjectGranted.class, grantedId);
		String hql = "select pa.applicantSubmitStatus, pa.finalAuditStatus, pa.finalAuditResult, pa.isImported " +
		...
		if(anns.size() > 0) {
			...
			int status = (Integer)o[1], result = (Integer)o[2], isImported = (Integer)o[3];
			if(status == 3 && result == 1){//当前年检没通过，能再添加年检成果
				submitStatus = 0;
			} else if(status == 3 && result == 2){//当前年检已通过，不能再添加年检成果
				submitStatus = 3;
			} else if(status != 3 && isImported == 1){//当前年检信息是录入的，且正在处理，不能添加年检成果
				submitStatus = 3;
			}
		}
	...
	public Boolean canAddMidProduct(String grantedId){
		ProjectGranted projectGranted = (ProjectGranted)dao.query(ProjectGranted.class, grantedId);
		String hql = "select pm.applicantSubmitStatus, pm.finalAuditStatus, pm.finalAuditResult, pm.isImported " +
	...
	public Boolean canAddEndProduct(String grantedId){
		ProjectGranted projectGranted = (ProjectGranted)dao.query(ProjectGranted.class, grantedId);
		String hql = "select pe.applicantSubmitStatus, pe.finalAuditStatus, pe.finalAuditResultEnd, pe.isImported " +
	...
	public void auditProjectProduct(Account account, String productId, String grantedId, int viewType, String inspectionId, int auditResult, int firstAuditLevel, int finalAuditLevel) {
		if(viewType == 1) {//年检成果
			ProjectAnninspectionProduct projectAnninspectionProduct = (ProjectAnninspectionProduct)dao.queryUnique(
				"from ProjectAnninspectionProduct pap where pap.projectMidinspection.id = ? and pap.product.id = ?", inspectionId, productId);
