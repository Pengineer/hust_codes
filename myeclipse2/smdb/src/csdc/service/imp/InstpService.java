...
public class InstpService extends ProjectService implements IInstpService  {
	...
	public List getDireProjectListByGrantedId(String projectid, String personid){
		...
		String hql1 = "select gra.name, app.englishName, app.year, gra.applicantName, " +
			"gra.university.name, gra.divisionName from InstpGranted gra, " +
			"InstpApplication app, InstpMember mem where " +
	...
	public List getDireProjectListByGrantedId(String projectid){
		...
		String hql1 = "select gra.name, app.englishName, app.year, gra.applicantName, " +
				"gra.university.name, gra.divisionName from InstpGranted gra, " +
				"InstpApplication app where gra.application.id=app.id and gra.id=:projectid ";
		List projectList = dao.query(hql1, map1);
