...
public class OfficerMerger extends PersonMerger  {
	...
	private Map<String, MergeStrategy> msMap = new HashMap<String, MergeStrategy>();	//Officer对象合并策略
		
	public OfficerMerger(){
		msMap.put("position", BuiltinMergeStrategies.APPEND);
		msMap.put("staffCardNumber", BuiltinMergeStrategies.SUPPLY);
		...
		msMap.put("organ", BuiltinMergeStrategies.SUPPLY);
	}
	...
	public void mergeOfficer(Serializable targetOfficerId, List<Serializable> incomeOfficerIds) throws IllegalArgumentException, IntrospectionException, IllegalAccessException, InvocationTargetException {
		...
		for (Officer incomeOfficer : incomeOfficers) {
			...
			for (String fieldName : msMap.keySet()) {
				beanFieldUtils.mergeField(tmpOfficer, incomeOfficer, fieldName, msMap.get(fieldName));
			}
		}
		//合并人员表
