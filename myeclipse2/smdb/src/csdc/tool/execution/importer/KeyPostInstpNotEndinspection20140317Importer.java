...
//		validateData();
	...
	private void importData() throws Exception {
		...
		while (next(excelReader)) {
			...
			//2013年之前的数据库中已存在，故仅处理2013年数据（其中有60条只有申报没有立项，6条既无申报也无立项）
			if ("2013".equals(C)) {
...
//		km.setSpecialistTitle(academic.getSpecialityTitle());
	...
	private void checkProjectExistence() throws Exception {
		...
		while (next(excelReader)) {
			...
			if (granted == null) {
				...
				if (keyApplication == null) {
					System.out.println("找不到的项目的申报数据：" + D + " - " + G + " - " + A);
					m++;
				}
			} else {
		...
		System.out.println("找不到立项也找不到申报的项目数"+m);
		...
		while (next(excelReader)) {
			...
			if (granted == null) {
				...
				if (postApplication == null) {
					System.out.println("找不到的项目的申报数据：" + A + " - " + B + "立项年度：" + C);
					m++;
				}
			} else {
				if (granted.getStatus() == 1) {
