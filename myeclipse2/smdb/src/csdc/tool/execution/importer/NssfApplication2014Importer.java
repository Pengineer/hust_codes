...
/**
 * 1、Excel:《2014年国家社科基金申报数据（全部）.xls》入库
 * 2、2014年国社科申报数据查重：
 *  初审规则：申请国家社科基金的负责人不能同时申请同年度的教育部一般项目和基地项目。
 * @author wangyi
 * @status 
 */
public class NssfApplication2014Importer extends Importer {
	/**
	 * 《2014年国家社科基金申报数据（全部）.xls》
	 */
	...
	public NssfApplication2014Importer() {}
	
	public NssfApplication2014Importer(String filePath) {
	...
	private void importData() throws Exception {
		...
		while (next(excelReader)) {
			...
			nApplication.setName(A);
			nApplication.setDiscipline(B);
			nApplication.setApplicant(C);
			nApplication.setGender(D);
			nApplication.setNational(E);
			nApplication.setBirthday(tool.getDate(F));
			nApplication.setProvince(G);
			nApplication.setUnit(H);
			nApplication.setYear(2014);
			Agency univ = null;
			List<SinossProjectApplication> spApplications = null;
			univ = universityFinder.getUniversityWithLongestName(H);
			if (univ != null) {
				...
				spApplications = (List<SinossProjectApplication>) applicantUnivMap.get(C + H);
			}
