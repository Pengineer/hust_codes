...
public class FirstAuditSpecialApplication2014 extends Execution{
	...
	private Map<String, List<String>> generalApplicationMap = null;//2014年一般项目的申报
...
//			}			
		...
		System.out.println("当前年申报的一般项目专项任务负责人所属高校未找到高校代码的共：" + notFindUniversityCodes.size() + "条");
...
//		generalMap = null;
	...
	private String checkDuplicate(String key){
		...
		if (nssfApplicationMap.containsKey(key)) {
			String reason_post = "国家社科基金项目申报（" + StringTool.join(nssfApplicationMap.get(key), "; ") + "）";
			firstAuditResult += (firstAuditResult.isEmpty()) ? reason_post : "; " + reason_post; 
		}
		if (generalApplicationMap.containsKey(key)) {
			String reason_post = "2014年一般项目申报（" + StringTool.join(generalApplicationMap.get(key), "; ") + "）";
			firstAuditResult += (firstAuditResult.isEmpty()) ? reason_post : "; " + reason_post;
		}
