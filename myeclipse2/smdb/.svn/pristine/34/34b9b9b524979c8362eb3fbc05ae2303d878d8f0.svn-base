...
define(function(require, exports, module) {
	...
	var submitOrSaveReviewResult = function(data, type){//奖励评审结果是否提交;data：2保存；3提交;type：1添加；2修改
		if(!$("#review_awardform").valid()){
			return;
		}
		$("#submitStatus").val(data);
		var url="";
		if(type==1){
			url="award/moesocial/application/review/addResult.action";
		}else if(type==2){
			url="award/moesocial/application/review/modifyResult.action";
		}
		if(checkReview()){
	...
	exports.init = function() {
		...
		$(".j_submitAddReviewResult").live("click", function(){
