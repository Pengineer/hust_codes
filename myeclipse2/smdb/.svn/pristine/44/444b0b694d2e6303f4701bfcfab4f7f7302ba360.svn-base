		...
		<table id="list_instp_member" width="100%" border="0" cellspacing="0" cellpadding="0" class="table_td_padding" style="text-align:center;">
			<thead>
				<tr class="table_title_tr2">
					...
					<td width="2"><img src="image/table_line1.gif" width="2" height="24" /></td>
					<td><s:text name="i18n_IsSubprojectDirector" /></td>
				</tr>
			</thead>
			<tbody>
					...
					<tr>
						...
						<td>
							...
							{elseif item[9] != "" && item[9] != null}
								<a id="${item[9]}" class="linkIns" href="" title="<s:text name='i18n_ViewDetails' />">${item[10]}</a>
							{else}
								${item[10]}
							{/if}
						</td>
						...
						<td></td>
						<td>{if item[13] == 1}<s:text name="i18n_Yes" />{else}<s:text name="i18n_No" />{/if}</td>
					</tr>
			{forelse}
				<tr>
					<td align="center"><s:text name="i18n_NoRecords" /></td>
