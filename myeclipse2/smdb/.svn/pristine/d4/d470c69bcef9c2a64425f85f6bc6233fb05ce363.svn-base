...
<s:form id="search" theme="simple" action="list" namespace="/account/department/sub">
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_td_padding">
			<tr class="table_main_tr">
				<td align="right"><span class="choose_bar">
					<s:if test="#session.loginer.currentType.compareWith(@csdc.tool.bean.AccountType@MINISTRY_UNIVERSITY) == 1">
						<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{getText('不限')}--"
							list="#{'1':getText('用户名'),'2':getText('所属高校'),'3':getText('所属院系'),'4':getText('所属人员')}" />
					</s:if>
					<s:elseif test="#session.loginer.currentType.compareWith(@csdc.tool.bean.AccountType@DEPARTMENT) == 1">
						<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{getText('不限')}--"
							list="#{'1':getText('用户名'),'3':getText('所属院系'),'4':getText('所属人员')}" />
					</s:elseif>
					<s:else>
						<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{getText('不限')}--"
							list="#{'1':getText('用户名'),'4':getText('所属人员')}" />
					</s:else>
...
<div id="adv_search" style="display:none;"><!-- 高级检索 -->	
	<s:include value="/security/account/searchSub.jsp" />
</div>