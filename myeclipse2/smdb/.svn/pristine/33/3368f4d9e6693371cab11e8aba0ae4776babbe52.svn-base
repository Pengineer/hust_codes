		...
		{if midinspection_index == 0}<!-- 最近一次中检 -->
			...
			<s:elseif test="#session.loginer.currentType.equals(@csdc.tool.bean.AccountType@MINISTRY)"><!-- 部级 -->
				<sec:authorize ifAllGranted='ROLE_PROJECT_GENERAL_MIDINSPECTION_DATA_ADD'><!-- 录入结项 -->
					...
					{elseif midPending!=0}
						<div>
							<s:text name="i18n_CanNotAddMidResForInDeal" />
				...
				<div class="p_box_b_2">
					...
					{if midinspection.file != null}
						<div style="float: right;"><img src="image/ico09.gif" />
							<a href="" id="${midinspection.id}" class="download_general_2"><s:text name='i18n_DownloadMidinspection' /></a>
	...
	{forelse}<!-- 无中检信息 -->
		...
		<s:if test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@EXPERT, @csdc.tool.bean.AccountType@STUDENT)"><!-- 研究人员-->
			{if isDirector == 1 && endPassAlready != 1 && endPending != 1 && midPassAlready != 1 && midPending != 1 && midForbid != 1 && granted != null && granted.status == 1}
				<div id="applyMid">
					<div class="p_box_b_2"><a class="j_downloadMidTemplate"  style="cursor: pointer;"><s:text name='i18n_DownloadMidFileTemplate'/></a></div>
					<sec:authorize ifAllGranted='ROLE_PROJECT_GENERAL_MIDINSPECTION_APPLY_ADD'>
						<div class="p_box_b_1"><input type="button" class="btn1 j_toAddMidApplyPop" value="<s:text name='i18n_Apply' />"/></div>
					</sec:authorize>
				</div>
			{else}
				<div class="p_box_body">
