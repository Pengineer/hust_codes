...
<div id="simple_search">
	<s:form id="search" theme="simple" action="list" namespace="/project/post/anninspection/apply">
					...
					<span class="choose_bar">
						<s:if test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@MINISTRY, @csdc.tool.bean.AccountType@INSTITUTE)"><!-- 管理人员-->
							<s:select cssClass="select" id="mainFlag" name="mainFlag" headerKey="" headerValue="--所有项目--"
								list="#{'0351':getText('已上报'),'0352':getText('待审核'),'0353':getText('已审核')}" />
						</s:if>
						<s:if test="!#session.loginer.currentType.within(@csdc.tool.bean.AccountType@MINISTRY_UNIVERSITY, @csdc.tool.bean.AccountType@INSTITUTE)">
							<s:select cssClass="select" name="searchType" headerKey="-1" headerValue="--%{getText('不限')}--"
								list="#{'1':getText('项目名称'),'2':getText('负责人'),'3':getText('依托高校'),'4':getText('项目子类'),'5':getText('学科门类'),'6':getText('项目年度')}" />
						</s:if>
						<s:else>
							<s:select cssClass="select" name="searchType" headerKey="-1" headerValue="--%{getText('不限')}--"
								list="#{'1':getText('项目名称'),'2':getText('负责人'),'4':getText('项目子类'),'5':getText('学科门类'),'6':getText('项目年度')}" />
						</s:else>
					</span>
...
<div id="adv_search" style="display:none;">
	<s:form id="advSearch" action="advSearch" theme="simple" namespace="/project/post/anninspection/apply">
		...
		<s:hidden name="listType" value="11" />
		<div class="adv_content">
			...
			<table class="adv_table" width="100%" border="0" cellspacing="2" cellpadding="0">
				...
				<tr class="adv_tr">
					...
					<td class="adv_td1"><s:select cssClass="select" name="projectSubtype" value="%{searchQuery.projectSubtype}" list="%{baseService.getSystemOptionMap('projectType', '03')}" headerKey="-1" headerValue="--%{getText('不限')}--" /></td>
					<td class="adv_td1"></td>
			...
			<s:if test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@EXPERT, @csdc.tool.bean.AccountType@STUDENT)"><!-- 研究人员-->
				<tr class="adv_tr">
					...
					<td class="adv_td1">
						   <s:select cssClass="select" name="applicantSubmitStatus" value="%{searchQuery.submitStatus}" list="#{'3':getText('提交'),'2':getText('暂存'),'1':getText('退回')}" headerKey="-1" headerValue="--%{getText('不限')}--" />
			   		</td>
					<td class="adv_td1"></td>
					<td class="adv_td1" align="right">年检状态：</td>
					<td class="adv_td1">
			...
			<s:else>
				<tr class="adv_tr">
					<td class="adv_td1" align="right"><s:if test="#session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@PROVINCE)<0">年检状态</s:if> <s:else>审核状态</s:else>：</td>
					<td class="adv_td1">
					...
					<td class="adv_td1"></td>
					<td class="adv_td1" align="right"><s:if test="#session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@PROVINCE)<0">年检时间</s:if> <s:else>审核时间</s:else>：</td>
					<td class="adv_td1">
