...
public class SubAction extends AccountAction {
	...
	private static final String HQL = "select a.id, pp.name, p.id, p.name, a.startDate, a.expireDate, a.status, a.lastLoginDate from Account a left join a.passport pp, Officer o left join o.person p left join o.agency ag where a.officer.id = o.id and ag.type = 1 ";
	...
	private static final String PAGE_NAME = "accountMinistrySubPage";
	...
	public Object[] simpleSearchCondition(){
		...
		if (loginer.getCurrentType().equals(AccountType.ADMINISTRATOR)) {// 系统管理员查询所有部级主账号
			hql.append(" 1=1 ");
		} else if (loginer.getCurrentType().equals(AccountType.MINISTRY) && loginer.getIsPrincipal() == 1) {// 部级主账号查询本单位所有部级子账号
