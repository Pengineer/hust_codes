...
/**
 * Excel：《2013年全国教育规划立项课题鉴定情况一览表.xls》
 * @author wangyi
 * @status 
 * 备注：
 ...
 */
public class NssfNespEndinspection2013Importer extends Importer {
	...
	public NssfNespEndinspection2013Importer() {}
	
	public NssfNespEndinspection2013Importer(String filePath) {
	...
	private void importData() throws Exception {
		...
		while (next(excelReader)) {
			...
			if (nssf == null) {
				...
				if (nssfId != null) {//项目名称和申报人都相同 则更新
			...
			if (nssf != null) {
				...
				if (L.length() > 0) {
					String ss = L.replaceAll("\\s+", "").replaceAll("、", "; ");
	...
	private void checkProjectExistence() throws Exception {
		...
		int k = 0;
		int m = 0;
		List applicantList = dao.query("select nssf.applicant from Nssf nssf");
