...
<div id="simple_search">
	<s:form id="search" theme="simple" action="list" namespace="/project/post/application/granted">
				...
				<td align="right"><span class="choose_bar">
					<s:if test="!#session.loginer.currentType.within(@csdc.tool.bean.AccountType@MINISTRY_UNIVERSITY, @csdc.tool.bean.AccountType@INSTITUTE)">
						<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{getText('不限')}--"
							list="#{'1':getText('批准号'),'2':getText('项目名称'),'3':getText('负责人'),'4':getText('依托高校'),'5':getText('项目子类'),'6':getText('学科门类'),'7':getText('项目年度')}" />
					</s:if>
					<s:else>
						<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{getText('不限')}--"
							list="#{'1':getText('批准号'),'2':getText('项目名称'),'3':getText('负责人'),'5':getText('项目子类'),'6':getText('学科门类'),'7':getText('项目年度')}" />
					</s:else>
