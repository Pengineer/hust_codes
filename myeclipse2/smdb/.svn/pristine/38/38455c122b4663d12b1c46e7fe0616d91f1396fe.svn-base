...
public abstract class AnninspectionApplyAction extends ProjectBaseAction{
	...
	public Object[] advSearchCondition(){
		...
		if(accountType.within(AccountType.EXPERT, AccountType.STUDENT)) {
			hql.append(" and (ann.status >= 1 or ann.isImported = 1) ");
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public ProjectAnninspection doWithAdd(ProjectAnninspection anninspection){
		 //保存上传的文件
		String groupId = "file_add";
...
//		}
	...
	public ProjectAnninspection doWithAddResult(ProjectAnninspection anninspection){
		 //保存上传的文件
		String groupId = "file_add";
		List<String> files = new ArrayList<String>();
		for (FileRecord fileRecord : uploadService.getGroupFiles(groupId)) {
		...
		anninspection.setImportedDate(new Date());
		...
		anninspection.setIsImported(1);//设为导入数据
