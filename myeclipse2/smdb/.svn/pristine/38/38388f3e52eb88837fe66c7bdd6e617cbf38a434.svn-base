...
<textarea id="view_end_template" style="display:none;">
	...
	<s:hidden name="endStatus" id="endStatus" value="%{projectService.getBusinessStatus('043', entityId)}"/>
	{for endinspection in endList}
		{if endinspection_index == 0}<!-- 最近一次结项 -->
			...
			<s:if test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@EXPERT, @csdc.tool.bean.AccountType@STUDENT)"><!-- 研究人员-->
				{if isDirector == 1 && endinspection.finalAuditResultEnd == 1 && endinspection.finalAuditStatus == 3 && endPassAlready != 1 && endPending != 1 && (midPassAlready == 1 || endAllow == 1) && varPending==0 && granted != null && granted.status == 1}
					<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_ENDINSPECTION_APPLY_ADD'>
						<div class="p_box_b_2">
			...
			<s:elseif test="#session.loginer.currentType.equals(@csdc.tool.bean.AccountType@MINISTRY)"><!-- 部级 -->
				<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_ENDINSPECTION_DATA_ADD'>
		...
		<div>
			<div class="p_box_b">
					...
					{if endinspection.isImported==0}
						...
						{if endinspection_index == 0 && endinspection.finalAuditStatus != 3}
							{if reviewflag == -1 && ((accountType == "MINISTRY" && endinspection.status > 5) || (accountType == "PROVINCE" && endinspection.status > 4) || ((accountType =="MINISTRY_UNIVERSITY" || accountType == "LOCAL_UNIVERSITY") && endinspection.status > 3))}
								<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_ENDINSPECTION_REVIEW_ADD'>
							...
							{elseif (reviewflag == 22 && accountType == "MINISTRY" && endinspection.status > 5) || (reviewflag == 32 && accountType == "PROVINCE" && endinspection.status > 4) || (reviewflag == 42 && (accountType =="MINISTRY_UNIVERSITY" || accountType == "LOCAL_UNIVERSITY") && endinspection.status > 3)}
								<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_ENDINSPECTION_REVIEW_MODIFY'>
					...
					{elseif endinspection.isImported==1}
						...
						{if endinspection_index == 0 && endinspection.finalAuditStatus==2 && granted != null && granted.status == 1}
							{if endPassAlready == 0 && accountType == "MINISTRY" && (midPassAlready == 1 || endAllow == 1)}
								<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_ENDINSPECTION_DATA_MODIFY'>
				...
				<div class="p_box_b_2">
				<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_ENDINSPECTION_APPLY_VIEW'>
					{if endinspection.file != null}
						<div style="float: right;"><img src="image/ico09.gif" />
							<a href="" id="${endinspection.id}" class="download_key_3">下载结项申请书</a>
					...
					<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_ENDINSPECTION_DATA_UPLOAD'>
						...
						<tbody>
							<tr class="table_tr7">
								...
								<td colspan="4" style="text-align:center">
									<img style="vertical-align:middle;" src="image/ico09.gif" /><span><a href="" id="${projectDatas[endinspection_index].id}" name="${projectDatas[endinspection_index].file}" class="download_key_5">下载研究数据包</span></a>
								...
								<td align="right">
									{if isDirector == 1 && granted != null && granted.status == 1}
										{if endPassAlready != 1 && (midPassAlready == 1 || endAllow == 1)}
											<sec:authorize ifAllGranted="ROLE_PROJECT_KEY_ENDINSPECTION_APPLY_MODIFY">
											...
											{/if}
										{/if}
										<sec:authorize ifAllGranted="ROLE_PROJECT_KEY_ENDINSPECTION_APPLY_DELETE">
			...
			<s:elseif test="#session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@EXPERT)<0 ">
				<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_ENDINSPECTION_APPLYAUDIT_VIEW'>
					<div class="p_box_body">
						<table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" style="border-collapse:collapse;">
							<thead>
								<tr class="head_title">
										...
										{if endinspection.isImported == 0 && endinspection_index == 0 && (accountType == "DEPARTMENT" || accountType == "INSTITUTE") && endinspection.status == 2 && endinspection.finalAuditStatus != 3}
											{if endPassAlready != 1 && granted != null && granted.status == 1}
												{if endinspection.deptInstAuditStatus == 0}
													<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_ENDINSPECTION_APPLYAUDIT_ADD'>
												...
												{elseif endinspection.deptInstAuditStatus < 3}
													...
													<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_ENDINSPECTION_APPLYAUDIT_MODIFY'>
			...
			<!-- 专家评审信息-->
			<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_ENDINSPECTION_REVIEW_VIEW'>
										...
										{if endinspection.isImported == 0 && endinspection_index == 0 && endinspection.status == 6 && endinspection.finalAuditStatus != 3 && endinspectionReview[endinspection_index].submitStatus != 3}
											{if endPassAlready != 1 && granted != null && granted.status == 1}
												...
												{elseif endinspectionReview[endinspection_index].submitStatus < 3}
													...
													<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_ENDINSPECTION_REVIEW_MODIFY'>
														<input class="btn1 j_modifyEndReviewPop" endId="${endinspection.id}" data="${endinspectionReview[endinspection_index].id}" type="button" value="修改" />
													</sec:authorize>
													<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_ENDINSPECTION_REVIEW_ADD'>
								...
								{if endinspectionReviews[endinspection_index] != null}
									{for item in endinspectionReviews[endinspection_index]}
										<tr class="table_tr4">
											...
											<td>
												{if item[5]!=0}
													<span class="ico_txt2"><a class="j_viewEndReviewPop" data="${item[0]}" style="cursor: pointer;">查看详情</a></span>
												{/if}
											</td>
										</tr>
									{/for}
							...
							<tr class="table_tr4">
								...
								<td>
									...
									{if endinspection.isImported == 0 && endinspection_index == 0 && accountType == "MINISTRY" && endinspection.status == 7 && endinspection.finalAuditStatus != 3}
										<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_ENDINSPECTION_REVIEWAUDIT_VIEW'>
											{if endPassAlready != 1 && granted != null && granted.status != 3 && granted.status != 4}
												{if endinspection.finalAuditStatus==0}
													<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_ENDINSPECTION_REVIEWAUDIT_ADD'>
												...
												{elseif endinspection.finalAuditStatus < 3}
													...
													<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_ENDINSPECTION_REVIEWAUDIT_MODIFY'>
									...
									{elseif endinspection.finalAuditStatus > 0}
										<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_ENDINSPECTION_DATA_VIEW'>
											<span class="ico_txt2"><a class="j_viewEndReviewAuditPop" endId="${endinspection.id}"  style="cursor: pointer;">查看详情</a></span>
										</sec:authorize>
			...
			{/if}
			<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_ENDINSPECTION_DATA_PRINT'>
					...
					<div style="margin-top:10px;margin-bottom:5px;">附：结项成果
						...
						<span>项
							{if endProdInfo[endinspection_index].productSize!=0}(&nbsp;
								...
								{if endProdInfo[endinspection_index].electronicSize != null && endProdInfo[endinspection_index].electronicSize != 0}著作${endProdInfo[endinspection_index].electronicSize}项&nbsp;{/if}
								{if endProdInfo[endinspection_index].patentSize != null && endProdInfo[endinspection_index].patentSize != 0}著作${endProdInfo[endinspection_index].patentSize}项&nbsp;{/if}
								{if endProdInfo[endinspection_index].otherProductSize != null && endProdInfo[endinspection_index].otherProductSize != 0}著作${endProdInfo[endinspection_index].otherProductSize}项&nbsp;{/if}
								)
							{/if}
						</span>
