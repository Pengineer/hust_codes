 ...
 * @author	fengcl
 */
public final class ProjectMidinspectionTrainer extends OnlineLogisticRegressionTrainer{
	 ...
	 */
	public ProjectMidinspectionTrainer(String predictType, String predictorVariables){
	...
	protected Object[] modelParameters() {
		...
		if (predictorVariables.contains("YEARS")) {//项目进行年数（已通过中检项目：立项时间->中检通过时间；未通过中检项目：立项时间->现在时间）
		...
		lmp.setMaxTargetCategories(6);	//目标变量的数量（这里是中检所需的年数）。
		lmp.setNumFeatures(1000);	//设置用于构建模型的特征向量大小，当输入为text-like类型的值时，大的值是更好的。
