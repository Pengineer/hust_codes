...
public class SubAction extends AccountAction {
	...
	private static final String HQL = "select a.id, pp.name, p.id, p.name, a.startDate, a.expireDate, a.status, a.lastLoginDate from Account a left join a.passport pp, Officer o left join o.person p left join o.department d left join o.agency ag where a.officer.id = o.id and a.type = 'DEPARTMENT' ";
	...
	private static final String PAGE_NAME = "accountDepartmentSubPage";
	...
	public Object[] simpleSearchCondition() {
		...
		hql.append(HQL);
		hql.append(" and ");
		if (loginer.getCurrentType().equals(AccountType.ADMINISTRATOR) || loginer.getCurrentType().equals(AccountType.MINISTRY)) {// 系统管理员或部级账号查询所有院系子账号
			hql.append(" 1=1 ");
		} else if (loginer.getCurrentType().equals(AccountType.PROVINCE)) {// 省级账号查询本省地方高校所有院系子账号
			hql.append(" ag.subjection.id = :currentUnitId and ag.type = 4 ");
			map.put("currentUnitId", loginer.getCurrentBelongUnitId());
		} else if (loginer.getCurrentType().equals(AccountType.MINISTRY_UNIVERSITY) || loginer.getCurrentType().equals(AccountType.LOCAL_UNIVERSITY)) {// 校级账号查询本校所有院系子账号
			hql.append(" ag.id = :currentUnitId ");
			map.put("currentUnitId", loginer.getCurrentBelongUnitId());
		} else if (loginer.getCurrentType().equals(AccountType.DEPARTMENT) && loginer.getIsPrincipal() == 1) {// 院系主账号查询本院系所有子账号
			hql.append(" o.department.id = :currentUnitId ");
			map.put("currentUnitId", loginer.getCurrentBelongUnitId());
		} else {// 其它账号无法查询任何院系子账号
			hql.append(" 1=0 ");
		}
		hql.append(" and ");
		if (searchType == 1) {// 按账号名检索
			...
			hql.append(" (LOWER(pp.name) like :keyword or LOWER(ag.name) like :keyword or LOWER(d.name) like :keyword or LOWER(p.name) like :keyword) ");
			map.put("keyword", "%" + keyword + "%");
		}
...
//		return SUCCESS;
	...
	public Object[] advSearchCondition() {
		...
		if (belongUnitName != null && !belongUnitName.isEmpty()) {// 按高校名称或院系名称检索
			...
			hql.append(" and (LOWER(d.name) like :belongUnitName or LOWER(ag.name) like :belongUnitName) ");
