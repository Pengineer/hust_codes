...
<div id="simple_search">
	<s:form id="search" theme="simple" action="list" namespace="/product/paper">
					...
					<span class="choose_bar">
						...
						<s:if test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@EXPERT, @csdc.tool.bean.AccountType@STUDENT)">
							<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{getText('不限')}--"
								list="#{'1': getText('论文名称'), '2': getText('论文类型'), '4': getText('所属单位'), '5': getText('发表刊物')}"/>
						</s:if>
						<!-- 校级管理人员 -->
						<s:elseif test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@MINISTRY_UNIVERSITY, @csdc.tool.bean.AccountType@LOCAL_UNIVERSITY)">
							<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{getText('不限')}--"
								list="#{'1': getText('论文名称'), '2': getText('论文类型'), '3': getText('第一作者'), '6': getText('所属部门'), '5': getText('发表刊物')}"/>
						</s:elseif>
						<!-- 其它人员 -->
						<s:else>
							<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{getText('不限')}--"
								list="#{'1': getText('论文名称'), '2': getText('论文类型'), '3': getText('第一作者'), '4': getText('所属单位'), '5': getText('发表刊物')}"/>
						</s:else>
					</span>
...
<div id="adv_search" style="display:none;">
	<s:form id="advSearch" action="advSearch" theme="simple" namespace="/product/paper">
		...
		<div class="adv_content">
			...
			<table class="adv_table" width="100%" border="0" cellspacing="2" cellpadding="0">
				<tr class="adv_tr">
					<td class="adv_td1" width="65" align="right">论文名称：</td>
					<td class="adv_td1" width="200"><s:textfield name="paper.chineseName" value="%{searchQuery.chineseName}" cssClass="input_css"/></td>
					<td class="adv_td1" width="100"></td>
					<td class="adv_td1" width="65" align="right">论文类型：</td>
					<td class="adv_td1" width="200">
						<s:select cssClass="select" list="%{baseService.getSystemOptionMap('productType', '01')}" name="paper.type.id" value="%{searchQuery.ptype}"
				...
				<tr class="adv_tr">
					...
					<td class="adv_td1">
						...
						<s:hidden name="paper.disciplineType"  value="%{searchQuery.disciplineType}" id="dispName"/>
					</td>
					<td class="adv_td1"></td>
					<td class="adv_td1" align="right">第一作者：</td>
					<td class="adv_td1"><s:textfield name="paper.authorName" value="%{searchQuery.authorName}" cssClass="input_css"/></td>
					<td class="adv_td1"></td>
				</tr>
				<tr class="adv_tr">
					<td class="adv_td1" align="right">发表刊物：</td>
					<td class="adv_td1"><s:textfield name="paper.publication" value="%{searchQuery.publication}" cssClass="input_css"/></td>
					<td class="adv_td1"></td>
					<td class="adv_td1" align="right">发表时间：</td>
				...
				<tr class="adv_tr">
					<s:if test=" #session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@MINISTRY_UNIVERSITY)<0 || #session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@LOCAL_UNIVERSITY)>0">
						...
						<td class="adv_td1"><s:textfield name="paper.agencyName" value="%{searchQuery.agencyName}" cssClass="input_css"/></td>
						<td class="adv_td1"></td>
					</s:if>
				<!-- 非院系、基地管理员-->
					<s:if test=" #session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@DEPARTMENT)<0 || #session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@INSTITUTE)>0">
						...
						<td class="adv_td1"><s:textfield name="paper.divisionName" value="%{searchQuery.divisionName}" cssClass="input_css"/></td>
						<td class="adv_td1"></td>
					</s:if> 
				</tr>
				<!-- 研究人员 -->
				<s:if test=" #session.loginer.currentType.within(@csdc.tool.bean.AccountType@EXPERT, @csdc.tool.bean.AccountType@STUDENT)">
					<tr class="adv_tr">
						...
						<td class="adv_td2">
							<s:select cssClass="select" name="paper.submitStatus" value="%{searchQuery.submitStatus}" list="#{'2': getText('保存'), '3': getText('已提交')}"
								headerKey="-1" headerValue="--%{getText('不限')}--"/></td>
						<td class="adv_td2"></td>
					</tr>
				</s:if> 
				...
				<s:else>
					<tr class="adv_tr">
						...
						<td class="adv_td1">
							<s:select cssClass="select" name="paper.auditResult" value="%{searchQuery.auditResult}" list="#{'0':getText('待审'),'1':getText('不同意'),'2':getText('同意')}"
								headerKey="-1" headerValue="--%{getText('不限')}--"/></td>
						<td class="adv_td1"></td>
				...
				<tr class="adv_tr">
					<s:if test="!#session.loginer.currentType.within(@csdc.tool.bean.AccountType@MINISTRY_UNIVERSITY, @csdc.tool.bean.AccountType@STUDENT)">						
						...
						<td class="adv_td2"><s:textfield name="paper.provinceName" value="%{searchQuery.provinceName}" cssClass="input_css" /></td>
						<td class="adv_td2"></td>
					</s:if>
				</tr>	
			</table>
