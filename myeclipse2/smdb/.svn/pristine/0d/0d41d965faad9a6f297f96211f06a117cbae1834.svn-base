...
public class PostProjectApplication2013Importer extends Importer {
	...
	private void importData() throws Exception {
		...
		while (next(reader)) if (!A.isEmpty()) {
			...
			Teacher applicant = univPersonFinder.findTeacher(F, university);
			...
			} else if (B.contains("重点")) {
				application.setSubtype(重点);
			} else if (B.contains("重大")) {
			...
			application.setApplicantSubmitDate(tool.getDate(2013,4,17));
			application.setFinalAuditDate(tool.getDate(2013,4,17));
