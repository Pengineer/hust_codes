...
public abstract class ApplicationReviewAction extends ProjectBaseAction{
	...
	protected String appRevId;//申报评审id
	...
	public Object[] simpleSearchCondition() {
		...
		hql.append(" and (app.status >= 6 or app.isImported = 1) ");
...
//		opinionList = this.projectService.getSOByParentName("一般项目申报评审定性意见");
		return SUCCESS;
	}
...
//		session.put("opinionList", opinionList);
	...
	public ProjectApplication doWithAddOrModifyResult(ProjectApplication projectApplication){
		...
		if(submitStatus == 3 && projectApplication.getIsImported() != 1 && projectApplication.getStatus() == 6){
...
//			ProjectEndinspectionReview endRev = (ProjectEndinspectionReview)this.dao.query(ProjectEndinspectionReview.class,entityIds.get(i));
//			//填充评审信息
//			endRev = (ProjectEndinspectionReview) this.fillReviewInfos(endRev, i);
//			this.projectService.modify(endRev);
//		}
//		endinspection = (ProjectEndinspection) this.dao.query(ProjectEndinspection.class, endId);
//		//保存总评信息
//		endinspection = (ProjectEndinspection) this.fillTotalReviewInfos(endinspection);
//		//查询各专家分数，计算总分、均分及等级
//		double[] reviewScore = this.projectService.getEndReviewScore(endId);
//		double reviewTotalScore = reviewScore[0];
//		double reviewAvgScore = reviewScore[1];
//		String reviewGrade = "";
//		reviewGrade = this.doWithReviewGrade(reviewAvgScore);
//		endinspection.setReviewTotalScore(reviewTotalScore);
//		endinspection.setReviewAverageScore(reviewAvgScore);
//		SystemOption so = (SystemOption)this.dao.query(SystemOption.class, reviewGrade);
//		endinspection.setReviewGrade(so);
//		//录入评审提交时判断是否非导入数据，状态是否已在评审状态
//		if(submitStatus == 3 && endinspection.getIsImported() != 1 && endinspection.getStatus() == 6){
//			endinspection.setStatus(endinspection.getStatus() + 1);//结项状态跳一级
//		}
//		this.projectService.modify(endinspection);
//		revResultFlag = 1;
//		session.remove("entityIds");
//		opinionList = this.projectService.getSOByParentName("一般项目结项评审定性意见");
//		return SUCCESS;
//	}
...
//		opinionList = this.projectService.getSOByParentName("一般项目结项评审定性意见");
	...
	public void doWithSubmitResult(){
		...
		if(application.getIsImported() != 1 && application.getStatus() == 6){
