...
//	protected ProjectFee newFee;//变更后经费
	...
	protected String projectName,projectNumber,researchType,projectSubtype,dtypeNames,applicant,university,divisionName,provinceName;//高级检索条件
...
//	取消公开发布
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public ProjectVariation doWithAddOrModify(ProjectVariation variation, int flag){
		if(note != null){
...
//			request.setAttribute("limitedDate", limitedDate);//限制日期
	...
	public ProjectVariation doWithSubmit(ProjectVariation variation){
		//File varWordFile = new File(ServletActionContext.getServletContext().getRealPath("") + "/" + variation.getFile());
	 ...
	 */
	@SuppressWarnings({ "rawtypes" })
	public ProjectVariation doWithAddOrModifyResult(ProjectVariation variation, int flag){
		variation.setIsImported(1);//设为导人数据
...
//				}
		...
		if(submitStatus == 3 || type == 3) {
			...
			if(type == 3) {
				...
				curFile = new File(ServletActionContext.getServletContext().getRealPath("") + "/" + variation.getFile());
			}
			if(curFile != null) {
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public ProjectVariation setVariationInfo(ProjectGranted granted, ProjectVariation variation, String selectIssues, int flag){
		if(selectIssues.indexOf("02") != -1){//变更机构
...
//						}
		...
		if(selectIssueItem.equals("09")){//其他变更
