...
/**
 * 项目申请父类管理：实现了子类需要实现的抽象方法并实现了所有项目申报申请共用的相关方法
 * @author 余潜玉
 */
public abstract class ApplicationApplyAction extends ProjectBaseAction {
	...
	protected Date appDate;//申报时间
	protected String projectName,researchType,projectSubtype,dtypeNames,applicant,university,projectTopic,divisionName,provinceName,memberName;//高级检索条件
	...
	protected int appFlag;//添加申报是否成功标志位	1：添加成功
...
//		File endWordFile = new File(ServletActionContext.getServletContext().getRealPath("") + "/" + application.getFile());
//		projectService.importEndinspectionWordXMLData(endWordFile, projectid, projectType());
	...
	public String toUploadFileResult(){
		GeneralApplication application = (GeneralApplication)dao.query(GeneralApplication.class, entityId);//entityId为项目申请id
		String groupId = "file_add";
		uploadService.resetGroup(groupId);
