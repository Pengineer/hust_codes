...
<div id="simple_search">
	<s:form id="search" theme="simple" action="list" namespace="/project/post/application/apply">
					...
					<span class="choose_bar">
						<s:if test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@MINISTRY, @csdc.tool.bean.AccountType@INSTITUTE)"><!-- 管理人员-->
							<s:select cssClass="select" id="mainFlag" name="mainFlag" headerKey="" headerValue="--所有项目--"
								list="#{'0311':getText('已上报'),'0312':getText('待审核'),'0313':getText('已审核')}" />
						</s:if>
						<s:if test="!#session.loginer.currentType.within(@csdc.tool.bean.AccountType@MINISTRY_UNIVERSITY, @csdc.tool.bean.AccountType@INSTITUTE)">
							<s:select cssClass="select" name="searchType" headerKey="-1" headerValue="--%{getText('不限')}--"
								list="#{'1':getText('项目名称'),'2':getText('申请人'),'3':getText('依托高校'),'4':getText('项目子类'),'5':getText('学科门类'),'6':getText('项目年度')}" />
						</s:if>
						<s:else>
							<s:select cssClass="select" name="searchType" headerKey="-1" headerValue="--%{getText('不限')}--"
								list="#{'1':getText('项目名称'),'2':getText('申请人'),'4':getText('项目子类'),'5':getText('学科门类'),'6':getText('项目年度')}" />
						</s:else>
					</span>
...
<div id="adv_search" style="display:none;">
	<s:form id="advSearch" action="advSearch" theme="simple" namespace="/project/post/application/apply">
		...
		<div class="adv_content">
			...
			<table class="adv_table" width="100%" border="0" cellspacing="2" cellpadding="0">
				<tr class="adv_tr">
					...
					<td class="adv_td1" width="200"><s:select cssClass="select" name="projectSubtype" value="%{searchQuery.projectSubtype}" list="%{baseService.getSystemOptionMap('projectType', '03')}" headerKey="-1" headerValue="--%{getText('不限')}--" /></td>
					<td class="adv_td1"></td>
				</tr>
				<tr class="adv_tr">
					<td class="adv_td1" align="right">项目主题：</td>
					<td class="adv_td1"><s:select cssClass="select" name="projectTopic" value="%{searchQuery.projectTopic}" list="%{baseService.getSystemOptionMap('projectTopic', null)}" headerKey="-1" headerValue="--%{getText('不限')}--" /></td>
					<td class="adv_td1"></td>
				...
				<tr class="adv_tr">
					...
					<td class="adv_td1">
						<s:select cssClass="select" name="isEstab" value="%{searchQuery.isEstab}" 
							list="#{'1':getText('是'), '2':getText('否'), '3':getText('待审')}" 
							headerKey="-1" headerValue="--%{getText('不限')}--"></s:select>
					</td>
					<td class="adv_td2"></td>
