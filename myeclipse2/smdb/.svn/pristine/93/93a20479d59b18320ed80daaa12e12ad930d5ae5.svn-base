...
<textarea id="view_end_template" style="display:none;">
	...
	<s:hidden name="endStatus" id="endStatus" value="%{projectService.getBusinessStatus('033', entityId)}"/>
	{for endinspection in endList}
		{if endinspection_index == 0}<!-- 最近一次结项 -->
			...
			<s:if test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@EXPERT, @csdc.tool.bean.AccountType@STUDENT)"><!-- 研究人员-->
				{if isDirector == 1 && endinspection.finalAuditResultEnd == 1 && endinspection.finalAuditStatus == 3 && endPassAlready != 1 && endPending != 1 && varPending==0 && granted != null && granted.status == 1}
					<sec:authorize ifAllGranted='ROLE_PROJECT_POST_ENDINSPECTION_APPLY_ADD'>
						<div class="p_box_b_2">
							<a href="javascript:void(0);" class="j_downloadEndTemplate" >下载结项申请书模板</a>
						</div>
						<div class="p_box_b_1" id="applyEnd">
							<input type="button" class="btn1 j_toAddEndApplyPop"  value="申报" />
						</div>
					</sec:authorize>
				{/if}
			</s:if>
			...
			<s:elseif test="#session.loginer.currentType.equals(@csdc.tool.bean.AccountType@MINISTRY)"><!-- 部级 -->
				<sec:authorize ifAllGranted='ROLE_PROJECT_POST_ENDINSPECTION_REVIEW_ADD'>
					{if endinspection.finalAuditResultEnd==1 && endinspection.finalAuditStatus==3 && annPending ==0 && endPassAlready==0 && endPending==0 && varPending==0 && reviewPending==0 && granted != null && granted.status == 5 && reviewFlag == 2}
						<div><input class="btn2 j_addEndReviewResultPop" reviewFlag="2" type="button" value="录入鉴定" /><br/></div>
		...
		<div>
			<div class="p_box_b">
					...
					{if endinspection.isImported==0}
						...
						{if endinspection_index == 0 && endinspection.finalAuditStatus != 3}
							{if reviewflag == -1 && ((accountType == "MINISTRY" && endinspection.status > 5) || (accountType == "PROVINCE" && endinspection.status > 4) || ((accountType =="MINISTRY_UNIVERSITY" || accountType == "LOCAL_UNIVERSITY") && endinspection.status > 3))}
								<sec:authorize ifAllGranted='ROLE_PROJECT_POST_ENDINSPECTION_REVIEW_ADD'>
									<span><input class="btn2 j_addEndReviewResultPop" reviewFlag="1" endId="${endinspection.id}" type="button" value="录入鉴定" /></span>
							...
							{elseif (reviewflag == 22 && accountType == "MINISTRY" && endinspection.status > 5) || (reviewflag == 32 && accountType == "PROVINCE" && endinspection.status > 4) || (reviewflag == 42 && (accountType =="MINISTRY_UNIVERSITY" || accountType == "LOCAL_UNIVERSITY") && endinspection.status > 3)}
								<sec:authorize ifAllGranted='ROLE_PROJECT_POST_ENDINSPECTION_REVIEW_MODIFY'>
									<span><input class="btn2 j_modifyEndReviewResultPop" endId="${endinspection.id}" reviewFlag="1" type="button" value="修改鉴定" /></span>
								</sec:authorize>
								<sec:authorize ifAllGranted='ROLE_PROJECT_POST_ENDINSPECTION_REVIEW_ADD'>
									<span><input class="btn2 j_submitEndReviewResult" endId="${endinspection.id}" reviewFlag="1" type="button" value="提交鉴定"/></span>
								</sec:authorize>
							{/if}
						{/if}
					{elseif endinspection.isImported==1 && accountType == "MINISTRY"}
						<span class="import_css">该数据为导入数据</span>
						<!-- 部级最近一次录入结项评审及结项结果 -->	
						{if endinspection_index == 0 &&  granted != null && endPassAlready==0 && endPending == 1}
							{if endinspection.reviewStatus ==2 && granted.status == 1}
								{if reviewPending == 1 && reviewPassAlready == 0}
									<sec:authorize ifAllGranted='ROLE_PROJECT_POST_ENDINSPECTION_REVIEW_MODIFY'>
										<input class="btn2 j_modifyEndReviewResultPop" endId="${endinspection.id}" reviewFlag="2" type="button" value="修改鉴定" />
									</sec:authorize>
									{if varPending == 0}
										<sec:authorize ifAllGranted='ROLE_PROJECT_POST_ENDINSPECTION_REVIEW_ADD'>
											<input class="btn2 j_submitEndReviewResult" endId="${endinspection.id}" reviewFlag="2" type="button" value="提交鉴定" />
										</sec:authorize>
									{/if}
								{/if}
							{elseif endinspection.reviewStatus ==3 && endinspection.reviewResult == 2 && granted.status == 5}
								{if endinspection.finalAuditStatus==0 && varPending == 0}
									<sec:authorize ifAllGranted='ROLE_PROJECT_POST_ENDINSPECTION_DATA_ADD'>
										<input class="btn1 j_addEndResultPop" type="button" value="添加" />
									</sec:authorize>
								{elseif endinspection.finalAuditStatus==2}
									<sec:authorize ifAllGranted='ROLE_PROJECT_POST_ENDINSPECTION_DATA_MODIFY'>
										<input class="btn1 j_modifyEndResultPop" modifyFlag="0" type="button" value="修改" />
									</sec:authorize>
									{if varPending==0}
										<sec:authorize ifAllGranted='ROLE_PROJECT_POST_ENDINSPECTION_DATA_ADD'>
				...
				<div class="p_box_b_2">
				<sec:authorize ifAllGranted='ROLE_PROJECT_POST_ENDINSPECTION_APPLY_VIEW'>
					{if endinspection.file != null}
						<div style="float: right;"><img src="image/ico09.gif" />
							<a href="" id="${endinspection.id}" class="download_post_3">下载结项申请书</a>
					...
					<sec:authorize ifAllGranted='ROLE_PROJECT_POST_ENDINSPECTION_DATA_UPLOAD'>
						...
						<tbody>
							<tr class="table_tr7">
								...
								<td colspan="4" style="text-align:center">
									...
									<a href="" id="${projectDatas[endinspection_index].id}" name="${projectDatas[endinspection_index].file}" class="download_post_5">下载研究数据包</span></a>
							...
							<tr height="36">
								...
								<td align="right">
									{if isDirector == 1 && granted != null && granted.status == 1}
										{if endPassAlready != 1}
											<sec:authorize ifAllGranted="ROLE_PROJECT_POST_ENDINSPECTION_APPLY_MODIFY">
											...
											{/if}
										{/if}
										<sec:authorize ifAllGranted="ROLE_PROJECT_POST_ENDINSPECTION_APPLY_DELETE">
			...
			<s:elseif test="#session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@EXPERT)<0 ">
				<sec:authorize ifAllGranted='ROLE_PROJECT_POST_ENDINSPECTION_APPLYAUDIT_VIEW'>	
					<div class="p_box_body">
						<table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" style="border-collapse:collapse;">
							<thead>
								<tr class="head_title">
										...
										{if endinspection.isImported == 0 && endinspection_index == 0 && (accountType == "DEPARTMENT" || accountType == "INSTITUTE") && endinspection.status == 2 && endinspection.finalAuditStatus != 3}
											{if endPassAlready != 1 && granted != null && granted.status == 1}
												{if endinspection.deptInstAuditStatus == 0}
													<sec:authorize ifAllGranted='ROLE_PROJECT_POST_ENDINSPECTION_APPLYAUDIT_ADD'>
												...
												{elseif endinspection.deptInstAuditStatus < 3}
													...
													<sec:authorize ifAllGranted='ROLE_PROJECT_POST_ENDINSPECTION_APPLYAUDIT_MODIFY'>
			...
			<!-- 专家评审信息-->
			<sec:authorize ifAllGranted='ROLE_PROJECT_POST_ENDINSPECTION_REVIEW_VIEW'>
							...
							<tr class="table_tr4">
								...
								<td>
									...
									{if endinspection.isImported == 0 && endinspection_index == 0 && accountType == "MINISTRY" && endinspection.status == 7 && endinspection.finalAuditStatus != 3}
										<sec:authorize ifAllGranted='ROLE_PROJECT_POST_ENDINSPECTION_REVIEWAUDIT_VIEW'>
											{if endPassAlready != 1 && granted != null && granted.status != 3 && granted.status != 4}
												{if endinspection.finalAuditStatus==0}
													<sec:authorize ifAllGranted='ROLE_PROJECT_POST_ENDINSPECTION_REVIEWAUDIT_ADD'>
												...
												{elseif endinspection.finalAuditStatus < 3}
													...
													<sec:authorize ifAllGranted='ROLE_PROJECT_POST_ENDINSPECTION_REVIEWAUDIT_MODIFY'>
									...
									{elseif endinspection.finalAuditStatus > 0}
										<sec:authorize ifAllGranted='ROLE_PROJECT_POST_ENDINSPECTION_DATA_VIEW'>
											<span class="ico_txt2"><a class="j_viewEndReviewAuditPop" endId="${endinspection.id}" style="cursor: pointer;">查看详情</a></span>
										</sec:authorize>
										{if endinspection_index == 0 && endinspection.finalAuditStatus==3 && granted != null && granted.status != 3 && granted.status != 4}
											<sec:authorize ifAllGranted='ROLE_PROJECT_POST_ENDINSPECTION_DATA_MODIFY'>
												<input class="btn1 j_modifyEndResultPop" modifyFlag="1" type="button" value="修改" />
											</sec:authorize>
										{/if}
									{/if}
								</td>
			...
			{/if}
			<sec:authorize ifAllGranted='ROLE_PROJECT_POST_ENDINSPECTION_DATA_PRINT'>
	...
	{forelse}<!-- 无结项信息 -->
		...
		<s:if test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@EXPERT, @csdc.tool.bean.AccountType@STUDENT)"><!-- 研究人员-->
			{if isDirector == 1 && endPassAlready != 1 && endPending != 1 && varPending==0 && granted != null && granted.status == 1}
				...
				<sec:authorize ifAllGranted='ROLE_PROJECT_POST_ENDINSPECTION_APPLY_ADD'>
					<div class="p_box_b_1"><input class="btn1 j_toAddEndApplyPop" type="button" value="申报"/><br/></div>
				</sec:authorize>
			{else}
				<div class="p_box_body">
		...
		<s:elseif test="#session.loginer.currentType.equals(@csdc.tool.bean.AccountType@MINISTRY)"><!-- 部级 -->
			<sec:authorize ifAllGranted='ROLE_PROJECT_POST_ENDINSPECTION_REVIEW_ADD'>	
				{if annPending ==0 && endPending==0 && varPending==0 && reviewPending==0 && reviewPassAlready==0 && granted != null && granted.status == 1}
					<div><input class="btn2 j_addEndReviewResultPop" reviewFlag="2"  type="button" value="录入鉴定" /><br/></div>
				{elseif granted != null && granted.status == 3}
					<div>项目已中止，不能录入数据</div>
				{elseif granted != null && granted.status == 4}
...
<s:form id="group_review_endform" theme="simple">
	...
	<s:hidden id="endReviewOpinionFeedback" name="reviewOpinionFeedback"/>
</s:form>
