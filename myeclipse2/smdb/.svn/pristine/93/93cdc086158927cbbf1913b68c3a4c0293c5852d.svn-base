	...
	public boolean checkBusinessType(String businessSubType, String businessId, int startYear, int endYear ) {
		if (businessSubType == null || businessSubType.equals("-1")) {// 业务类型为空，则视为已存在
			...
			if(businessId == null || businessId.equals("")){//添加判断
				...
				map.put("code", code);
				if(code.substring(2).equals("1") || code.substring(2).equals("5") || code.substring(2).equals("6")){//若为申报或者年检或年度选题业务且存在同类激活的业务，则不能添加
				...
				map.put("endYear", endYear);
				map.put("code", code);
				if(code.substring(2).equals("1") || code.substring(2).equals("5") || code.substring(2).equals("6")){//若为申报或者年检业务且存在同类激活的业务，则不能添加
