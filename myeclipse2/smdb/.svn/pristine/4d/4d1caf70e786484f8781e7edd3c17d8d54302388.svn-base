...
public class ApplicationApplyStrictAction extends DbListAction {
	...
	private int graYearScope;//申报立项项目起止年度跨度
	...
	private int strictAppYear;//限项申报年度
	private int avgAnnAppTotal;//平均年度申报总数
	...
	private int priAppTotal;//初算：限项申报总数
	private int moeTiltTotal;//部属高校倾斜
	private int westTiltTotal;//西部高校倾
	private int pubAppTotal;//发布：限项申报总数
	private int forAppTotal;//预测：预期申报总数
	...
	public void setDefaultData(){
		...
		if (flag == 1) {
			...
			systemConfig.setDescription("限项申报核算");
...
//		Map ResPubAwardThird = generalService.getAwardScore(session1, generalService.getSystemId("013", "awardType"), generalService.getSystemId("04", "awardGrade"), ResPubAwardThi);
		...
		for (Object[] objs : univs) {
			...
			datas[4] = (appMap.containsKey(univId)) ? ((Long)appMap.get(univId)).intValue() : 0;//申报数量
...
//					+ PAward;//最近一届获奖得分
			...
			datas[14] = Math.round((((Integer) datas[4]).doubleValue())/5.0);//年度限项申报基数（往年平均申报数
			...
			if (app < 0) {
				datas[19] = 0;//初算：限项申报数量
			} else {
				datas[19] = app;//初算：限项申报数量
			}
			...
			if (datas[3] != null && (datas[3].equals("新疆维吾尔自治区") || datas[3].equals("西藏自治区"))) {
				datas[22] = "不限";//发布：限项申报数量
			} else {
				datas[22] = 0;//发布：限项申报数量
			}
			datas[23] = 0;//预测：预期申报数量
			if (!(datas[22] instanceof String)) {
				...
				if (datas[22] instanceof Integer) {
					if ((Long) datas[14] < (Integer) datas[22]) {
						datas[23] = (Long) datas[14];//预测：预期申报数量
					} else {
						datas[23] = (Integer) datas[22];//预测：预期申报数量
					}
				} else {
					if ((Long) datas[14] < (Long) datas[22]) {
						datas[23] = (Long) datas[14];//预测：预期申报数量
					} else {
						datas[23] = (Long) datas[22];//预测：预期申报数量
					}
				}
			} else {
				datas[23] = (Long) datas[14];//预测：预期申报数量
