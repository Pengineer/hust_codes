...
//		List<String> statistic_parm = (List<String>)getMdx().get("statistic_parm");//定制统计相关参数
		...
		try {
			dataList = statisticService.getData(result, 1);
	...
	public String getMdxToView() throws Exception{
		...
		//中文相关参数在客户端编码后解码
		String statisticUniversityName =(null == statistic_university_name) ? null : new String(statistic_university_name.getBytes("ISO8859-1"),"UTF-8");//指定高校
		String statisticIndex =(null == statistic_index) ? null : new String(statistic_index.getBytes("ISO8859-1"),"UTF-8");
		String statisticData =(null == statistic_data) ? null : new String(statistic_data.getBytes("ISO8859-1"),"UTF-8");
		String statisticSortColumn =(null == statistic_sortColumn) ? null : new String(statistic_sortColumn.getBytes("ISO8859-1"),"UTF-8");
		String statisticUniversity =(null == statistic_university) ? null : new String(statistic_university.getBytes("ISO8859-1"),"UTF-8");//所属高校
		String statisticDiscipline =(null == statistic_discipline) ? null : new String(statistic_discipline.getBytes("ISO8859-1"),"UTF-8");
		String statisticProjectType =(null == statistic_projectType) ? null : new String(statistic_projectType.getBytes("ISO8859-1"),"UTF-8");
		String statisticSubType =(null == statistic_subType) ? null : new String(statistic_subType.getBytes("ISO8859-1"),"UTF-8");
		String statisticProjectArea =(null == statistic_projectArea) ? null : new String(statistic_projectArea.getBytes("ISO8859-1"),"UTF-8");
		String statisticProductType =(null == statistic_productType) ? null : new String(statistic_productType.getBytes("ISO8859-1"),"UTF-8");
		String statisticProvince =(null == statistic_province) ? null : new String(statistic_province.getBytes("ISO8859-1"),"UTF-8");
		String statisticEvaluType =(null == statistic_evaluType) ? null : new String(statistic_evaluType.getBytes("ISO8859-1"),"UTF-8");
		String statisticUniversityType =(null == statistic_universityType) ? null : new String(statistic_universityType.getBytes("ISO8859-1"),"UTF-8");
...
//			if(statisticData.contains("人员数量同比增加")) data.append("[Measures].[人员数目],");
//			if(statisticData.contains("全时人员数量")) data.append("[Measures].[人员数目],");
...
//			else if(statisticData.equals("人员数量同比增加")) statisticData = new String("[Measures].[人员数目]");
//			else if(statisticData.equals("全时人员数量")) statisticData = new String("[Measures].[人员数目]");
			else return INPUT;
...
//			request.getSession().setAttribute("measureNum", statistic_index.split(",").length);
			...
			StringBuffer data = new StringBuffer();
			if(statisticData.contains("申报项目数量")) data.append("[Measures].[申报数],");
...
//			if(statisticData.contains("支出经费同比增加")) data.append("[Measures].[地方高校],");
			statisticData = data.substring(0, data.toString().length()-1).toString();
			if(statisticSortColumn.equals("申报项目数量")) statisticSortColumn = new String("[Measures].[申报数]");
...
//			setChartConfig(chartConfigTmp.toString());
			...
			if(statisticData.contains("[Measures].[申报数]")) {
...
//			Map session = ActionContext.getContext().getSession();
	...
	public void setMobileStatisticService(
			IMobileStatisticService mobileStatisticService) {
		this.mobileStatisticService = mobileStatisticService;
	}
	@Override
