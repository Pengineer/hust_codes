...
<div id="simple_search">
	<s:form id="search" theme="simple" action="list" namespace="/product/electronic">
				...
				<td align="right"><span class="choose_bar">
					...
					<s:if test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@EXPERT, @csdc.tool.bean.AccountType@STUDENT)">
						<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{getText('不限')}--"
							list="#{'1': getText('出版物名称'), '2': getText('出版物类型'), '4': getText('所属单位'), '5': getText('出版单位')}"/>
					</s:if>
					<!-- 校级管理人员 -->
					<s:elseif test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@MINISTRY_UNIVERSITY, @csdc.tool.bean.AccountType@LOCAL_UNIVERSITY)">
						<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{getText('不限')}--"
							list="#{'1': getText('出版物名称'), '2': getText('出版物类型'), '3': getText('第一作者'), '6': getText('所属部门'), '5': getText('出版单位')}"/>
					</s:elseif>
					<!-- 其它人员 -->
					<s:else>
						<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{getText('不限')}--"
							list="#{'1': getText('出版物名称'), '2': getText('出版物类型'), '3': getText('第一作者'), '4': getText('所属单位'), '5': getText('出版单位')}"/>
					</s:else>
				</span>
...
<div id="adv_search" style="display:none;">
	<s:form id="advSearch" action="advSearch" theme="simple" namespace="/product/electronic">
		...
		<div class="adv_content">
			...
			<table class="adv_table" width="100%" border="0" cellspacing="2" cellpadding="0">
				<tr class="adv_tr">
					<td class="adv_td1" width="75" align="right">出版物名称：</td>
					<td class="adv_td1" width="200"><s:textfield name="electronic.chineseName" value="%{searchQuery.chineseName}" cssClass="input_css"/></td>
					<td class="adv_td1" width="80"></td>
					<td class="adv_td1" width="75" align="right">出版物类型：</td>
					<td class="adv_td1" width="200" >
						<s:select cssClass="select" name="electronic.type.id" value="%{searchQuery.type}" list="%{baseService.getSystemOptionMap('productType', '04')}"
				...
				<tr class="adv_tr">
					...
					<td class="adv_td1">
						...
						<s:hidden name="electronic.disciplineType" value="%{searchQuery.disciplineType}" id="dispName"/>
					</td>
					<td class="adv_td1"></td>
					<td class="adv_td1" align="right">第一作者：</td>
					<td class="adv_td1"><s:textfield name="electronic.authorName" value="%{searchQuery.authorName}" cssClass="input_css"/></td>
					<td class="adv_td1"></td>
				</tr>
				<tr class="adv_tr">
					<!-- 非校级管理员 -->
					<s:if test=" #session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@MINISTRY_UNIVERSITY)<0 || #session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@LOCAL_UNIVERSITY)>0">
						...
						<td class="adv_td1"><s:textfield name="electronic.agencyName" value="%{searchQuery.agencyName}" cssClass="input_css"/></td>
						<td class="adv_td1"></td>
					</s:if>
					<!-- 非院系、基地管理员 -->
					<s:if test=" #session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@DEPARTMENT)<0 || #session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@INSTITUTE)>0">
						...
						<td class="adv_td1"><s:textfield name="electronic.divisionName" value="%{searchQuery.divisionName}" cssClass="input_css"/></td>
						<td class="adv_td1"></td>
					</s:if>
				</tr>
				<tr class="adv_tr">
					<td class="adv_td1" align="right">出版单位：</td>
					<td class="adv_td1"><s:textfield name="electronic.publishUnit" value="%{searchQuery.publishUnit}" cssClass="input_css"/></td>
					<td class="adv_td1"></td>
				...
				<s:if test=" #session.loginer.currentType.within(@csdc.tool.bean.AccountType@EXPERT, @csdc.tool.bean.AccountType@STUDENT)">
					<tr class="adv_tr">
						...
						<td class="adv_td2">
							<s:select cssClass="select" name="electronic.submitStatus" value="%{searchQuery.submitStatus}" list="#{'2': getText('保存'), '3': getText('已提交')}"
				...
				<s:else>
					<tr class="adv_tr">
						...
						<td class="adv_td1">
							<s:select cssClass="select" name="electronic.auditResult" value="%{searchQuery.auditResult}" list="#{'0':getText('待审'),'1':getText('不同意'),'2':getText('同意')}"
								headerKey="-1" headerValue="--%{getText('不限')}--"/></td>
						<td class="adv_td1"></td>
				...
				<tr class="adv_tr">
					<s:if test="!#session.loginer.currentType.within(@csdc.tool.bean.AccountType@MINISTRY_UNIVERSITY, @csdc.tool.bean.AccountType@STUDENT)">						
						...
						<td class="adv_td2"><s:textfield name="electronic.provinceName" value="%{searchQuery.provinceName}" cssClass="input_css" /></td>
						<td class="adv_td2"></td>
					</s:if>
				</tr>	
			</table>
