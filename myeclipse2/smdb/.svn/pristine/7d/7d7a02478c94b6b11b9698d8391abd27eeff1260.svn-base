...
define(function(require, exports, module) {
	...
	var toAddGrantedFeeApplyPop = function(projectType, showProjectDetails) {
		...
		popAddGrantedFee({
			title : "添加立项经费预算明细",
			src : "project/entrust/application/granted/toAddGrantedFeeApply.action?projectid=" + $("#projectid").val()+ "&approveFee=" + $("#approveFee").val(),
//			inData : {"varPending" : $("#varPending").val(),"midPending" : $("#midPending").val()},
			callBack : function(dis){
				$("#update").val(1);
				$.ajax({
					url: "project/entrust/application/granted/addGrantedFeeApply.action",
					type: "post",
	...
	exports.init = function() {
		...
		$(".j_toAddGrantedFeeApplyPop").live('click',function(){//进入项目状态设置弹层页面
			toAddGrantedFeeApplyPop(projectType, viewEntrust.showProjectDetails);
		});
