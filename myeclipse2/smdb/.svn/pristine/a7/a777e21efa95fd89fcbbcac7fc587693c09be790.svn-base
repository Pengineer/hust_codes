					...
					<span class="choose_bar">
						<s:if test="#session.loginer.currentType.compareWith(@csdc.tool.bean.AccountType@ADMINISTRATOR) == 0">
							<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{getText('不限')}--"
								list="#{'1':getText('新闻标题'),'2':getText('新闻正文'),'3':getText('发布账号'),'4':getText('发布者')}" />
						</s:if>
						<s:else>
							<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{getText('不限')}--"
								list="#{'1':getText('新闻标题'),'2':getText('新闻正文')}" />
						</s:else>
					</span>
		...
		<div class="adv_content">
			...
			<table class="adv_table" width="100%" border="0" cellspacing="2" cellpadding="0">
				...
				<tr class="adv_tr">
					...
					<td class="adv_td1"><s:select cssClass="select" name="keyword3" value="%{searchQuery.type}" headerKey="" headerValue="--%{getText('不限')}--" list="#application.newsItems" listKey="id" listValue="name" /></td>
					<td class="adv_td1"></td>
