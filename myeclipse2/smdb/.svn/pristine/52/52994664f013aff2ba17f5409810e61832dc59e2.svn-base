...
<textarea id="view_end_template" style="display:none;">
	...
	<s:hidden name="endStatus" id="endStatus" value="%{projectService.getBusinessStatus('023', entityId)}"/>
	{for endinspection in endList}
		{if endinspection_index == 0}<!-- 最近一次结项 -->
			...
			<s:if test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@EXPERT, @csdc.tool.bean.AccountType@STUDENT)"><!-- 研究人员-->
				{if isDirector == 1 && endinspection.finalAuditResultEnd == 1 && endinspection.finalAuditStatus == 3 && endPassAlready != 1 && endPending != 1 && (midPassAlready == 1 || endAllow == 1) && varPending==0 && granted != null && granted.status == 1}
					<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_APPLY_ADD'>
						<div class="p_box_b_2">
							<a href="javascript:void(0);" class="j_downloadEndTemplate" ><s:text name="i18n_DownloadEndFileTemplate0"/></a>
						</div>
						<div class="p_box_b_1" id="applyEnd">
			...
			<s:elseif test="#session.loginer.currentType.equals(@csdc.tool.bean.AccountType@MINISTRY)"><!-- 部级 -->
				<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_DATA_ADD'>
		...
		<div>
			<div class="p_box_b">
				<div class="p_box_b_1">
					{if endinspection.isImported==0}
						...
						{if endinspection_index == 0 && endinspection.finalAuditStatus != 3}
							{if reviewflag == -1 && ((accountType == "MINISTRY" && endinspection.status > 5) || (accountType == "PROVINCE" && endinspection.status > 4) || ((accountType =="MINISTRY_UNIVERSITY" || accountType == "LOCAL_UNIVERSITY") && endinspection.status > 3))}
								<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_REVIEW_ADD'>
									<span><input class="btn2 j_addEndReviewResultPop" endId="${endinspection.id}" type="button" value="<s:text name='i18n_AddReviewResult1' />" /></span>
									<span><input class="btn2 j_addEndReviewExpert" type="button" endId = "${endinspection.id}" value="<s:text name='i18n_AddExpert' />" /></span>
								</sec:authorize>
							{elseif (reviewflag == 22 && accountType == "MINISTRY" && endinspection.status > 5) || (reviewflag == 32 && accountType == "PROVINCE" && endinspection.status > 4) || (reviewflag == 42 && (accountType =="MINISTRY_UNIVERSITY" || accountType == "LOCAL_UNIVERSITY") && endinspection.status > 3)}
								<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_REVIEW_MODIFY'>
									<span><input class="btn2 j_modifyEndReviewResultPop" type="button" endId="${endinspection.id}" value="<s:text name='i18n_ModifyReviewResult1' />" /></span>
								</sec:authorize>
								<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_REVIEW_ADD'>
									<span><input class="btn2 j_submitEndReviewResult" type="button" endId="${endinspection.id}" value="<s:text name='i18n_SubmitReviewResult1' />" /></span>
								</sec:authorize>
							{/if}
						{/if}
					{elseif endinspection.isImported==1}
						...
						{if endinspection_index == 0 && endinspection.finalAuditStatus==2 && granted != null && granted.status == 1}
							{if endPassAlready==0 && (midPassAlready == 1 || endAllow == 1)}
								<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_DATA_MODIFY'>
									<input class="btn1 j_modifyEndResultPop" data="0" type="button" value="<s:text name='i18n_Modify'/>" />
								</sec:authorize>
								{if varPending==0 && midPending==0}
									<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_DATA_ADD'>
										<input class="btn1 j_submitEndResult" type="button" value="<s:text name='i18n_Submit'/>" />
									</sec:authorize>
								{/if}
							{/if}
						{/if}
				...
				<div class="p_box_b_2">
					<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_APPLY_VIEW'>
					{if endinspection.file != null}
						<div style="float: right;"><img src="image/ico09.gif" />
							<a href="" id="${endinspection.id}" class="download_instp_3"><s:text name='i18n_DownloadEndinspection' /></a>
					...
					<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_DATA_UPLOAD'>
						<div style="float: right;"><img src="image/ico10.gif" /><a class="j_uploadEndPop" endId="${endinspection.id}" href="javascript:void(0);" ><s:text name='i18n_UploadEndApply'/></a>&nbsp;&nbsp;</div>
					</sec:authorize>
					</s:if>
					{/if}
					</sec:authorize>
						...
						<tbody>
							<tr class="table_tr7">
								...
								<td colspan="4" style="text-align:center">
									...
									<a href="" id="${projectDatas[endinspection_index].id}" name="${projectDatas[endinspection_index].file}" class="download_instp_5"><s:text name='i18n_DownloadEndData' /></span></a>
			...
			<s:if test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@EXPERT, @csdc.tool.bean.AccountType@STUDENT)"><!-- 研究人员-->
				{if endinspection_index == 0 && (endinspection.applicantSubmitStatus == 1 || endinspection.applicantSubmitStatus == 2)}<!-- 结项申请暂存或退回 -->
					<div style="padding-left:10px; padding-right:10px;" id="addProductEnd">
								...
								<td align="right">
									{if isDirector == 1 && granted != null && granted.status == 1}
										{if endPassAlready != 1 && (midPassAlready == 1 || endAllow == 1)}
											<sec:authorize ifAllGranted="ROLE_PROJECT_INSTP_ENDINSPECTION_APPLY_MODIFY">
											...
											{/if}
										{/if}
										<sec:authorize ifAllGranted="ROLE_PROJECT_INSTP_ENDINSPECTION_APPLY_DELETE">
			...
			<s:elseif test="#session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@EXPERT)<0 ">
				<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_APPLYAUDIT_VIEW'>
					<div class="p_box_body">
						<table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" style="border-collapse:collapse;">
							<thead>
								<tr class="head_title">
										...
										{if endinspection.isImported == 0 && endinspection_index == 0 && (accountType == "DEPARTMENT" || accountType == "INSTITUTE") && endinspection.status == 2 && endinspection.finalAuditStatus != 3}
											{if endPassAlready != 1 && granted != null && granted.status == 1}
												{if endinspection.deptInstAuditStatus == 0}
													<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_APPLYAUDIT_ADD'>
												...
												{elseif endinspection.deptInstAuditStatus < 3}
													<span class="ico_txt2"><a class="j_viewEndAuditPop" endId="${endinspection.id}" data="1" style="cursor: pointer;"><s:text name='i18n_ViewMoreDetail'/></a></span>
													{if (isPrincipal == 1 && (endAuditorInfo[1] == belongId || endAuditorInfo[2] == belongId)) || (isPrincipal == 0 && endAuditorInfo[0] == belongId)}
													<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_APPLYAUDIT_MODIFY'>
											...
											{if endinspection.isImported == 0 && endinspection_index == 0 && (accountType =="MINISTRY_UNIVERSITY" || accountType == "LOCAL_UNIVERSITY") && endinspection.status == 3 && endinspection.finalAuditStatus != 3}
												{if endPassAlready != 1 && granted != null && granted.status == 1}
													...
													{elseif endinspection.universityAuditStatus < 3}
														<span class="ico_txt2"><a class="j_viewEndAuditPop" endId="${endinspection.id}" data="2" style="cursor: pointer;"><s:text name='i18n_ViewMoreDetail'/></a></span>
												...
												{if endinspection.isImported == 0 && endinspection_index == 0 && accountType == "PROVINCE" && endinspection.status == 4 && endinspection.finalAuditStatus != 3}
													{if endPassAlready != 1 && granted != null && granted.status == 1}
														...
														{elseif endinspection.provinceAuditStatus < 3}
															<span class="ico_txt2"><a class="j_viewEndAuditPop" endId="${endinspection.id}" data="3" style="cursor: pointer;"><s:text name='i18n_ViewMoreDetail'/></a></span>
											...
											{if endinspection.isImported == 0 && endinspection_index == 0 && accountType == "MINISTRY" && endinspection.status == 5 && endinspection.finalAuditStatus != 3}
												{if endPassAlready != 1 && granted != null && granted.status == 1}
													...
													{elseif endinspection.ministryAuditStatus < 3}
														<span class="ico_txt2"><a class="j_viewEndAuditPop" endId="${endinspection.id}" data="4" style="cursor: pointer;"><s:text name='i18n_ViewMoreDetail'/></a></span>
			...
			<!-- 专家评审信息-->
			<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_REVIEW_VIEW'>
						...
						<table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" style="border-collapse:collapse;">
							...
							<tbody>
								<tr class="table_tr3">
									...
									<td width="70"><s:text name='i18n_SubmitStatus' /></td>
									<td width="250"><s:text name='i18n_ViewMoreDetail'/></td>
								</tr>
								<tr class="table_tr4">
										...
										{if endinspection.isImported == 0 && endinspection_index == 0 && endinspection.status == 6 && endinspection.finalAuditStatus != 3 && endinspectionReview[endinspection_index].submitStatus != 3}
											{if endPassAlready != 1 && granted != null && granted.status == 1}
												{if endinspectionReview[endinspection_index].submitStatus==0}
													<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_REVIEW_ADD'>
														<input class="btn1 j_addEndReviewPop" endId="${endinspection.id}" type="button" value="<s:text name='i18n_Review1' />" />
													</sec:authorize>
												{elseif endinspectionReview[endinspection_index].submitStatus < 3}
													<span class="ico_txt2"><a class="j_viewEndReviewPop" entityId="${endinspectionReview[endinspection_index].id}" style="cursor: pointer;"><s:text name='i18n_ViewMoreDetail'/></a></span>
													<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_REVIEW_MODIFY'>
														<input class="btn1 j_modifyEndReviewPop" endId="${endinspection.id}" entityId="${endinspectionReview[endinspection_index].id}" type="button" value="<s:text name='i18n_Modify' />" />
													</sec:authorize>
													<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_REVIEW_ADD'>
														<input class="btn1 j_submitEndReview" endId="${endinspection.id}" type="button" value="<s:text name='i18n_Submit'/>" />
													</sec:authorize>
												{/if}
											{/if}
										{else}
								...
								{if endinspectionReviews[endinspection_index] != null}
									{for item in endinspectionReviews[endinspection_index]}
										<tr class="table_tr4">
											...
											<td>
												{if item[5]!=0}
													<span class="ico_txt2"><a class="j_viewEndReviewPop" entityId="${item[0]}" style="cursor: pointer;"><s:text name='i18n_ViewMoreDetail'/></a></span>
												{/if}
											</td>
										</tr>
									{/for}
										...
										{if endinspection.isImported == 0 && endinspection_index == 0 && (accountType =="MINISTRY_UNIVERSITY" || accountType == "TEACHER") && endinspection.status == 6 && endinspection.finalAuditStatus != 3 && allReviewSubmit != -1}
											{if endPassAlready != 1 && granted != null && granted.status == 1}
												{if endinspection.reviewStatus == 0}
													<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_REVIEW_ADD'>
														<input class="btn1 j_addEndGroupReviewPop" endId="${endinspection.id}" type="button" value="<s:text name='i18n_Review1' />" />
													</sec:authorize>
												{elseif endinspection.reviewStatus == 2}
													...
													<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_REVIEW_MODIFY'>
														<input class="btn1 j_modifyEndGroupReviewPop" endId="${endinspection.id}" type="button" value="<s:text name='i18n_Modify' />" />
													</sec:authorize>
													<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_REVIEW_ADD'>
														<input class="btn1 j_submitEndGroupReview" endId="${endinspection.id}" type="button" value="<s:text name='i18n_Submit'/>" />
													</sec:authorize>
												{/if}
											{/if}
										{else}
							...
							<tr class="table_tr4">
								...
								<td>
									...
									{if endinspection.isImported == 0 && endinspection_index == 0 && accountType == "MINISTRY" && endinspection.status == 7 && endinspection.finalAuditStatus != 3}
										<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_REVIEWAUDIT_VIEW'>
											{if endPassAlready != 1 && granted != null && granted.status != 3 && granted.status != 4}
												{if endinspection.finalAuditStatus==0}
													<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_REVIEWAUDIT_ADD'>
														<input class="btn1 j_addEndReviewAuditPop" endId="${endinspection.id}" type="button" value="<s:text name='i18n_Audit'/>" />
													</sec:authorize>
												{elseif endinspection.finalAuditStatus < 3}
													...
													<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_REVIEWAUDIT_MODIFY'>
														<input class="btn1 j_modifyEndReviewAuditPop" endId="${endinspection.id}" type="button" value="<s:text name='i18n_Modify' />" />
													</sec:authorize>
													<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_REVIEWAUDIT_ADD'>
														<input class="btn1 j_submitEndReviewAuditResult" endId="${endinspection.id}" type="button" value="<s:text name='i18n_Submit'/>" />
													</sec:authorize>
												{/if}
											{/if}
										</sec:authorize>
									{elseif endinspection.finalAuditStatus > 0}
										<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_DATA_VIEW'>
											<span class="ico_txt2"><a class="j_viewEndReviewAuditPop" endId="${endinspection.id}" style="cursor: pointer;"><s:text name='i18n_ViewMoreDetail'/></a></span>
										</sec:authorize>
										{if endinspection_index == 0 && endinspection.finalAuditStatus==3 && granted != null && granted.status != 3 && granted.status != 4}
											<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_DATA_MODIFY'>
												<input class="btn1 j_modifyEndResultPop" data="1" type="button" value="<s:text name='i18n_Modify'/>" />
											</sec:authorize>
										{/if}
									{/if}
								</td>
			...
			{/if}
			<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_PRINT'>
	...
	{forelse}<!-- 无结项信息 -->
		...
		<s:if test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@EXPERT, @csdc.tool.bean.AccountType@STUDENT)"><!-- 研究人员-->
			{if isDirector == 1 && endPassAlready != 1 && endPending != 1 && (midPassAlready == 1 || endAllow == 1) && varPending==0 && granted != null && granted.status == 1}
				<div class="p_box_b_2"><a class="j_downloadEndTemplate" style="cursor: pointer;"><s:text name='i18n_DownloadEndFileTemplate'/></a></div>
				<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_APPLY_ADD'>
