				...
				<td align="right">
					<span class="choose_bar">
						<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{getText('不限')}--"
							list="#{'1':getText('研究基地代码'),'2':getText('研究基地名称'),'3':getText('研究基地负责人')}" />
		...
		<div class="adv_content">
			...
			<table class="adv_table" width="100%" border="0" cellspacing="2" cellpadding="0">
				...
				<tr class="adv_tr">
					...
					<td class="adv_td1"><s:select cssClass="select" name="iType" value="%{searchQuery.iType}" headerKey="-1" headerValue="--%{getText('不限')}--" list="%{baseService.getSystemOptionMap('researchAgencyType', null)}" /></td>
					<td class="adv_td1"></td>
