					...
					{if reviewflagFromApp != 22 && reviewflagFromApp != 23 && reviewflagFromApp != 32 && reviewflagFromApp != 33 && reviewflagFromApp != 42 && reviewflagFromApp != 43 && accountType == "MINISTRY" && application.status == 6}
						<sec:authorize ifAllGranted='ROLE_PROJECT_GENERAL_APPLICATION_REVIEW_ADD'>
							{if reviewflagFromApp == -1  && accountType == "MINISTRY" && application.status == 6}
								<span><input class="btn2 j_addAppReviewResultPop" type="button" appId="${application.id}" value="<s:text name='i18n_AddReviewResult' />" /></span>
							{/if}
			...
			<sec:authorize ifAllGranted='ROLE_PROJECT_GENERAL_APPLICATION_APPLY_DOWNLOAD'>
				{if application.file != null}
					<div style="float: right;"><img src="image/ico09.gif" />
						<a href="" id="${application.id}" class="download_general_1"><s:text name='i18n_DownloadApplication' /></a>
	...
	<div class="p_box_body">
		<table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" style="border-collapse:collapse;">
			...
			<tr class="table_tr7">
				...
				<td class="key" width="120"><s:text name="i18n_ProjectSubtype" />：</td>
							...
							<td>
								...
								{if application.isImported == 0 && (accountType == "DEPARTMENT" || accountType == "INSTITUTE") && application.status == 2 && application.finalAuditStatus != 3}
									...
									{elseif application.deptInstAuditStatus < 3}
										<span class="ico_txt2"><a class="j_viewAppAuditPop" appId="${application.id}" alt="1" style="cursor: pointer;"><s:text name='i18n_ViewMoreDetail'/></a></span>
								...
								{if application.isImported == 0 && application.status == 6 && application.finalAuditStatus != 3 && applicationReview.submitStatus != 3}
									{if appPassAlready != 1}
										{if applicationReview.submitStatus==0}
											<sec:authorize ifAllGranted='ROLE_PROJECT_GENERAL_APPLICATION_REVIEW_ADD'>
												<input class="btn1 j_addAppReviewPop" type="button" appId="${application.id}" value="<s:text name='i18n_Review' />"/>
											</sec:authorize>
										{elseif applicationReview.submitStatus < 3}
											...
											<sec:authorize ifAllGranted='ROLE_PROJECT_GENERAL_APPLICATION_REVIEW_MODIFY'>
												<input class="btn1 j_modifyAppReviewPop" type="button" appId="${application.id}" appRevId="${applicationReview.id}" value="<s:text name='i18n_Modify' />"/>
											</sec:authorize>
											<sec:authorize ifAllGranted='ROLE_PROJECT_GENERAL_APPLICATION_REVIEW_ADD'>
												<input class="btn1 j_submitAppReview" type="button" appId="${application.id}" value="<s:text name='i18n_Submit'/>"/>
											</sec:authorize>
										{/if}
									{/if}
								{else}
					...
					<tbody>
								...
								{for item in applicationReviewExpert}
									{if item != null}
										<tr class="table_tr4">
											...
											<td></td>
											<td>
												{if item[1]==0}<s:text name='i18n_Pending' />
												{elseif item[1]==2}<s:text name='i18n_Saved' />
												{elseif item[1]==3}<s:text name='i18n_Submit' />
												{/if}
											</td>
											<td>
											</td>
						...
						<tr class="table_tr4">
							...
							<td>
								...
								{if application.isImported == 0 && (accountType =="EXPERT" || accountType == "TEACHER") && application.status > 5 && application.finalAuditStatus != 3 && allAppReviewSubmit != -1 && reviewNum == 1 }
									{if appPassAlready != 1}
										{if application.reviewStatus == 0}
											<sec:authorize ifAllGranted='ROLE_PROJECT_GENERAL_APPLICATION_REVIEW_ADD'>
												<input class="btn1 j_addAppGroupReviewPop" appId="${application.id}" type="button" value="<s:text name='小组评审' />" />
											</sec:authorize>
								...
								{elseif reviewflagFromApp == 22 && accountType == "MINISTRY" && application.status == 6}
									<span class="ico_txt2"><a class="j_viewAppGroupReviewPop" appId="${application.id}" style="cursor: pointer;"><s:text name='i18n_ViewMoreDetail'/></a></span>
									<sec:authorize ifAllGranted='ROLE_PROJECT_GENERAL_APPLICATION_REVIEW_MODIFY'>
										<span><input class="btn2 j_modifyAppReviewResultPop" type="button" appId="${application.id}" value="<s:text name='i18n_Modify' />" /></span>
									</sec:authorize>
									<sec:authorize ifAllGranted='ROLE_PROJECT_GENERAL_APPLICATION_REVIEW_ADD'>
										<span><input class="btn2 j_submitAppReviewResult" type="button" appId="${application.id}" value="<s:text name='i18n_Submit' />" /></span>
									</sec:authorize>
								{/if}
							</td>
						</tr>
	...
	<div class="p_box_body"><!-- 最终审核结果 -->
	 	<table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" style="border-collapse:collapse;">
			<s:if test="#session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@PROVINCE)<0"><!-- 部级及以上管理人员 -->
				...
				<tbody>
					<tr class="table_tr3">
						<td width="120"><s:text name='i18n_EndStopWithdrawDate' /></td>
									...
									{if appPassAlready != 1}
										{if application.finalAuditStatus==0}
											<sec:authorize ifAllGranted='ROLE_PROJECT_GENERAL_APPLICATION_REVIEWAUDIT_ADD'>
												<input class="btn1 j_addAppReviewAuditPop" type="button" appId="${application.id}" value="<s:text name='i18n_Audit'/>" />
											</sec:authorize>
										{elseif application.finalAuditStatus < 3}
											...
											<sec:authorize ifAllGranted='ROLE_PROJECT_GENERAL_APPLICATION_REVIEWAUDIT_MODIFY'>
												<input class="btn1 j_modifyAppReviewAuditPop" type="button" appId="${application.id}" value="<s:text name='i18n_Modify' />" />
											</sec:authorize>
											<sec:authorize ifAllGranted='ROLE_PROJECT_GENERAL_APPLICATION_REVIEWAUDIT_ADD'>
												<input class="btn1 j_submitAppReviewAuditResult" type="button" appId="${application.id}" value="<s:text name='i18n_Submit'/>" />
											</sec:authorize>
										{/if}
									{/if}
								</sec:authorize>
			...
			<s:else><!-- 部级以下所有人员 -->
				{if application.isImported == 1 || application.applicantSubmitStatus == 3}
					...
					<tbody>
						<tr class="table_tr3">
							<td width="50%"><s:text name='i18n_EndStopWithdrawDate'/></td>
