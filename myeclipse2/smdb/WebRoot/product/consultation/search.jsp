...
<div id="simple_search">
	<s:form id="search" theme="simple" action="list" namespace="/product/consultation">
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_td_padding">
			<tr class="table_main_tr">
				<td align="right">
					<span class="choose_bar">
						...
						<s:if test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@EXPERT, @csdc.tool.bean.AccountType@STUDENT)">
							<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{getText('不限')}--"
								list="#{'1': getText('报告名称'), '4': getText('所属单位'), '5': getText('采纳单位')}"/>
						</s:if>
						<!-- 校级管理人员 -->
						<s:elseif test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@MINISTRY_UNIVERSITY, @csdc.tool.bean.AccountType@LOCAL_UNIVERSITY)">
							<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{getText('不限')}--"
								list="#{'1': getText('报告名称'), '3': getText('第一作者'), '6': getText('所属部门'), '5': getText('采纳单位')}"/>
						</s:elseif>
						<!-- 其它人员 -->
						<s:else>
							<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{getText('不限')}--"
								list="#{'1': getText('报告名称'), '3': getText('第一作者'), '4': getText('所属单位'), '5': getText('采纳单位')}"/>
						</s:else>
					</span>
					...
					<s:hidden id="protype" value="consultation"/>
				</td>
...
<div id="adv_search" style="display:none;">
	<s:form id="advSearch" action="advSearch" theme="simple" namespace="/product/consultation">
		...
		<div class="adv_content">
			...
			<table class="adv_table" width="100%" border="0" cellspacing="2" cellpadding="0">
				<tr class="adv_tr">
					<td class="adv_td1" width="65" align="right">报告名称：</td>
					<td class="adv_td1" width="200"><s:textfield name="consultation.chineseName" value="%{searchQuery.chineseName}" cssClass="input_css"/></td>
					...
					<s:if test="!#session.loginer.currentType.within(@csdc.tool.bean.AccountType@MINISTRY_UNIVERSITY, @csdc.tool.bean.AccountType@STUDENT)">						
						...
						<td class="adv_td1"><s:textfield name="consultation.provinceName" value="%{searchQuery.provinceName}" cssClass="input_css" /></td>
						<td class="adv_td1"></td>
					</s:if>
				</tr>
				<tr class="adv_tr">
					<td class="adv_td1" width="65" align="right">学科门类：</td>
					<td class="adv_td1" width="200">
						...
						<s:hidden name="consultation.disciplineType" value="%{searchQuery.disciplineType}" id="dispName"/>
					</td>
					<td class="adv_td1" width="100"></td>
					<td class="adv_td1" width="65" align="right">第一作者：</td>
					<td class="adv_td1" width="200"><s:textfield name="consultation.authorName" value="%{searchQuery.authorName}" cssClass="input_css"/></td>
					<td class="adv_td1"></td>
				</tr>
				<tr class="adv_tr">
					<!-- 非校级管理员 -->
					<s:if test=" #session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@MINISTRY_UNIVERSITY)<0 || #session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@LOCAL_UNIVERSITY)>0">
						...
						<td class="adv_td1"><s:textfield name="consultation.agencyName" value="%{searchQuery.agencyName}" cssClass="input_css"/></td>
						<td class="adv_td1"></td>
					</s:if>
					<!-- 非院系、基地管理员 -->
					<s:if test=" #session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@DEPARTMENT)<0 || #session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@INSTITUTE)>0">
						...
						<td class="adv_td1"><s:textfield name="consultation.divisionName" value="%{searchQuery.divisionName}" cssClass="input_css"/></td>
						<td class="adv_td1"></td>
					</s:if>
				</tr>
				<tr class="adv_tr">
					<td class="adv_td1" align="right">采纳单位：</td>
					<td class="adv_td1"><s:textfield name="consultation.useUnit" value="%{searchQuery.useUnit}" cssClass="input_css"/></td>
					<td class="adv_td1"></td>
					<td class="adv_td1" align="right">采纳时间：</td>
				...
				<s:if test=" #session.loginer.currentType.within(@csdc.tool.bean.AccountType@EXPERT, @csdc.tool.bean.AccountType@STUDENT)"><!-- 研究人员 -->
					<tr class="adv_tr">
						...
						<td class="adv_td2">
							<s:select cssClass="select" name="consultation.submitStatus" value="%{searchQuery.submitStatus}" list="#{'2': getText('保存'), '3': getText('已提交')}"
				...
				<s:else>
					<tr class="adv_tr">
						<td class="adv_td2" align="right">审核状态：</td>
						<td class="adv_td2">
							<s:select cssClass="select" name="consultation.auditResult" value="%{searchQuery.auditResult}" list="#{'0': getText('待审'), '1': getText('不同意'),'2':getText('同意')}"
								headerKey="-1" headerValue="--%{getText('不限')}--"/></td>
						<td class="adv_td2"></td>
						<td class="adv_td2" align="right">审核时间：</td>
						<td class="adv_td2"><s:textfield id="audDateId1" value="%{searchQuery.audDate1}" cssClass="input_css_self date_hint FloraDatepick" disabled="disabled" size="10" />&nbsp;至&nbsp;<s:textfield id="audDateId2" value="%{searchQuery.audDate2}" cssClass="input_css_self date_hint FloraDatepick" disabled="disabled" size="10" /></td>
						<td class="adv_td2"></td>
					</tr>
				</s:else>				
			</table>
