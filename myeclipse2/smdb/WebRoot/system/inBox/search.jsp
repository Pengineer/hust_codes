				...
				<td align="right">
					<span class="choose_bar">
						<s:if test="#session.loginer.currentType.equals(@csdc.tool.bean.AccountType@ADMINISTRATOR)">
							<s:select cssClass="select" name="keyword1" value="%{searchQuery.keyword1}" headerKey="0" headerValue="%{getText('收件箱')}"
								list="#{'1':getText('发件箱'),'2':getText('所有信箱')}" id="session1" />
						</s:if>
						<s:else>
						<s:select cssClass="select" name="keyword1" value="%{searchQuery.keyword1}" headerKey="0" headerValue="%{getText('收件箱')}"
								list="#{'1':getText('发件箱')}" id="session1" />
						</s:else>
						<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{getText('不限')}--"
							list="#{'1':getText('主题'),'2':getText('发信者')}" />
					</span>	
