...
<div class="table_main_tr_adv">
	<s:include value="/projectFund/generalFund/searchForFundList.jsp"/>
</div>
	<textarea id="list_template" style="display:none;">
		<table id="list_table" width="100%" border="0" cellspacing="0" cellpadding="0" class="table_td_padding" style="text-align:center;">
			<thead id="list_head">
				...
				<sec:authorize ifAllGranted="ROLE_PROJECT_GENERAL_APPLICATION_GRANTED_DELETE">
					...
					<td width="2"><img src="image/table_line.gif" width="2" height="24" /></td>
					<td width="50"><a id="sortcolumn2" href="" class="{if sortColumn == 2}{if sortColumnLabel == 1}up_css{else}down_css{/if}{/if}" title="<s:text name='i18n_SortByDirector' />"><s:text name="i18n_Director" /></a></td>
				<s:if test="!#session.loginer.currentType.within(@csdc.tool.bean.AccountType@MINISTRY_UNIVERSITY, @csdc.tool.bean.AccountType@INSTITUTE)">
					...
					<td width="2"><img src="image/table_line.gif" width="2" height="24" /></td>
					<td width="70"><a id="sortcolumn7" href="" class="{if sortColumn == 7}{if sortColumnLabel == 1}up_css{else}down_css{/if}{/if}" title="<s:text name='点击按立项拨款金额排序' />"><s:text name="立项拨款" /></a></td>
				</tr>
			</thead>
			<tbody>
			{for item in root}
...
<%--					<td>--%>
<%--					{if item.laData[15]==0}--%>
<%--						<a id="${item.laData[18]}" class="modify_fee" href="" title="<s:text name='点击修改金额' />">${item.laData[12]}</a>--%>
<%--					{else}${item.laData[12]}--%>
<%--					{/if}--%>
<%--					</td>--%>
					<td>
	...
	<s:form id="list" theme="simple" action="fundListDelete" namespace="/fundList/general/granted">
