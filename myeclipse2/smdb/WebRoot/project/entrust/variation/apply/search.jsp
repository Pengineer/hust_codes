...
<div id="simple_search">
	<s:form id="search" theme="simple" action="list" namespace="/project/entrust/variation/apply">
				...
				<td align="right">
					...
					<s:if test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@MINISTRY, @csdc.tool.bean.AccountType@INSTITUTE)"><!-- 管理人员-->
						<s:select cssClass="select" id="mainFlag" name="mainFlag" headerKey="" headerValue="--所有项目--"
							list="#{'0541':getText('i18n_Received'),'0542':getText('i18n_ToAudit'),'0543':getText('i18n_Submitted')}" />
					</s:if>
					<s:if test="!#session.loginer.currentType.within(@csdc.tool.bean.AccountType@MINISTRY_UNIVERSITY, @csdc.tool.bean.AccountType@INSTITUTE)">
						<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{getText('i18n_NoLimit')}--"
							list="#{'1':getText('i18n_ProjectName'),'2':getText('i18n_Director'),'3':getText('i18n_University'),'4':getText('i18n_IssueType'),'5':getText('i18n_DisciplineType'),'6':getText('i18n_ProjectYear')}" />
					</s:if>
					<s:else>
						<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{getText('i18n_NoLimit')}--"
							list="#{'1':getText('i18n_ProjectName'),'2':getText('i18n_Director'),'4':getText('i18n_IssueType'),'5':getText('i18n_DisciplineType'),'6':getText('i18n_ProjectYear')}" />
					</s:else>
					</span>
...
<div id="adv_search" style="display:none;">
	<s:form id="advSearch" action="advSearch" theme="simple" namespace="/project/entrust/variation/apply">
		...
		<div class="adv_content">
			...
			<table class="adv_table" width="100%" border="0" cellspacing="2" cellpadding="0">
				...
				<tr class="adv_tr">
					...
					<td class="adv_td1"><s:select cssClass="select" name="projectSubtype" value="%{searchQuery.projectSubtype}" list="%{baseService.getSystemOptionMap('projectType', '05')}" headerKey="-1" headerValue="--%{getText('i18n_NoLimit')}--" /></td>
					<td class="adv_td1"></td>
				...
				<tr class="adv_tr">
					<td class="adv_td1" align="right"><s:text name="i18n_VariationIssue" />：</td>
				...
				<s:if test="#session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@MINISTRY)>0">
					<tr class="adv_tr">
						<td class="adv_td1" align="right"><s:text name="i18n_VariationStatus" />：</td>
						<td class="adv_td1">
				...
				<s:else>
					<tr class="adv_tr">
						<td class="adv_td1" align="right"><s:if test="#session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@PROVINCE)<0"><s:text name="i18n_VariationStatus" /></s:if> <s:else><s:text name="i18n_AuditStatus" /></s:else>：</td>
						<td class="adv_td1">
						...
						<td class="adv_td1"></td>
						<td class="adv_td1" align="right"><s:if test="#session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@PROVINCE)<0"><s:text name="i18n_VariationDate" /></s:if> <s:else><s:text name="i18n_AuditDate" /></s:else>：</td>
						<td class="adv_td1">
