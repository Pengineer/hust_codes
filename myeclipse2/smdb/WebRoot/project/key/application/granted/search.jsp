				...
				<td align="right"><span class="choose_bar">
					<s:if test="!#session.loginer.currentType.within(@csdc.tool.bean.AccountType@MINISTRY_UNIVERSITY, @csdc.tool.bean.AccountType@INSTITUTE)">
						<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{getText('不限')}--"
							list="#{'1':getText('批准号'),'2':getText('项目名称'),'3':getText('首席专家'),'4':getText('依托高校'),'5':getText('研究类型'),'6':getText('学科门类'),'7':getText('项目年度')}" />
					</s:if>
					<s:else>
						<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{getText('不限')}--"
							list="#{'1':getText('批准号'),'2':getText('项目名称'),'3':getText('首席专家'),'5':getText('研究类型'),'6':getText('学科门类'),'7':getText('项目年度')}" />
					</s:else>
		...
		<div class="adv_content">
			...
			<table class="adv_table" width="100%" border="0" cellspacing="2" cellpadding="0">
				...
				<tr class="adv_tr">
					...
					<td class="adv_td1">
						<s:select cssClass="select" name="midiStatus" value="%{searchQuery.midiStatus}" headerKey="-1" headerValue="--%{getText('不限')}--" list="#{'1':getText('通过'),'2':getText('未通过')}" />
					</td>
					<td class="adv_td1"></td>
				</tr>
				<tr class="adv_tr">
