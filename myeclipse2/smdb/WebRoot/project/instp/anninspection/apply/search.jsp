...
<div id="simple_search">
	<s:form id="search" theme="simple" action="list" namespace="/project/instp/anninspection/apply">
					...
					<span class="choose_bar">
						<s:if test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@MINISTRY, @csdc.tool.bean.AccountType@INSTITUTE)"><!-- 管理人员-->
							<s:select cssClass="select" id="mainFlag" name="mainFlag" headerKey="" headerValue="--所有项目--"
								list="#{'0251':getText('i18n_Received'),'0252':getText('i18n_ToAudit'),'0253':getText('i18n_Submitted')}" />
						</s:if>
...
<div id="adv_search" style="display:none;">
	<s:form id="advSearch" action="advSearch" theme="simple" namespace="/project/instp/anninspection/apply">
		...
		<div class="adv_content">
			...
			<s:if test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@EXPERT, @csdc.tool.bean.AccountType@STUDENT)"><!-- 研究人员-->
				<tr class="adv_tr">
					...
					<td class="adv_td1"></td>
					<td class="adv_td1" align="right"><s:text name="i18n_AnninspectionStatus" />：</td>
					<td class="adv_td1">
			...
			<s:else>
				<tr class="adv_tr">
					<td class="adv_td1" align="right"><s:if test="#session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@PROVINCE)<0"><s:text name="i18n_AnninspectionStatus" /></s:if> <s:else><s:text name="i18n_AuditStatus" /></s:else>：</td>
					<td class="adv_td1">
						   <s:select cssClass="select" name="auditStatus" value="%{searchQuery.projectNumber}" list="#{'32':getText('i18n_Approve'),'31':getText('i18n_NotApprove'),'22':getText('i18n_SaveApprove'),'21':getText('i18n_SaveNotApprove'),'12':getText('i18n_SendBack'),'0':getText('i18n_Pending')}" headerKey="-1" headerValue="--%{getText('i18n_NoLimit')}--" />
			   		</td>
					<td class="adv_td1"></td>
					<td class="adv_td1" align="right"><s:if test="#session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@PROVINCE)<0"><s:text name="i18n_AnninspectionDate" /></s:if> <s:else><s:text name="i18n_AuditDate" /></s:else>：</td>
					<td class="adv_td1">
