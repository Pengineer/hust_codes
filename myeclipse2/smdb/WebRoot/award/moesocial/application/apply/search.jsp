				...
				<td align="right">
					<span class="choose_bar">
						...
						<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{getText('不限')}--"
							list="#{'1':getText('成果名称'),'2':getText('申请人'),'3':getText('依托高校'),'4':getText('学科门类'),'5':getText('成果类型')}" />
					</span>
			...
			<table class="adv_table" width="100%" border="0" cellspacing="2" cellpadding="0">
				...
				<s:elseif test="#session.loginer.currentType.equals(@csdc.tool.bean.AccountType@MINISTRY)">
					<tr class="adv_tr">
						...
						<td class="adv_td1">
						   	<s:select cssClass="select" name="reviewAuditStatus" value="%{searchQuery.reviewAuditStatus}" list="#{'32':getText('同意'),'31':getText('不同意'),'22':getText('同意（暂存）'),'21':getText('不同意（暂存）'),'0':getText('待审')}" headerKey="-1" headerValue="--%{getText('不限')}--" />
				   		</td>
						<td class="adv_td1"></td>
						<td class="adv_td1" align="right">审核状态：</td>
						<td class="adv_td1">
							<s:select cssClass="select" name="status" value="%{searchQuery.status}" list="#{'32':getText('同意'),'31':getText('不同意'),'22':getText('同意（暂存）'),'21':getText('不同意（暂存）'),'12':getText('被退回'),'0':getText('待审')}" headerKey="-1" headerValue="--%{getText('不限')}--" />
				   		</td>
						<td class=adv_td1></td>
					</tr>					
				</s:elseif>		
				...
				<s:else>
					<tr class="adv_tr">
						...
						<td class="adv_td2">
							<s:select cssClass="select" name="auditStatus" value="%{searchQuery.auditStatus}" list="#{'3':getText('已提交'),'2':getText('保存'),'1':getText('被退回')}" headerKey="-1" headerValue="--%{getText('不限')}--" />
				   		</td>
						<td class="adv_td2"></td>
					</tr>				
				</s:else>	
