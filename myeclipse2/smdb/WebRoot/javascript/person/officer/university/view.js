define(function(require, exports, module) {
	...
	var nameSpace = "person/universityOfficer";
	var showDetails = function(result) {// 显示查看内容
		if (result.errorInfo == null || result.errorInfo == "") {
			$("#entityId").val(result.officer.id);
			$("#entityIds").val(result.officer.id);
	...
	exports.init = function() {
		...
		$("#view_addAccount").live("click", function(){
			popAccountAdd({
				title : "添加子账号",
				src : "account/university/sub/extIfToAdd.action?belongId=" + $("#entityId").val(),
				inData : {data : {"typeName" : "校级子账号", "belongId" : $("#entityId").val(), "belongName" : $("#personName").html()}},
