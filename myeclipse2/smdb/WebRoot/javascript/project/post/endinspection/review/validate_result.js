/**
 * 用于录入结项评审的校验
 * @author 余潜玉、肖雅
 */
define(function(require, exports, module) {
	require('validate');
	
	var accountType = $("#accountType").val();
	var attrRules = {
		"review_prod":[
       		["isApplyExcellent", {required: true}],
       		["endProductInfo", {required: true}],
       		["endMember", {maxlength:200, multipleNames:true}]
//			       		["endMember", {maxlength:200, multipleNames:true, required: true}]
       	],
		"review_moe":[
			[".reviewerType", {selected:true}],
			[".reviewer.id", {required:true, maxlength:50}]
		],
		"review_man":[
			[".idcardType", {selected:true}],
			[".idcardNumber", {required:true, cardNumber:true}],
			[".reviewerType", {selected:true}],
			[".reviewerName", {required:true, maxlength:80}],
			[".gender", {selected:true}],
			[".agencyName", {required:true, maxlength:80}],
			[".divisionType", {selected:true}],
			[".divisionName", {required:true, maxlength:80}]
		],
		"review_same":[
			[".innovationScore", {required: true, number: true, range:[0,50]}],
			[".scientificScore", {required: true, number: true, range:[0,25]}],
			[".benefitScore", {required: true, number: true, range:[0,25]}],
			[".qualitativeOpinion", {selected:true}],
			[".opinion", {maxlength:2000}]
		],
		"review_group":[
			["reviewWay", {required:true}],
			["reviewResult", {required:true}],
			["reviewOpinionQualitative", {selected:true}],
			["reviewOpinion", {maxlength: 2000}]
		]
	};
	
	var valid = function(){
		$(function() {
			$("#review_endform").validate({
				errorElement: "span",
				event: "change",
				rules:{
					
				},
				errorPlacement: function(error, element) { 
					error.appendTo( element.parents("td").next("td") );
				}
			});
			valid_review();
			valid_review_group();
			var reviewFlag = $("#reviewFlag").val();
			if(reviewFlag == 2){//教育部录入鉴定，校验成果相关信息
				valid_review_prod();
			}
		});
	};
	
	var valid_review = function(){
		//各专家信息校验
		$("#review .table_valid").each(function(key1, value1){
			//专家鉴定信息
			if(accountType == "MINISTRY"){//教育部录入
				for(i = 0; i < attrRules.review_moe.length; i++){
					if($("':input[name=" + attrRules.review_moe[i][0] + "]'").length != 0){
						$("':input[name*=" + attrRules.review_moe[i][0] + "]'", $(this)).rules("add", attrRules.review_moe[i][1]);
					}
				}
			}else{
				for(i = 0; i < attrRules.review_man.length; i++){
					if($("':input[name=" + attrRules.review_man[i][0] + "]'").length != 0){
						$("':input[name*=" + attrRules.review_man[i][0] + "]'", $(this)).rules("add", attrRules.review_man[i][1]);
					}
				}
			}
			for(i = 0; i < attrRules.review_same.length; i++){
				if($("':input[name=" + attrRules.review_same[i][0] + "]'").length != 0){
					$("':input[name*=" + attrRules.review_same[i][0] + "]'", $(this)).rules("add", attrRules.review_same[i][1]);
				}
			}
			//总评信息校验
			for(var j = 0; j < attrRules.review_group.length; j++){
				if($("':input[name=" + attrRules.review_group[i][0] + "]'").length != 0){
					$("':input[name=" + attrRules.review_group[j][0] + "]'").rules("add", attrRules.review_group[j][1] );
				}
			}
		});
	};
	
	var valid_review_prod = function(){
		//成果信息校验
		for(var k = 0; k < attrRules.review_prod.length; k++){
			$("':input[name=" + attrRules.review_prod[k][0] + "]'").rules("add", attrRules.review_prod[k][1] );
		}
	};
	
	var valid_review_group = function(){
		//总评信息校验
		for(var j = 0; j < attrRules.review_group.length; j++){
			$("':input[name=" + attrRules.review_group[j][0] + "]'").rules("add", attrRules.review_group[j][1] );
		}
	};
	
	exports.valid = function() {
		valid();
	};
	exports.valid_review = function() {
		valid_review();
	};
	exports.valid_review_group = function() {
		valid_review_group();
	};
	exports.valid_review_prod = function() {
		valid_review_prod();
	};
});