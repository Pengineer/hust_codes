...
define(function(require, exports, module) {
	...
	require('form');
	require('jquery-ui');
	var appValid = require('javascript/project/project_share/application/review/validate_result');
	...
	var reviewNum = function(){
		$(".table_valid").each(function(key, value){
	 	...
	 	appValid.valid_review();//分配下标后重新设置校验，否则修改完成时会出错
	}
	...
	//添加结项评审结果
	//data：2保存；3提交
	var submitOrSaveAddAppReviewResult = function(data, projectType){
		submitOrSaveAppReviewResult(data, 1, projectType);
	};

	var submitOrSaveModifyAppReviewResult = function(data, projectType){
		submitOrSaveAppReviewResult(data, 2, projectType );
	};

	//申报评审结果是否提交
	//data：2保存；3提交
	//type：1添加；2修改
	var submitOrSaveAppReviewResult = function(data, type, projectType){
		if(!$("#review_appform").valid()){
			return;
		}
		$("#submitStatus").val(data);
		var url="";
		if(type==1){
			url="project/" + projectType + "/application/review/addResult.action";
		}else if(type==2){
			url="project/" + projectType + "/application/review/modifyResult.action";
		}
		if(checkReview()){
			$("#review_appform").attr("action",url);
			$("#review_appform").submit();
		}
	}
	...
	module.exports = {
		 submitOrSaveAddAppReviewResult: function(data, projectType){submitOrSaveAddAppReviewResult(data, projectType)},
		 submitOrSaveModifyAppReviewResult: function(data, projectType){submitOrSaveModifyAppReviewResult(data, projectType)},
		 init: function(){init()} 
	};
});
