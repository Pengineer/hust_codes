		...
		*/
		...
		if (typeof this.customSettings["my_queue_limit"] == 'number'){
			...
			if (this.customSettings["findDivId"] != undefined) {
				file_length = ($("#" + this.customSettings["findDivId"]).find("input[name='fileIdsforVarPost']")).length;
			}else{
				file_length = $("input[name='fileIdsforVarPost']").length;
			}
...
//		alert(file.name);
		...
		processFileId.name = "fileIdsforVarPost";
...
function queueComplete1(numFilesUploaded) {
