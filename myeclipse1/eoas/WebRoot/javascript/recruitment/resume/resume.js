$(document).ready(function(){
	...
	var addTable1 = function(obj ,tableId){
		...
		addTable1.html($("#" + tableId).html());
		$(":input", addTable1).each(function(){
			this.name = this.name  +  "[" + flag1 + "]";
			this.id = this.name;//校验需要修改id，可能是jquery的内部实现
		});
		flag1++;
		sortNum1();
	};
	...
	var addTable2 = function(obj ,tableId){
		...
		$(":input", addTable2).each(function(){
			this.name = this.name + "[" + flag2 + "]";
			this.id = this.name;//校验需要修改id，可能是jquery的内部实现
		});
		flag2++;
		sortNum2();
