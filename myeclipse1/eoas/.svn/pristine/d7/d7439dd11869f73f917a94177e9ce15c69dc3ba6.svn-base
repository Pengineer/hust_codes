...
function uploadStart(file) {
	try {
		...
		*/
		...
		if (typeof swfu.customSettings["my_queue_limit"] == 'number'){
			if ($("input[name='fileIds']").length >= swfu.customSettings["my_queue_limit"]){
				swfu.cancelUpload(file.id);
				alert("最多只能上传" + swfu.customSettings["my_queue_limit"] + "个文件!");
				return false;
			}
		}
