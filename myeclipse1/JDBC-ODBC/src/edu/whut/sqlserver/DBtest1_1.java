...
public class DBtest1_1 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println(rsd.getColumnName(0) + ":" + rsd.getColumnTypeName(0));
			
			//对于查询的另一种方式：直接通过连接获取元数据，DatabaseMetaData还是很强大的，因为它包含数据库的元信息，可以获取表getTables()，获取表的外键getCrossReference(),授权，数据库最大连接数等。
			/*DatabaseMetaData dmd = con.getMetaData();
			ResultSet rSet = dmd.getColumns(catalog, schemaPattern, tableNamePattern, columnNamePattern);
			while(rSet.next()){
