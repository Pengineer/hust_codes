...
var setting = {
	...
	async : {//异步请求，获取专家树数据
		enable : true,
		url : "project/instp/createExpertTree.action",
	},
	check: {
		enable: true,
		chkStyle: "checkbox"
...
function init() {
	if(projectExpertNumber == null || expertProjectMax == null) {
		...
		instpService.initParam("Tree", initParamCallback);
	}
	initHeight();
}
...
function initParamCallback(data) {
	...
	projectExpertNumber = param[0];	//基地项目_每个项目_所需的专家数
	expertProjectMin = param[1];	//基地项目_每个专家_评审的最小项目数
	expertProjectMax = param[2];	//基地项目_每个专家_评审的最大项目数
...
function initHeight() {
	...
	if(selectExpertIds != null || selectExpertIds != 'null') {
		...
		instpService.getExpInfo("instp", selectExpertIds, getDetail);
		return false;
	}
	getBodyHeight();
}
...
//	selectDis();//更新选择后的discipline
	...
	$("#expertTree").attr("action", "project/instp/searchExpert.action");
	$("#expertTree").submit();
}
...
//		}
	...
	if (str.split(';').length <= projectExpertNumber) {
		...
		instpService.getExpInfo("instp", str, getDetail);
	} else {
		alert('每个项目分配专家应≤' + projectExpertNumber + '个！');
		treeNode.checked = false;//取消勾选
		zTreeObj.updateNode(treeNode, true);
...
function updateShow(expertId) {
	...
	instpService.getExpInfo("instp", toShowExpertIdstr, getDetail);
}
...
function reRank(act) {
	...
	instpService.getExpInfo("instp", selectExpIdStr, getDetail);
}
 ...
 * 弹出层查看专家详情，提供的功能有：<br>
 * 1、专家详情查看；
 * 2、查看页面提供专家的项目退评功能：执行的是InstpAction中的deleteExpertProjects()方法。
 * @param {Object} expertId 专家id
 * @return {TypeName} 
 */
