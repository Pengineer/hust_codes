...
//		fixProjectApplicationMemeber2015Info();
	...
	protected void fixProjectApplicationMemeber2015Info() throws Throwable{
		...
		if (!resultList.isEmpty()) {
			...
			for (int i = 0; i < resultList.size(); i++) {
				...
				//更新字段
				String backData = smdbClient.invokeDirect("fwzx", "csdc702", "SmasService", "fixSmasProgram", argsMap);
				if (null == backData) {
	...
	/**
	 * 修复smas申请表中的C_file字段
	 * 按照smdb项目申请表C_File字段，更新smas对应的字段
	 * @throws Throwable
	 */
	...
	protected void fixProjectApplicationFirstAuditInfo() throws Throwable {
		...
		if (!resultList.isEmpty()) {
			...
			for (int i = 0; i < resultList.size(); i++) {
				...
				String backData = smdbClient.invokeDirect("fwzx", "csdc702", "SmasService", "requestApplicationFirstAudit", argsMap);
				if (null == backData) {
					throw new RuntimeException("执行异常");//事务回滚
				} else if(backData.equals("busy")) {
					//3分钟后再请求
