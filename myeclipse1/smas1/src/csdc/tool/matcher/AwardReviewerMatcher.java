 ...
 * @author zhangn
 * 2015-06-14
 * 通过抽象方法与方法覆盖的形式处理奖励参数的差异
 */
public abstract class AwardReviewerMatcher {
...
//	public abstract Object getTheProjectService();
	public abstract String getAwardType();
	/**
	 * 每位专家若参评，至少需参评的奖励数
	 */
	...
	@Transactional
	public void matchExpert(Integer currentYear, List<String> expertIds, List<String> awardIds, List<String> rejectExpertIds) throws Exception {
		...
		paraMap.put("awardIds", awardIds);	//待匹配奖励id
		paraMap.put("rejectExpertIds", rejectExpertIds);	//待剔除专家id
		paraMap.put("projectExpertNumber", Long.valueOf(projectExpertNumber));	//每个奖励计划匹配人数
	
		List<Expert> experts = null;	//待匹配专家
		List<AwardApplication> awards = null;	//待匹配奖励
		List<AwardApplicationReview> existingMatches = null;	//已匹配条目
		List<AwardApplicationReview> rejectMatches = null;	//待剔除的匹配条目
			
		experts = getReviewerInfo(expertIds, awardIds, rejectExpertIds, paraMap);
		
        //2、筛选出当前年度（所有）待匹配的奖励（不包含已经匹配成功的奖励）
		awards = getSubjectInfo(awardIds, rejectExpertIds, paraMap);
		
		//3、找出和已有匹配条目(不包含待剔除条目)
		existingMatches = getExistingMatcher(getAwardType(), rejectExpertIds, paraMap);
		
		//4、找出待剔除的匹配条目
		rejectMatches = getRejectMatcher(getAwardType(), rejectExpertIds, paraMap);
		
		/*
		 * 以上找出的experts、awards、existingMatches、rejectMatches可能很多，为了节省内存，
		...
		//最终匹配结果(直接用于添加到数据库)
		List<AwardApplicationReview> matchResult = new ArrayList<AwardApplicationReview>();
		
		/*
		 * 将awards、experts、existingMatches转化为和数据库无关的subjects、reviewers、matchPairs，
		 * 并设置好subjects和reviewers的固有属性
		 */
		MatcherBeanTransformer transformer = new MatcherBeanTransformer();
		transformer.transFormerAward(awards, experts, existingMatches, getAwardType(), dao);
		...
		for (MatchPair matchPair : matchPairs) {
			AwardApplicationReview pr = getPRBean(matchPair, getAwardType());
			matchResult.add(pr);
		}
		...
		for (AwardApplicationReview aar : rejectMatches) {
			Subject subject = transformer.getSubject(aar.getAward());
			if (subject != null) {
				/*
				 * 一个评审对象已选的专家
				 */
				...
				if (selectedReviewers.size() >= projectExpertNumber) {
					 ...
					 */
					AwardApplicationReviewUpdate aaru = new AwardApplicationReviewUpdate(aar, 0, 0);
					dao.add(aaru);
					dao.delete(aar);
				} else {
					/*
					 ...
					 */
					selectedReviewers.add(transformer.getReviewer(aar.getReviewer()));
				}
			}
		}
		...
		for (int i = 0; i < matchResult.size(); i++) {
			AwardApplicationReview ar = matchResult.get(i);
			//向 匹配表 写入
			dao.add(ar);
			//向 匹配更新表 写入
			AwardApplicationReviewUpdate aaru = new AwardApplicationReviewUpdate(ar, 1, 0);
			dao.add(aaru);
			/*
			 * 每隔一万条flush一次
			 * 若最后一次性flush会有什么问题来着?
		...
		//匹配最后，更新警告信息
		System.out.println("\n开始更新警告信息...");
		for (AwardApplication award : awards) {//对本年度（所有）奖励进行更新操作
	...
	/**
	 * 筛选出当前年度（所有）待匹配的奖励（不包含已经匹配成功的奖励）
	 * @param projectIds
	 * @param rejectExpertIds
	 * @param paraMap
	 * @return
	 */
	public abstract List<AwardApplication> getSubjectInfo(List<String> projectIds, List<String> rejectExpertIds, Map paraMap);
	 ...
	 * @return
	 */
	public List<AwardApplicationReview> getExistingMatcher(String awardType, List<String> rejectExpertIds, Map paraMap){
		List<AwardApplicationReview> existingMatches = new ArrayList<AwardApplicationReview>();
		Date begin = new Date();
		String hql4ExistingMatch = "select ar from AwardApplicationReview ar left join fetch ar.award left join fetch ar.reviewer where ar.type = '" + awardType +"' and ar.year = :year " + (rejectExpertIds.isEmpty() ? "" : " and pr.reviewer.id not in (:rejectExpertIds) ");
	 ...
	 * @return
	 */
	public List<AwardApplicationReview> getRejectMatcher(String awardType, List<String> rejectExpertIds, Map paraMap){
		List<AwardApplicationReview> rejectMatches = new ArrayList<AwardApplicationReview>();
		Date begin = new Date();
		String hql4RejectMatch = "select aar from AwardApplicationReview aar left join fetch aar.award left join fetch aar.reviewer where aar.type = '" + awardType + "' and aar.year = :year " + (rejectExpertIds.isEmpty() ? " and 1 = 0" : " and aar.reviewer.id in (:rejectExpertIds) ");
	...
	/**
	 * 根据匹配器返回的Map形式的匹配结果得到smas的AwardApplicationReview实体bean
	 * (公共方法)
	 * @param match 不含奖励类别信息
	 * @return
	 */
	private AwardApplicationReview getPRBean(MatchPair matchPair ,String awardType) {
		...
		return new AwardApplicationReview(reviewer.getIntrinsicProperty(ID), subject.getIntrinsicProperty(ID), 0, year, awardType);
	}
