...
$(document).ready(function(){
	...
	*/
	$(".modifyEndFee").live("click",function(){
		...
		popInstpAudit({
			title : "经费结算明细",
			src : "project/instp/endinspection/toAddEndFee.action?projectid=" + projectid + "&feeNote=" + $("#feeNote").val() + 
	...
	$(".confirm").bind("click",function(){
		...
		$.ajax({
			url: "project/instp/endinspection/auditReview.action",
			type: "post",
			data: "projectid=" + projectid + "&endId=" + endId + "&isApplyNoevaluation=" + $("input[name=isApplyNoevaluation]:checked").val()
				+ "&endNoauditResultn=" + $("input[name=finalAuditResultNoevaluation]:checked").val()
				...
				+ "&endMember=" + $("#memberName").val() + "&reviewAuditResultEnd=" + $("input[name=reviewAuditResultEnd]:checked").val()
				+ "&endCertificate=" + $("#certificate").val() + "&endDate=" + $("#finalAuditDate").val()
				+ "&endImportedOpinion=" + $("#endImportedOpinion").val() + "&endAuditOpinionFeedback" + $("#endAuditOpinionFeedback").val()
