$(document).ready(function(){
	$('#workflowList').dataTable({  
	    ...
	    "sAjaxSource": "workflow/list.action",
	    "fnRowCallback": function( nRow, aData, iDisplayIndex ) {
            $('td:eq(0)', nRow).html('<a href="overflow/view.action?accountId=' + aData[3] + '">' +
                aData[0] + '</a>');
            $('td:eq(5)', nRow).html('<a href="workflow/view.action?processDefinitionId=' + aData[0] + '&resourceType=image' +  '">' +
                    aData[5] + '</a>');
            $('td:eq(8)', nRow)
            		.html('<a title="修改" href="account/toModify.action?accountId='
            			...
            			.append(('<a title="删除" id="'
    							+ aData[1]
    							+ '" class="deleteWorkflow" href="javascript:void(0)">'
...
/*	$('#myModal').on('hidden.bs.modal', function (e) {
	...
	$("body").on("click", ".deleteWorkflow", function(){
		...
		dialog({
		    ...
		    okValue: '确定',
		    ok: function () {
		    	var url = "workflow/delete.action?deploymentId=" + $(that).attr("id");
		    	$.get(url, function(data){
		    		if(data.result == "1"){
		    			dialog({
		    			    ...
		    			    okValue: '确定',
		    			    ok: function () {
		    			    	window.location.href = "workflow/toList.action";
		    			    }
		    			}).showModal();
		    		} else {
		    			dialog({
