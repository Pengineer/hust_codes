	...
	<div id="MainArea">
			...
			<tbody id="TableData" class="dataContainer" datakey="departmentList">
				...
				<s:iterator value="#departmentList">
					<tr class="TableDetail1 template">
						<td><s:a action="department/list?parentId=%{id}">${name}</s:a> &nbsp;</td>
						<td>${parent.name}&nbsp;</td>
						<td>${description}&nbsp;</td>
						<td><s:a action="delete?id=%{id}"
								onclick="return window.confirm('这将删除所有的下级部门，您确定要删除吗？')">删除</s:a>
							<s:a action="/modify?id=%{id}">修改</s:a>
						</td>
					</tr>
				</s:iterator>
		...
		<div id="TableTail">
			<div id="TableTail_inside">
				<s:a action="departmentAction_addUI"> <img src="style/images/createNew.png" />
				</s:a>
			</div>
		</div>
	</div>
