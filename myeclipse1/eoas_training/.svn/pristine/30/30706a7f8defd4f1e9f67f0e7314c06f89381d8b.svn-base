$(document).ready(function(){
	...
	$('#noticeList').dataTable({  
	    ...
	    "sAjaxSource": "notice/list.action",
	    "fnRowCallback": function( nRow, aData, iDisplayIndex ) {
            $('td:eq(0)', nRow).html('<a href="notice/view.action?noticeId=' + aData[4] + '">' +
                aData[0] + '</a>');
            $('td:eq(4)', nRow).html('<a href="notice/delete.action?noticeId=' + aData[4] + '">' +
                    '删除' + '</a>').append(('<a href="notice/toModify.action?noticeId=' + aData[4] + '">' +
                            '修改' + '</a>'));
            return nRow;
        },
	    "bServerSide": false, 
