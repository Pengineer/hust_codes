...
function loadDiscpTree(value) {
	if(value != null && value != "" && value != -1) {
		...
		tree2.setImagePath("image/tree/csh_bluebooks/");
		tree2.enableCheckBoxes(1);
		tree2.enableThreeStateCheckboxes(false);
		
		url = "expert/createDisciplineTree.action?hiddenId=" + ids + "&discpId=" + value;
...
function setDisable(id) {
	...
	if(id == rootId) {
		if(tree2.isItemChecked(rootId) == 1) {
			expertService.getDispIdCodeNameById(id, addToHiddenId);
			expertService.getDispIdCodeNameById(id, addToHiddenId);
		}
	} else {
		if(tree2.isItemChecked(id) == 1){
...
function sync() {
	...
	var relyDisciplineId = window.parent.dialog.get(window).data + "";//兼顾弹出层中弹出层
	expertService.getDispIdCodeNameByCodeName(relyDisciplineId,getIdCodeName);
}
