$(document).ready(function(){
	...
	$('#assessList').dataTable({  
	    ...
	    "sAjaxSource": "assess/list.action",
	    "fnRowCallback": function( nRow, aData, iDisplayIndex ) {
            $('td:eq(0)', nRow).html('<a href="assess/view.action?assessId=' + aData[7] + '">' +
            		aData[0]  + '</a>');
            $('td:eq(7)', nRow).html('<a href="assess/delete.action?assessId=' + aData[7] + '">' +
                    '删除' + '</a>');
            return nRow;
        },
	    "bServerSide": false, 
