$(document).ready(function(){
	...
	$('#roleList').dataTable({  
		...
		"fnRowCallback" : function(nRow, aData, iDisplayIndex) {
			$('td:eq(0)', nRow).html('<a href="role/view.action?roleId='
					+ aData[2] + '">' + aData[0] + '</a>');
			$('td:eq(2)', nRow)
					.html('<a title="修改" href="role/toModify.action?roleId='
							+ aData[2]
							+ '">'
					...
					.append(('<a title="删除" id="'
							+ aData[2]
							+ '" class="deleteRole" href="javascript:void(0)">'
		...
		"aoColumns": [{"mData": "0"},
		              {"mData": "1"},
                      {"mRender": function (data, type, full) {return '<input type="checkbox" name="roleIds" value="' + data + '">'}}],   	
		"oLanguage": {
	...
	$("body").on("click", ".deleteRole", function(){
		...
		dialog({
		    ...
		    okValue: '确定',
		    ok: function () {
		    	var url = "role/delete.action?roleId=" + $(that).attr("id");
