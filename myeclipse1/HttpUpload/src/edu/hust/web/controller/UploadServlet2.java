...
/*实际的文件上传比UploadServlet1要复杂的多，比如添加监听器，设置缓冲区大小（不能太大，每一个用户创建一个缓冲区的后果？），指定临时文件目录（文件太大，大于缓冲区大小，最后必须用代码手动删除该目录），如何防止直接访问（放到WEB-INF下），如何防止所有放在一个目录下（如果一个目录下的文件过多，会极大减慢文件的访问速度。算法分散），如何防止覆盖（UUID等），如何防止中文名乱码等等
 * 
