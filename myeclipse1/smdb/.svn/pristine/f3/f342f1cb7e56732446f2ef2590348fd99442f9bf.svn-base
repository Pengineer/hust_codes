...
public class ExpertMerger extends PersonMerger  {
	...
	private Map<String, MergeStrategy> exMap = new HashMap<String, MergeStrategy>();//专家对象合并策略
	
	public ExpertMerger(){
		exMap.put("agencyName", BuiltinMergeStrategies.SUPPLY);
		exMap.put("divisionName", BuiltinMergeStrategies.SUPPLY);
		exMap.put("position", BuiltinMergeStrategies.SUPPLY);
		exMap.put("type", BuiltinMergeStrategies.SUPPLY);
	}
	...
	public void mergeExpert(Serializable targetId, List<Serializable> incomeIds) throws IllegalArgumentException, IntrospectionException, IllegalAccessException, InvocationTargetException {
		...
		for (Expert incomeExpert : incomeExperts) {
			...
			for (String fieldName : exMap.keySet()) {
				beanFieldUtils.mergeField(tmpExpert, incomeExpert, fieldName, exMap.get(fieldName));
			}
		}
