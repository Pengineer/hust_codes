...
//	}
	...
	private Map<String, GeneralMember> generalMember = null;
	...
	public void initMember() {
		Date begin = new Date();
		generalMember = new HashMap<String, GeneralMember>();
		List<GeneralMember> generalMemberList = dao.query("select gm from GeneralMember gm left join gm.application app where gm.isDirector = 1 and app.year=2015");
		for(GeneralMember gm: generalMemberList) {
			generalMember.put(gm.getApplicationId() + gm.getMemberName(), gm);
		}
		System.out.println("init GeneralMember complete! use time " + (new Date().getTime() - begin.getTime()));
	}
	...
	private void firstAudit(){
		...
		//初审当前年一般项目任务(必须是省级或则校级审核过的)
		List<GeneralApplication> generalProjects = dao.query("select genApp from GeneralApplication genApp where genApp.year = ? " +
				" and ((genApp.provinceAuditStatus = 3 and genApp.provinceAuditResult = 2) " +
				" or ( genApp.provinceAuditStatus = 0 and genApp.provinceAuditResult = 0 and genApp.universityAuditStatus = 3 and genApp.universityAuditResult = 2 ))" , year);
//		List<GeneralApplication> generalProjects = dao.query("select genApp from GeneralApplication genApp where genApp.year = ? ", year);
		int currentNum=0;
		int totalNum=generalProjects.size();
		for (GeneralApplication project : generalProjects) {
...
//			List directors = dao.query("select mem from GeneralMember mem left join mem.application app where app.id = "+ project.getId() +" and mem.isDirector = 1 and mem.memberName = "+project.getApplicantName());
			GeneralMember director = generalMember.get(project.getId() + project.getApplicantName());
			if (director == null) {
...
//		generalMap = new HashMap<String, List<String>>();
		...
		高级 = dao.query("select h.name from SystemOption h where h.standard='GBT8561-2001' and h.description like '%高级'");
...
//		} 
	...
	private boolean isSpecialityTitleQualified(GeneralApplication project){
		boolean isQualified = false;
		
//		Map map = new HashMap(); 
//		map.put("appId", project.getId());
//		map.put("applicantName", project.getApplicantName());
//		GeneralMember director =  (GeneralMember) dao.query("select mem from GeneralMember mem left join mem.application app where app.id =:appId and mem.isDirector = 1 and mem.memberName =:applicantName",map).get(0);
		GeneralMember director = generalMember.get(project.getId() + project.getApplicantName());
		title = director.getSpecialistTitle();
		if(title != null && title.contains("/")){
			title = title.split("/")[1];
		}
		String degree = director.getMember().getAcademic().getLastDegree();//有点拖慢速度
