...
public class ProjectFundingFlushListener implements FlushEntityEventListener {
	...
	public void onFlushEntity(FlushEntityEvent event) throws HibernateException {
		...
		if (entity instanceof GeneralFunding) {
			...
			funding.setGrantedId(funding.getGranted().getId());
		} else if (entity instanceof DevrptFunding) {
			DevrptFunding funding = (DevrptFunding) entity;
			funding.setGrantedId(funding.getGranted().getId());
		} else if (entity instanceof SpecialFunding) {
			SpecialFunding funding = (SpecialFunding) entity;
			funding.setGrantedId(funding.getGranted().getId());
		} else {
