...
//		members = this.generalService.getDirectorList(granted.getApplication().getId(), granted.getMemberGroupNumber());
	...
	public String addResult() throws Exception{
		...
		defaultSelectCode = "";
		//根据变更成员信息检索库中成员
		personList = (List) this.doWithMember();
...
//			this.generalService.deleteMulMember(granted.getApplication().getId(), variation.getNewMemberGroupNumber());//去重
	 ...
	 * @throws Exception 
	 */
	public GeneralVariation doWithVariation(GeneralVariation variation, GeneralGranted granted, int type, int flag, String entityId) throws Exception{
		String selectIssues = "";
