...
/**
 * 国家地区导入_工具包
 * ISO3166-1国家和地区代码.xls
 * @author 徐涵
 *
 */
public class CountryRegionKit implements ITableKit{
	/**
	 * 用于在页面显示list
	 */
...
///////////////////////////////////////
	...
	private void getList(File xls) throws Exception{
		...
		god.setName("ISO3166-1国家和地区代码");
		god.setStandard("ISO3166-1");
		god.setCode("");
		...
		for (int i = 1; i < list.size(); i++){
			...
			so.setName(list.get(i).get(0));
			so.setDescription(list.get(i).get(1));
			so.setCode(list.get(i).get(2));
			so.setAbbr(list.get(i).get(3));
			so.setStandard("ISO3166-1");
	...
	public void imprt() throws Exception {
		baseService.execute("delete from SystemOption so where so.standard = 'ISO3166-1' ");
	...
	static public void setBaseService(IBaseService baseService) {
		CountryRegionKit.baseService = baseService;
	}
