				...
				<td align="right">
					...
					<s:if test="#session.loginer.currentType.compareWith(@csdc.tool.bean.AccountType@ADMINISTRATOR) == 0">
						<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--不限--"
							list="#{'1':'通知标题','2':'通知正文','3':'发布账号','4':'发布者'}" />
					</s:if>
					<s:else>
						<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--不限--"
							list="#{'1':'通知标题','2':'通知正文'}" />
	...
	<s:form id="advSearch" action="advSearch" theme="simple" namespace="/notice/inner">
		...
		<div class="adv_content">
			...
			<table class="adv_table" width="100%" border="0" cellspacing="2" cellpadding="0">
					...
					<td class="adv_td1"><s:select cssClass="select" name="keyword3" value="%{searchQuery.type}" headerKey="" headerValue="--不限--" list="#application.noticeItems" listKey="id" listValue="name" /></td>
					<td class="adv_td1"></td>
				...
				<tr class="adv_tr">
					...
					<td class="adv_td2"><s:select cssClass="select" name="isPop" value="%{searchQuery.isPop}" headerKey="-1" headerValue="--不限--" list="#{'1':'是','0':'否'}" /></td>
					<td class="adv_td2"></td>
