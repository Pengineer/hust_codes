...
public class ApplicationApplyStrictAction extends DbListAction {
	...
	private static final String projectType ="special";//项目类别
	private ISpecialService specialService;
	private static final String PAGE_NAME = "specialApplicationCheckPage";// 列表页面名称
...
//		}
	...
	public List<Object[]> check(){
		...
		avgMidRatio = (Double) specialService.calculate(avgEnd, avgMid);
		avgEndRatio = (Double) specialService.calculate(end, gra4End);
		avgAnnAppTotal = Math.round(appSum/5);
		if (null == keyword || keyword.equals("")) {
			if (graSum !=0 || appSum!=0) {
				avgGraRatio = (Double) specialService.calculate(graSum, appSum);//平均立项率
			} else {
				avgGraRatio = 0.0;
			}
		...
		//著作奖 一、二、三等奖
		Map bookAwardFirst = specialService.getAwardScore(session1, specialService.getSystemId("014", "awardType"), specialService.getSystemId("02", "awardGrade"), bookAwardFir);
		Map bookAwardSecond = specialService.getAwardScore(session1, specialService.getSystemId("014", "awardType"), specialService.getSystemId("03", "awardGrade"), bookAwardSec);
		Map bookAwardThird = specialService.getAwardScore(session1, specialService.getSystemId("014", "awardType"), specialService.getSystemId("04", "awardGrade"), bookAwardThi);
		//论文奖 一、二、三等奖
		Map paperAwardFirst = specialService.getAwardScore(session1, specialService.getSystemId("012", "awardType"), specialService.getSystemId("02", "awardGrade"), paperAwardFir);
		Map paperAwardSecond = specialService.getAwardScore(session1, specialService.getSystemId("012", "awardType"), specialService.getSystemId("03", "awardGrade"), paperAwardSec);
		Map paperAwardThird = specialService.getAwardScore(session1, specialService.getSystemId("012", "awardType"), specialService.getSystemId("04", "awardGrade"), paperAwardThi);
		//研究报告奖（出版）一、二、三等奖的权值
//		Map ResPubAwardFirst = specialService.getAwardScore(session1, specialService.getSystemId("013", "awardType"), specialService.getSystemId("02", "awardGrade"), ResPubAwardFir);
//		Map ResPubAwardSecond = specialService.getAwardScore(session1, specialService.getSystemId("013", "awardType"), specialService.getSystemId("03", "awardGrade"), ResPubAwardSec);
//		Map ResPubAwardThird = specialService.getAwardScore(session1, specialService.getSystemId("013", "awardType"), specialService.getSystemId("04", "awardGrade"), ResPubAwardThi);
		//研究报告奖（采纳/批示） 一、二、三等奖的权值
		Map ResAdoAwardFirst = specialService.getAwardScore(session1, specialService.getSystemId("013", "awardType"), specialService.getSystemId("02", "awardGrade"), ResAdoAwardFir);
		Map ResAdoAwardSecond = specialService.getAwardScore(session1, specialService.getSystemId("013", "awardType"), specialService.getSystemId("03", "awardGrade"), ResAdoAwardSec);
		Map ResAdoAwardThird = specialService.getAwardScore(session1, specialService.getSystemId("013", "awardType"), specialService.getSystemId("04", "awardGrade"), ResAdoAwardThi);
		//成果普及奖
		Map PopuAward = specialService.getAwardScore(session1, specialService.getSystemId("011", "awardType"), null, achPopuAward);
		...
		for (Object[] objs : univs) {
			...
			projectApplicationResult.setProjrctType("special");
			datas[0] = objs[1];//高校名称
			...
			datas[6] = specialService.calculate(datas[5], datas[4]);//立项率
			...
			datas[9] = specialService.calculate(datas[8], datas[7]);//按期中检率
			...
			datas[12] = specialService.calculate(datas[11], datas[10]);//按期结项率
...
//					+ PAward;//最近一届获奖得分
			...
			if (!(datas[22] instanceof String)) {
				...
				Long titlr = 0L;
				if (specialService.calculate(annStrictTarget,priAppTotal) instanceof Double) {
					Double test = (Double) specialService.calculate(annStrictTarget,priAppTotal);
					...
					Long test = (Long) specialService.calculate(annStrictTarget,priAppTotal);
					titlr = (long) (westTiltTotal * test);
					if (titlr<univStrictLB) {
						long data  =  Math.round(univStrictLB);
						datas[22] = data;//发布：限项申请数量
	...
	public InputStream getDownloadFile() throws UnsupportedEncodingException{
		...
		String header = "教育部人文社会科学研究专项任务项目限项申请核算表";
		...
		String modelFilePath = realPath + "/template/special/applyStrictAction/20131114_2014年度教育部人文社会科学研究专项任务项目限项申报核算表.xls"; 
		//调用导出公共方法
