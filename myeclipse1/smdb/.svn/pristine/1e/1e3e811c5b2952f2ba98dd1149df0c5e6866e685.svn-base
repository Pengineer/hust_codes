define(function(require, exports, module) {
	...
	exports.init = function() {
		valid();
		var grantedTotalFee = $("#grantedTotalFee").html()*1;
		...
		for(var i=0;i<ids.length;i++){
			$(ids[i]).blur(function() {
				...
				$("#totalFeeD").html(sum);
				if(grantedTotalFee < sum){
					$("#totalFeeD").css("color", "red");
				}else{
					$("#totalFeeD").css("color", "black");
				}
			});
		}
		$("#submit").click(function(){
			...
			var grantedTotalFee = $("#grantedTotalFee").html()*1;
			if(total != grantedTotalFee){
				if(total > grantedTotalFee){
					if (!confirm("变更后总额大于预算经费总额，您确定要调增经费吗？")) {
						return;
						}
					}else{
						alert("变更后与预算经费总额不相等");
