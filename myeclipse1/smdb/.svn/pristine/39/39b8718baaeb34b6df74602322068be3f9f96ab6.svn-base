 ...
 * 
 * 以上项目全是由《2011年一般项目申请一览表_修正导入.xls》中项目名称有多余空格所致，
...
public class GeneralProjectGranted2011Importer extends Importer {
	...
	private void importData() throws Exception {
		...
		while (next()) {
			...
			if (!ga.getAgencyName().equals(C)) {
				exeptionMessage.append(" * 所在学校和申请数据不一致：" + projectName + " \n");
			}
			if (!ga.getSubtype().getName().equals(D)) {
				exeptionMessage.append(" * 项目子类和申请数据不一致：" + projectName + " \n");
			}
	...
	private void init() {
		...
		for (GeneralApplication generalApplication : list) {
			//只有在当年唯一的[项目名+申请人名]才能作为寻找相应申请实体的依据
