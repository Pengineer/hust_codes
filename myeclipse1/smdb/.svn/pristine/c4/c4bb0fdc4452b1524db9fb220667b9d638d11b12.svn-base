...
<div id="simple_search">
	<s:form id="search" theme="simple" action="list" namespace="/award/moesocial/application/apply">
...
<div id="adv_search" style="display:none;">	
	<s:form id="advSearch" action="advSearch" namespace="/award/moesocial/application/apply" theme="simple">
		...
		<div class="adv_content">
			...
			<table class="adv_table" width="100%" border="0" cellspacing="2" cellpadding="0">
				...
				<tr class="adv_tr">
					...
					<td class="adv_td1"><s:select cssClass="select" name="ptype" value="%{searchQuery.ptype}" headerKey="-1" headerValue="--%{'不限'}--" list="%{baseService.getSystemOptionMapAsName('productType', null)}"/></td>
					<td class="adv_td1"></td> 
				</tr>
				<tr class="adv_tr">
					...
					<td class="adv_td1">
						...
						<s:select cssClass="select" name="session3" value="%{searchQuery.session3}" list="{}" id="session3" headerKey="-1" headerValue="--%{'不限'}--"/>
			   		 	至
			   			<s:select cssClass="select" name="session2"  value="%{searchQuery.session2}" list="{}" id="session2" headerKey="-1" headerValue="--%{'不限'}--"/>
			   		</td>
					<td class="adv_td1"></td>
				</tr>
				<s:if test="#session.loginer.currentType.equals(@csdc.tool.bean.AccountType@ADMINISTRATOR)">
					<tr class="adv_tr">
						<td class="adv_td1" align="right">获奖状态：</td>
						<td class="adv_td1">
						   	<s:select cssClass="select" name="status" value="%{searchQuery.status}" list="#{'32':'同意','31':'不同意','22':'同意（暂存）','21':'不同意（暂存）','0':'待审'}" headerKey="-1" headerValue="--%{'不限'}--" />
				   		</td>
						<td class="adv_td1"></td>
						<td class="adv_td1" align="right">获奖时间：</td>
				...
				<s:elseif test="#session.loginer.currentType.equals(@csdc.tool.bean.AccountType@MINISTRY)">
					<tr class="adv_tr">
						<td class="adv_td1" align="right">评审审核状态：</td>
						<td class="adv_td1">
						   	<s:select cssClass="select" name="reviewAuditStatus" value="%{searchQuery.reviewAuditStatus}" list="#{'32':'同意','31':'不同意','22':'同意（暂存）','21':'不同意（暂存）','0':'待审'}" headerKey="-1" headerValue="--%{'不限'}--" />
				   		</td>
						<td class="adv_td1"></td>
						<td class="adv_td1" align="right">审核状态：</td>
						<td class="adv_td1">
							<s:select cssClass="select" name="status" value="%{searchQuery.status}" list="#{'32':'同意','31':'不同意','22':'同意（暂存）','21':'不同意（暂存）','12':'被退回','0':'待审'}" headerKey="-1" headerValue="--%{'不限'}--" />
				   		</td>
						<td class=adv_td1></td>
					</tr>					
				</s:elseif>		
				<s:elseif test="#session.loginer.currentType.equals(@csdc.tool.bean.AccountType@PROVINCE)">
					<tr class="adv_tr">
						<td class="adv_td1" align="right">审核状态：</td>
						<td class="adv_td1">
						   	<s:select cssClass="select" name="status" value="%{searchQuery.status}" list="#{'32':'同意','31':'不同意','22':'同意（暂存）','21':'不同意（暂存）','12':'被退回','0':'待审'}" headerKey="-1" headerValue="--%{'不限'}--" />
				...
				<s:elseif test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@MINISTRY_UNIVERSITY, @csdc.tool.bean.AccountType@INSTITUTE)">
					<tr class="adv_tr">
				...
				<s:else>
					<tr class="adv_tr">
						...
						<td class="adv_td2">
							<s:select cssClass="select" name="auditStatus" value="%{searchQuery.auditStatus}" list="#{'3':'已提交','2':'保存','1':'被退回'}" headerKey="-1" headerValue="--%{'不限'}--" />
				   		</td>
						<td class="adv_td2"></td>
					</tr>				
				</s:else>	
				<tr class="adv_tr">												
					...
					<s:if test="#session.loginer.currentType.equals(@csdc.tool.bean.AccountType@MINISTRY)">	
						<td class="adv_td2" align="right">评审审核时间：</td>
						<td class="adv_td2"><s:textfield id="startDate1" value="%{searchQuery.startDate}" cssClass="input_css_self date_hint FloraDatepick" disabled="disabled" size="10" />&nbsp;至&nbsp;<s:textfield id="endDate1" value="%{searchQuery.endDate}" cssClass="input_css_self date_hint FloraDatepick" disabled="disabled" size="10" /></td>
						<td class="adv_td2"></td>
					</s:if>					
				</tr>		
