...
<div id="simple_search">
	<s:form id="search" theme="simple" action="list" namespace="/project/post/endinspection/apply">
				...
				<td align="right">
					...
					<s:if test="!#session.loginer.currentType.within(@csdc.tool.bean.AccountType@MINISTRY_UNIVERSITY, @csdc.tool.bean.AccountType@INSTITUTE)">
						<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--不限--"
							list="#{'1':'项目名称','2':'负责人','3':'依托高校','4':'项目子类','5':'项目年度'}" />
					</s:if>
					<s:else>
						<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--不限--"
							list="#{'1':'项目名称','2':'负责人','4':'项目子类','5':'项目年度'}" />
					</s:else>
					</span>
		...
		<div class="adv_content">
			...
			<table class="adv_table" width="100%" border="0" cellspacing="2" cellpadding="0">
				...
				<tr class="adv_tr">
					...
					<td class="adv_td1"><s:select cssClass="select" name="projectSubtype" value="%{searchQuery.projectSubtype}" list="%{baseService.getSystemOptionMap('projectType', '04')}" headerKey="-1" headerValue="--不限--" /></td>
					<td class="adv_td1"></td>
				</tr>
				<tr class="adv_tr">
				...
				<s:if test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@EXPERT, @csdc.tool.bean.AccountType@STUDENT)"><!-- 研究人员-->
					<tr class="adv_tr">
						...
						<td class="adv_td1">
							   <s:select cssClass="select" name="applicantSubmitStatus" value="%{searchQuery.applicantSubmitStatus}" list="#{'3':'提交','2':'暂存','1':'退回'}" headerKey="-1" headerValue="--不限--" />
				   		</td>
						<td class="adv_td1"></td>
					</tr>
				</s:if>
				<s:elseif test="#session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@MINISTRY)>0">
					...
					<tr class="adv_tr">
						...
						<td class="adv_td1">
							<s:textfield id="startDate11" value="%{searchQuery.startDate}" cssClass="input_css_self date_hint FloraDatepick j_startDate" disabled="disabled" size="10" />
							至
				...
				<s:else>
					<tr class="adv_tr">
						...
						<td class="adv_td1">
							...
							至
							<s:textfield name="endDate" value="%{searchQuery.endDate}" cssClass="input_css_self date_hint FloraDatepick j_endDate" disabled="disabled"/>
				   		</td>
						<td class="adv_td1"></td> 
						<td class="adv_td1" align="right">结项状态：</td>
						<td class="adv_td1">
					   		<s:select cssClass="select" name="finalAuditStatus" value="%{searchQuery.finalAuditStatus}" list="#{'32':'同意','31':'不同意','22':'同意（暂存）','21':'不同意（暂存）','0':'待审'}" headerKey="-1" headerValue="--不限--" />
				   		</td>
						<td class="adv_td1"></td>
					</tr>
				...
				<tr class="adv_tr">
					...
					<s:if test="!#session.loginer.currentType.within(@csdc.tool.bean.AccountType@PROVINCE, @csdc.tool.bean.AccountType@STUDENT)">
						<td class="adv_td1" align="right">高校所在省份：</td>
				...
				<tr class="adv_tr">
					...
					<s:if test="!#session.loginer.currentType.within(@csdc.tool.bean.AccountType@PROVINCE, @csdc.tool.bean.AccountType@STUDENT)">
						<td class="adv_td2" align="right">是否重复显示：</td>
						<td class="adv_td2"><s:radio name="repeatFlag" list="#{'1':'是','0':'否'}" value="%{searchQuery.repeatFlag}"/></td>
						<td class="adv_td2"></td>
					</s:if>
				</tr>
			</table>
