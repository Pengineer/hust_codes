...
//    	});
   ...
   //打印结项证书
    ...
    var switchEndPublish = function(projectType) {
    	$.ajax({
			url:"project/" + projectType + "/endinspection/apply/switchPublish.action",
			data:{
				endResultPublish:$("input[name='endinspection.finalAuditResultPublish']").val(),
				endId:$("input[name='endinspection.id']").val()
			},
			dataType:"json",
			type:"post",
			success:function(){
				if($(".j_switchEndPublish").val() == "发布") {
					$("#endinspection_finalAuditResultPublishStatus").text("已发布 ");
					$(".j_switchEndPublish").val("取消发布");
				}
				else {
					$("#endinspection_finalAuditResultPublishStatus").text("未发布 ");
					$(".j_switchEndPublish").val("发布");
				}
				$("input[name='endinspection.finalAuditResultPublish']").val(1 - parseInt($("input[name='endinspection.finalAuditResultPublish']").val()));
			}
		});
    };
	...
	module.exports = {
		...
		switchEndPublish: function(projectType){switchEndPublish(projectType)}
	};
});
