		...
		{if endinspection_index == 0}<!-- 最近一次结项 -->
			...
			<s:if test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@EXPERT, @csdc.tool.bean.AccountType@STUDENT)"><!-- 研究人员-->
				{if isDirector == 1 && endinspection.finalAuditResultEnd == 1 && endinspection.finalAuditStatus == 3 && endPassAlready != 1 && endPending != 1 && (midPassAlready == 1 || endAllow == 1) && varPending==0 && granted != null && granted.status == 1}
					<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_APPLY_ADD'>
						<div class="p_box_b_2">
							<a class="downloadEndTemplate" href="javascript:void(0);" >下载结项申请书模板</a>
						</div>
						<div class="p_box_b_1">
			...
			<s:elseif test="#session.loginer.currentType.equals(@csdc.tool.bean.AccountType@MINISTRY)"><!-- 部级 -->
				<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_DATA_ADD'>
					...
					{if endinspection.createMode == 0 }
						...
						{if endinspection_index == 0 && endinspection.finalAuditStatus != 3}
							{if reviewflag == -1 && ((accountType == "MINISTRY" && endinspection.status > 5) || (accountType == "PROVINCE" && endinspection.status > 4) || ((accountType =="MINISTRY_UNIVERSITY" || accountType == "LOCAL_UNIVERSITY") && endinspection.status > 3))}
								<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_REVIEW_ADD'>
									<span><input class="btn2 addEndReviewResultPop" type="button" endinspectionId = "${endinspection.id}" value="录入评审" /></span>
									<span><input class="btn2 j_addEndReviewExpert" type="button" endinspectionId = "${endinspection.id}" value="选择专家" /></span>
<%--									<span><input class="btn2" type="button" endinspectionId = "${endinspection.id}" value="录入鉴定" onclick="addEndReviewResultPop('${endinspection.id}');"/></span>--%>
								</sec:authorize>
							{elseif (reviewflag == 22 && accountType == "MINISTRY" && endinspection.status > 5) || (reviewflag == 32 && accountType == "PROVINCE" && endinspection.status > 4) || (reviewflag == 42 && (accountType =="MINISTRY_UNIVERSITY" || accountType == "LOCAL_UNIVERSITY") && endinspection.status > 3)}
								<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_REVIEW_MODIFY'>
								<span><input class="btn2" type="button modifyEndReviewResultPop" endinspectionId = "${endinspection.id}" value="修改鉴定" /></span>
<%--									<span><input class="btn2" type="button" value="修改鉴定" onclick="modifyEndReviewResultPop('${endinspection.id}');"/></span>--%>
								</sec:authorize>
								<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_REVIEW_ADD'>
									<span><input class="btn2 submitEndReviewResult" endinspectionId = "${endinspection.id}" type="button" value="提交鉴定" /></span>
									<span><input class="btn2 j_addEndReviewExpert" type="button" endinspectionId = "${endinspection.id}" value="选择专家" /></span>
								</sec:authorize>
							{/if}
						{/if}
					{elseif endinspection.createMode == 1 || endinspection.createMode == 2}
						...
						{if endinspection_index == 0 && endinspection.finalAuditStatus==2 && granted != null && granted.status == 1}
							{if endPassAlready == 0 && accountType == "MINISTRY" && (midPassAlready == 1 || endAllow == 1)}
								<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_DATA_MODIFY'>
									<input class="btn1 modifyEndResultPop" modifyFlag = "0" type="button" value="修改" />
								</sec:authorize>
								{if varPending==0 && midPending==0}
									<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_DATA_ADD'>
										<input class="btn1 submitEndResult" type="button" value="提交" />
									</sec:authorize>
								{/if}
							{/if}
						{/if}
				...
				<div class="p_box_b_2">
					<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_APPLY_VIEW'>
					{if endinspection.file != null}
						<div style="float: right;"><img src="image/ico09.gif" />
							<a href="" id="${endinspection.id}" class="download_special_3">下载结项申请书</a>
					...
					<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_DATA_UPLOAD'>
						<div style="float: right;"><img src="image/ico10.gif" /><a href="javascript:void(0);" class="uploadEndPop" endinspectionId = "${endinspection.id}" >上传结项申请书</a>&nbsp;&nbsp;</div>
					</sec:authorize>
					</s:if>
					{elseif endinspection.finalAuditStatus==3 && granted != null}
					<s:if test="#session.loginer.currentType.equals(@csdc.tool.bean.AccountType@MINISTRY)"><!-- 部级 -->
					<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_DATA_UPLOAD'>
						<div style="float: right;"><img src="image/ico10.gif" /><a href="javascript:void(0);" class="uploadEndPop" endinspectionId = "${endinspection.id}" >上传结项申请书</a>&nbsp;&nbsp;</div>
					</sec:authorize>
						...
						<tbody>
							<tr class="table_tr7">
								...
								<td colspan="4" style="text-align:center">
									...
									<a href="" id="${projectDatas[endinspection_index].id}" name="${projectDatas[endinspection_index].file}" class="download_special_5">下载研究数据包</span></a>
								...
								<td align="right">
									{if isDirector == 1 && granted != null && granted.status == 1}
										{if endPassAlready != 1 && (midPassAlready == 1 || endAllow == 1)}
											<sec:authorize ifAllGranted="ROLE_PROJECT_SPECIAL_ENDINSPECTION_APPLY_MODIFY">
											...
											{/if}
										{/if}
										<sec:authorize ifAllGranted="ROLE_PROJECT_SPECIAL_ENDINSPECTION_APPLY_DELETE">
			...
			<s:elseif test="#session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@EXPERT)<0 ">
				<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_APPLYAUDIT_VIEW'>
					<div class="p_box_body">
						<table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" style="border-collapse:collapse;">
							<thead>
								<tr class="head_title">
										...
										{if endinspection.createMode == 0 && endinspection_index == 0 && (accountType == "DEPARTMENT" || accountType == "INSTITUTE") && endinspection.status == 2 && endinspection.finalAuditStatus != 3}
											{if endPassAlready != 1 && granted != null && granted.status == 1}
												{if endinspection.deptInstAuditStatus == 0}
													<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_APPLYAUDIT_ADD'>
														<input class="btn1 j_addEndAuditPop" type="button" value="审核" />
														<input class="btn1 j_backEndApply" type="button" value="退回"/>
<%--														<input class="btn1" type="button" value="审核" onclick="addEndAuditPop();"/>--%>
<%--														<input class="btn1" type="button" value="退回" onclick="backEndApply();"/>--%>
													</sec:authorize>
												{elseif endinspection.deptInstAuditStatus < 3}
													<span class="ico_txt2"><a class="j_viewEndAuditPop" endId="${endinspection.id}" alt="1" style="cursor: pointer;">查看详情</a></span>
													{if (isPrincipal == 1 && (endAuditorInfo[1] == belongId || endAuditorInfo[2] == belongId)) || (isPrincipal == 0 && endAuditorInfo[0] == belongId)}
													<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_APPLYAUDIT_MODIFY'>
														<input class="btn1 j_modifyEndAuditPop" type="button" value="修改" />
<%--														<input class="btn1" type="button" value="修改" onclick="modifyEndAuditPop();"/>--%>
													</sec:authorize>
													<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_APPLYAUDIT_ADD'>
											...
											{if endinspection.createMode == 0 && endinspection_index == 0 && (accountType =="MINISTRY_UNIVERSITY" || accountType == "LOCAL_UNIVERSITY") && endinspection.status == 3 && endinspection.finalAuditStatus != 3}
												{if endPassAlready != 1 && granted != null && granted.status == 1}
													...
													{elseif endinspection.universityAuditStatus < 3}
														<span class="ico_txt2"><a class="j_viewEndAuditPop" endId="${endinspection.id}" alt="2" style="cursor: pointer;">查看详情</a></span>
												...
												{if endinspection.createMode == 0 && endinspection_index == 0 && accountType == "PROVINCE" && endinspection.status == 4 && endinspection.finalAuditStatus != 3}
													{if endPassAlready != 1 && granted != null && granted.status == 1}
														...
														{elseif endinspection.provinceAuditStatus < 3}
															<span class="ico_txt2"><a class="j_viewEndAuditPop" endId="${endinspection.id}" alt="3" style="cursor: pointer;">查看详情</a></span>
											...
											{if endinspection.createMode == 0 && endinspection_index == 0 && accountType == "MINISTRY" && endinspection.status == 5 && endinspection.finalAuditStatus != 3}
												{if endPassAlready != 1 && granted != null && granted.status == 1}
													...
													{elseif endinspection.ministryAuditStatus < 3}
														<span class="ico_txt2"><a class="j_viewEndAuditPop" endId="${endinspection.id}" alt="4" style="cursor: pointer;">查看详情</a></span>
			...
			<!-- 专家评审信息-->
			<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_REVIEW_VIEW'>
										...
										{if endinspection.createMode == 0 && endinspection_index == 0 && endinspection.status == 6 && endinspection.finalAuditStatus != 3 && endinspectionReview[endinspection_index].submitStatus != 3}
											{if endPassAlready != 1 && granted != null && granted.status == 1}
												{if endinspectionReview[endinspection_index].submitStatus==0}
													<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_REVIEW_ADD'>
<%--													<input class="btn1" type="button" value="鉴定" onclick="addEndReviewPop('${endinspection.id}');"/>--%>
														<input class="btn1 j_addEndReviewPop" type="button" endId="${endinspection.id}" value="鉴定" />
													</sec:authorize>
												{elseif endinspectionReview[endinspection_index].submitStatus < 3}
													...
													<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_REVIEW_MODIFY'>
														<input class="btn1" type="button j_modifyEndReviewPop" endId="${endinspection.id}" entityId="${endinspectionReview[endinspection_index].id}" value="修改" />
													</sec:authorize>
													<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_REVIEW_ADD'>
														<input class="btn1 j_submitEndReview" type="button" endId="${endinspection.id}" value="提交" />
<%--														<input class="btn1" type="button" value="提交" onclick="submitEndReview('${endinspection.id}');"/>--%>
													</sec:authorize>
												{/if}
											{/if}
										{else}
										...
										{if endinspection.createMode == 0 && endinspection_index == 0 && (accountType =="MINISTRY_UNIVERSITY" || accountType == "TEACHER") && endinspection.status == 6 && endinspection.finalAuditStatus != 3 && allReviewSubmit != -1}
											{if endPassAlready != 1 && granted != null && granted.status == 1}
												{if endinspection.reviewStatus == 0}
													<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_REVIEW_ADD'>
														<input class="btn1 j_addEndGroupReviewPop" type="button" endId="${endinspection.id}" value="鉴定" />
													</sec:authorize>
												{elseif endinspection.reviewStatus == 2}
													...
													<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_REVIEW_MODIFY'>
<%--														<input class="btn1" type="button" value="修改" onclick="modifyEndGroupReviewPop('${endinspection.id}');"/>--%>
														<input class="btn1 j_modifyEndGroupReviewPop" type="button" endId="${endinspection.id}" value="修改" />
													</sec:authorize>
													<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_REVIEW_ADD'>
														<input class="btn1 j_submitEndGroupReview" type="button" endId="${endinspection.id}" value="提交" />
													</sec:authorize>
												{/if}
											{/if}
										{else}
							...
							<tr class="table_tr4">
								...
								<td>
									...
									{if endinspection.createMode == 0 && endinspection_index == 0 && accountType == "MINISTRY" && endinspection.status == 7 && endinspection.finalAuditStatus != 3}
										<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_REVIEWAUDIT_VIEW'>
											{if endPassAlready != 1 && granted != null && granted.status != 3 && granted.status != 4}
												{if endinspection.finalAuditStatus==0}
													<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_REVIEWAUDIT_ADD'>
														<input class="btn1 j_addEndReviewAuditPop" type="button" endId="${endinspection.id}" value="审核" />
													</sec:authorize>
												{elseif endinspection.finalAuditStatus < 3}
													...
													<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_REVIEWAUDIT_MODIFY'>
														<input class="btn1 j_modifyEndReviewAuditPop" type="button"  endId="${endinspection.id}"  value="修改"/>
													</sec:authorize>
													<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_REVIEWAUDIT_ADD'>
														<input class="btn1 j_submitEndReviewAuditResult" type="button" endId="${endinspection.id}"  value="提交" />
													</sec:authorize>
												{/if}
											{/if}
										</sec:authorize>
									{elseif endinspection.finalAuditStatus > 0}
										<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_DATA_VIEW'>
											<span class="ico_txt2"><a class="j_viewEndReviewAuditPop" endId="${endinspection.id}" style="cursor: pointer;">查看详情</a></span>
										</sec:authorize>
										{if endinspection_index == 0 && endinspection.finalAuditStatus==3 && granted != null && granted.status != 3 && granted.status != 4}
											<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_DATA_MODIFY'>
												<input class="btn1 modifyEndResultPop" modifyFlag = "1" type="button" value="修改" />
											</sec:authorize>
										{/if}
									{/if}
								</td>
			...
			{/if}
			<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_DATA_PRINT'>
	...
	<!-- 研究人员 -->
		<s:if test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@EXPERT, @csdc.tool.bean.AccountType@STUDENT)"><!-- 研究人员-->
			{if isDirector == 1 && endPassAlready != 1 && endPending != 1 && (midPassAlready == 1 || endAllow == 1) && varPending==0 && granted != null && granted.status == 1}
				<div class="p_box_b_2"><a class="downloadEndTemplate" style="cursor: pointer;">下载结项申请书模板</a></div>
				<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_ENDINSPECTION_APPLY_ADD'>
