define(function(require, exports, module) {
	...
	exports.init = function() {
		...
		//选择项目所关联的选题
		$(".select_topic_btn").live("click", function(){
			$(this).parent("td").next("td").html("");
			popSelect({
				type : 20,
				callBack : function(result){
					$("#topsId").val(result.data.id);
					$("#topic_div").html(result.data.name);
				}
			});
		});
...
//			};
			popAddApplyFee({
				title : "修改申请经费概算明细",
				src : "project/key/application/apply/toAddApplyFee.action?applyFee=" + $("#applyFee").val() + "&otherFee=" + $("#otherFee").val() + 
...
//					location.href = url;
		...
		$("#addGrantedFee").live("click",function() {
			popAddGrantedFee({
				title : "添加立项经费预算明细",
				src : "project/key/application/apply/toAddGrantedFee.action?approveFee=" + $("#approveFee").val() +  
