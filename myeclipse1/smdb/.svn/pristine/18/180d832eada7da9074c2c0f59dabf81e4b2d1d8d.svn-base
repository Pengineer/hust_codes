...
define(function(require, exports, module) {
	...
	var setUpProjectStatusPop = function(projectType, showProjectDetails) {
		...
		popProjectOperation({
			title : "设置项目状态",
			src : 'project/post/application/granted/toSetUpProjectStatus.action?projectid='+$("#projectid").val(),
			inData : {"varPending" : $("#varPending").val()},
			callBack : function(dis){
				$("#update").val(1);
				$.ajax({
					url: "project/post/application/granted/setUpProjectStatus.action",
					type: "post",
	...
	exports.init = function() {
		...
		$(".j_setUpProjectStatusPop").live('click',function(){//进入项目状态设置弹层页面
			setUpProjectStatusPop(projectType, viewPost.showProjectDetails);
		});
		...
		$(".j_toAddGraApplyPop").live('click',function(){//申请添加
			viewAppGranted.toAddGraPlanPop(projectType, viewPost.showProjectDetails);
		});
		$(".j_toModifyGraApplyPop").live('click',function(){//申请修改
			viewAppGranted.toModifyMidApplyPop(projectType, viewPost.showProjectDetails);
		});
		$(".j_submitGraApply").live('click',function(){//申请提交
			viewAppGranted.submitGraApply(projectType, viewPost.showProjectDetails);
		});
		$(".j_deleteGraApply").live('click',function(){//申请删除
			viewAppGranted.deleteGraPlanPop(projectType, viewPost.showProjectDetails);
		});
		$(".j_addGraAuditPop").live('click',function(){//添加审核
			viewAppGranted.addGraAuditPop(projectType, viewPost.showProjectDetails);
		});
		...
		$(".j_modifyGraAuditPop").live('click',function(){//审核修改
			viewAppGranted.modifyGraAuditPop(projectType, viewPost.showProjectDetails)
		});
		$(".j_submitGraAudit").live('click',function(){//审核提交
			viewAppGranted.submitGraAudit(projectType, viewPost.showProjectDetails)
		});
		$(".j_backGraApply").live('click',function(){//审核退回
			viewAppGranted.backGraApply(projectType, viewPost.showProjectDetails)
		});
		
		$(".j_modifyGraResultPop").live('click',function(){//录入修改
			viewAppGranted.modifyGraResultPop(projectType, viewPost.showProjectDetails);
		});
		$(".j_addGraResultPop").live('click',function(){//录入添加
			viewAppGranted.addGraResultPop(projectType, viewPost.showProjectDetails);
		});
		$(".j_submitGraResult").live('click',function(){//录入提交
			viewAppGranted.submitGraResult(projectType, viewPost.showProjectDetails);
		});
		
//		window.setUpProjectStatusPop = function(){setUpProjectStatusPop(projectType, viewPost.showProjectDetails)};
