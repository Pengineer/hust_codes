	...
	{for item in varList}
		{if item_index == 0}
			...
			<s:if test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@EXPERT, @csdc.tool.bean.AccountType@STUDENT)"><!-- 研究人员-->
				{if isDirector == 1 && item.finalAuditStatus == 3 && endPassAlready != 1 && varPending != 1 && granted != null && granted.status == 1}
					<div id="applyVar">
						...
						<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_VARIATION_APPLY_ADD'>
			...
			<s:elseif test="#session.loginer.currentType.equals(@csdc.tool.bean.AccountType@MINISTRY)"><!-- 部级 -->
				<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_VARIATION_DATA_ADD'>
		...
		<div>
			<div class="p_box_b">
				<div class="p_box_b_1">
					...
					{elseif item.createMode == 1 || item.createMode == 2}
						...
						{if item_index == 0 && accountType == "MINISTRY" && item.finalAuditStatus == 2 && endPassAlready != 1 && granted != null && granted.status == 1}
							<sec:authorize ifAnyGranted='ROLE_PROJECT_SPECIAL_VARIATION_DATA_MODIFY'>
								<input class="btn1 j_modifyVarResultPop" type="button" value="修改" itemIds="${item.id}"/>
							</sec:authorize>
							<sec:authorize ifAnyGranted='ROLE_PROJECT_SPECIAL_VARIATION_DATA_ADD'>
				...
				<div class="p_box_b_2">
				<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_VARIATION_APPLY_VIEW'>
					{if item.file != null}
						<div style="float: right;"><img src="image/ico09.gif" />
							<a href="" id="${item.id}" class="download_special_4">下载变更申请书</a>
			...
			<div id="varInfo" class="p_box_body">
				<table id="list_special_variation${item_index}" width="100%" border="0" cellspacing="0" cellpadding="0" class="table_td_padding" style="text-align:center;">
					<thead>
						...
						<tr>
							...
							<td>延期
								{if item.postponementPlanFile != null}
									<a href="" id="${item.id}" class="download_special_7">[下载延期项目计划书]</a>
							...
							<tr height="36">
								...
								<td align="right">
									{if isDirector == 1 && granted != null && granted.status == 1}
										{if endPassAlready != 1}
											<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_VARIATION_APPLY_MODIFY'>
										...
										{/if}
										<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_VARIATION_APPLY_DELETE'>
			...
			<s:elseif test="#session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@EXPERT)<0 "><!-- 部门及以上管理人员 -->
				<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_VARIATION_APPLYAUDIT_VIEW'>
					<div class="p_box_body">
						<table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" style="border-collapse:collapse;">
							<thead>
								<tr class="head_title">
										...
										{if item.createMode == 0 && (accountType == "DEPARTMENT" || accountType == "INSTITUTE") && item.status == 2 && item.finalAuditStatus != 3}
											{if endPassAlready != 1 && granted != null && granted.status == 1}
												{if item.deptInstAuditStatus == 0}
													<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_VARIATION_APPLYAUDIT_ADD'>
												...
												{elseif item.deptInstAuditStatus < 3}
													...
													<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_VARIATION_APPLYAUDIT_MODIFY'>
