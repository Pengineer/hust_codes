		...
		if (mailIds != null && !mailIds.isEmpty()) {
			...
			for (String mailId : mailIds) {
				if (mailId != null && !mailId.isEmpty()) {
					...
					//删除云存储中的文件
					if(null != mail.getDfs() && !mail.getDfs().isEmpty() && dmssService.getStatus()) {
...
//		}
	...
	public String uploadToDmss(Mail mail) throws Exception {
		if(dmssService.getStatus() && mail.getAttachmentName() != null && mail.getAttachment() != null && !mail.getAttachment().isEmpty() && !mail.getAttachmentName().isEmpty()) {
			String[] tempFileRealpath = mail.getAttachment().split("; ");
			String[] attchmentNames = mail.getAttachmentName().split("; ");
			...
			for(int i = 0; i<tempFileRealpath.length; i++) {
				...
				form.setRating("5.0");
				form.setTags("");
				form.setSourceAuthor(mail.getAccountBelong());
