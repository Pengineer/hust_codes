...
//		jsonMap.put("organization", organization);
		...
		if (patent.getFile() != null) {//显示下载文件的大小
			...
			for (String path : attachPath) {
				...
				} else if(patent.getDfs() != null && !patent.getDfs().isEmpty() && dmssService.getStatus()) {
					long fileSize = dmssService.accquireFileSize(patent.getDfs());
...
//		}
		...
		if(null != viewType && viewType != 0){
			...
			String dfsId = productService.uploadToDmss(patent);
			patent.setDfs(dfsId);			
			return "finish";
		}
		uploadService.flush(groupId);
...
//		}
	...
	public String toModify(){
		...
		if (patent.getFile() != null) {
			...
			} else if(patent.getDfs() != null && !patent.getDfs().isEmpty() && dmssService.getStatus()) {//如果文件在本地服务器不存在，则从云存储中获取临时文件至服务器
				try {
					InputStream downloadStream = dmssService.download(patent.getDfs());
					String sessionId = ServletActionContext.getRequest().getSession().getId();
					File dir = new File(ApplicationContainer.sc.getRealPath("temp" + "/" + sessionId.replaceAll("\\W+", "")));
					dir.mkdirs();
					String fileName = patent.getFile().substring(patent.getFile().lastIndexOf("/") + 1);
	...
	public String modify()throws Exception{
		...
		String orignFile = prePatent.getFile();
		...
		//非外部修改成果
	 	...
	 	prePatent.setUpdateDate(new Date());
		dao.modify(prePatent);
		//将文件组(groupId)中文件的更新操作（删除、移动）在磁盘中实施
		uploadService.flush(groupId);
		//DMSS同步 
		if(orignFile != prePatent.getFile() && dmssService.getStatus()){ //文件已修改
			if(null != orignFile){//原来有文件
				if(null != prePatent.getFile()){ //现在有文件
					productService.checkInToDmss(prePatent);
				}else{ //现在没文件
					dmssService.deleteFile(prePatent.getDfs());
					prePatent.setDfs(null);
					dao.modify(prePatent);
				}
			}else{ //原来没有文件
				if(prePatent.getFile()!=null){ //现在有文件
					String dfsId = productService.uploadToDmss(prePatent);
					prePatent.setDfs(dfsId);
					dao.modify(prePatent);
				}
			}
		}
