...
<div id="simple_search"><!-- 初级检索 -->
	<s:form id="search" theme="simple" action="list" namespace="/inBox">
				...
				<td align="right">
					<span class="choose_bar">
						<s:if test="#session.loginer.currentType.equals(@csdc.tool.bean.AccountType@ADMINISTRATOR)">
							<s:select cssClass="select" name="keyword1" value="%{searchQuery.keyword1}" headerKey="0" headerValue="收件箱"
								list="#{'1':'发件箱','2':'所有信箱'}" id="session1" />
						</s:if>
						<s:else>
						<s:select cssClass="select" name="keyword1" value="%{searchQuery.keyword1}" headerKey="0" headerValue="收件箱"
								list="#{'1':'发件箱'}" id="session1" />
						</s:else>
...
<div id="adv_search" style="display:none;"><!-- 高级检索 -->
	<s:form id="advSearch" action="advSearch" theme="simple" namespace="/inBox">
		...
		<div class="main_content">
			...
			<table class="adv_table" width="100%" border="0" cellspacing="2" cellpadding="0">
				...
				<tr class="adv_tr">
					...
					<td class="adv_td1"><s:select cssClass="select" name="keyword4" value="%{searchQuery.type}" headerKey="" headerValue="--不限--" list="#application.messageItems" listKey="id" listValue="name" /></td>
					<td class="adv_td1"></td>
				</tr>
					<tr class="adv_tr">
						<td class="adv_td2" align="right">是否公开：</td>
						<td class="adv_td2"><s:select cssClass="select" name="isOpen" value="%{searchQuery.isOpen}" headerKey="-1" headerValue="--不限--" list="#{'1':'是','0':'否'}" /></td>
						<td class="adv_td2"></td>
