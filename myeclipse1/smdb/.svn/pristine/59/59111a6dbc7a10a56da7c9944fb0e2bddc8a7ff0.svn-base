...
//		jsonMap.put("organization", organization);
		...
		if (book.getFile() != null) {
			...
			for (String path : attachPath) {
				...
				} else if(book.getDfs() != null && !book.getDfs().isEmpty() && dmssService.getStatus()) {
					long fileSize = dmssService.accquireFileSize(book.getDfs());
...
//		}
		...
		if(null != viewType && viewType != 0){
			...
			String dfsId = productService.uploadToDmss(book);
			book.setDfs(dfsId);
			return "finish";
		}
...
//		}
	...
	public String toModify() throws Exception{
		...
		if (book.getFile() != null) {
			...
			} else if(book.getDfs() != null && !book.getDfs().isEmpty() && dmssService.getStatus()) {//如果文件在本地服务器不存在，则从云存储中获取临时文件至服务器
				try {
					InputStream downloadStream = dmssService.download(book.getDfs());
					String sessionId = ServletActionContext.getRequest().getSession().getId();
					File dir = new File(ApplicationContainer.sc.getRealPath("temp" + "/" + sessionId.replaceAll("\\W+", "")));
					dir.mkdirs();
					String fileName = book.getFile().substring(book.getFile().lastIndexOf("/") + 1);
	...
	public String modify() throws Exception{
		...
		String orignFile = preBook.getFile();
		//基本信息
		...
		//非外部修改成果
	 	...
	 	preBook.setUpdateDate(new Date());
		dao.modify(preBook);
		//将文件组(groupId)中文件的更新操作（删除、移动）在磁盘中实施
		uploadService.flush(groupId);
		//DMSS同步 
		if(orignFile != preBook.getFile() && dmssService.getStatus()){ //文件已修改
			if(null != orignFile){//原来有文件
				if(null != preBook.getFile()){ //现在有文件
					productService.checkInToDmss(preBook);
				}else{ //现在没文件
					dmssService.deleteFile(preBook.getDfs());
					preBook.setDfs(null);
					preBook.setUpdateDate(new Date());
					dao.modify(preBook);
				}
			}else{ //原来没有文件
				if(preBook.getFile()!=null){ //现在有文件
					String dfsId = productService.uploadToDmss(preBook);
					preBook.setDfs(dfsId);
					preBook.setUpdateDate(new Date());
					dao.modify(preBook);
				}
			}
