						...
						{if endinspection_index == 0 && endinspection.finalAuditStatus != 3}
							...
							{elseif (reviewflag == 22 && accountType == "MINISTRY" && endinspection.status > 5) || (reviewflag == 32 && accountType == "PROVINCE" && endinspection.status > 4) || (reviewflag == 42 && (accountType =="MINISTRY_UNIVERSITY" || accountType == "LOCAL_UNIVERSITY") && endinspection.status > 3)}
								<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_REVIEW_MODIFY'>
									<span><input class="btn2 j_modifyEndReviewResultPop" type="button" endId="${endinspection.id}" value="修改鉴定" /></span>
								</sec:authorize>
								<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_REVIEW_ADD'>
									<span><input class="btn2 j_submitEndReviewResult" type="button" endId="${endinspection.id}" value="提交鉴定" /></span>
								</sec:authorize>
							{/if}
						{/if}
						...
						{if endinspection_index == 0 && endinspection.finalAuditStatus==2 && granted != null && granted.status == 1}
							{if endPassAlready==0 && (midPassAlready == 1 || endAllow == 1)}
								<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_DATA_MODIFY'>
									<input class="btn1 j_modifyEndResultPop" data="0" type="button" value="修改" />
								</sec:authorize>
				...
				<div class="p_box_b_2">
					...
					{if endinspection.file != null}
						<div style="float: right;"><img src="image/ico09.gif" />
							<a href="" id="${endinspection.id}" class="download_instp_3">下载结项申请书</a>
					...
					<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_DATA_UPLOAD'>
						<div style="float: right;"><img src="image/ico10.gif" /><a class="j_uploadEndPop" endId="${endinspection.id}" href="javascript:void(0);" >上传结项申请书</a>&nbsp;&nbsp;</div>
					</sec:authorize>
					</s:if>
						...
						<tbody>
							<tr class="table_tr7">
								...
								<td colspan="4" style="text-align:center">
									...
									<a href="" id="${projectDatas[endinspection_index].id}" name="${projectDatas[endinspection_index].file}" class="download_instp_5">下载研究数据包</span></a>
										...
										{if endinspection.createMode == 0 && endinspection_index == 0 && endinspection.status == 6 && endinspection.finalAuditStatus != 3 && endinspectionReview[endinspection_index].submitStatus != 3}
											{if endPassAlready != 1 && granted != null && granted.status == 1}
												...
												{elseif endinspectionReview[endinspection_index].submitStatus < 3}
													<span class="ico_txt2"><a class="j_viewEndReviewPop" entityId="${endinspectionReview[endinspection_index].id}" style="cursor: pointer;">查看详情</a></span>
													<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_REVIEW_MODIFY'>
														<input class="btn1 j_modifyEndReviewPop" endId="${endinspection.id}" entityId="${endinspectionReview[endinspection_index].id}" type="button" value="修改" />
													</sec:authorize>
													<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_REVIEW_ADD'>
								...
								{if endinspectionReviews[endinspection_index] != null}
									{for item in endinspectionReviews[endinspection_index]}
										<tr class="table_tr4">
											...
											<td>
												{if item[5]!=0}
													<span class="ico_txt2"><a class="j_viewEndReviewPop" entityId="${item[0]}" style="cursor: pointer;">查看详情</a></span>
												{/if}
											</td>
										</tr>
									{/for}
					...
					<s:if test="#session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@PROVINCE)<0"><!-- 部级及以上管理人员 -->
						...
						<tbody>
							<tr class="table_tr3">
								...
								<td>提交状态</td>
								<td width="250">查看详情</td>
								<sec:authorize ifAllGranted="ROLE_PROJECT_INSTP_ENDINSPECTION_APPLY_PUBLISH_SWITCH">
								...
								<td>
									...
									{elseif endinspection.finalAuditStatus > 0}
										...
										{if endinspection_index == 0 && endinspection.finalAuditStatus==3 && granted != null && granted.status != 3 && granted.status != 4}
											<sec:authorize ifAllGranted='ROLE_PROJECT_INSTP_ENDINSPECTION_DATA_MODIFY'>
												<input class="btn1 j_modifyEndResultPop" data="1" type="button" value="修改" />
											</sec:authorize>
										{/if}
									{/if}
								</td>
