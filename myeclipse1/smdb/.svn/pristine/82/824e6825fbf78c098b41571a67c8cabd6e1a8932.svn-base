...
<textarea id="view_apply_template" style="display:none;">
	...
	<s:hidden name="appStatus" id="appStatus" value="%{projectService.getBusinessStatus('031')}"/>
	<div class="p_box_b">
		<div class="p_box_b_1">
			{if application.createMode==0}
				...
				{if application.finalAuditStatus != 3}
					{if reviewflagFromApp != 22 && reviewflagFromApp != 23 && reviewflagFromApp != 32 && reviewflagFromApp != 33 && reviewflagFromApp != 42 && reviewflagFromApp != 43 && accountType == "MINISTRY" && application.status == 6}
						<sec:authorize ifAllGranted='ROLE_PROJECT_POST_APPLICATION_REVIEW_ADD'>
							{if reviewflagFromApp == -1  && accountType == "MINISTRY" && application.status == 6}
								<span><input class="btn2 j_addAppReviewResultPop" appId="${application.id}" type="button" value="录入评审" /></span>
							{/if}
		...
		<div class="p_box_b_2">
		<sec:authorize ifAllGranted='ROLE_PROJECT_POST_APPLICATION_APPLY_VIEW'>
			<sec:authorize ifAllGranted='ROLE_PROJECT_POST_APPLICATION_APPLY_DOWNLOAD'>
			{if application.file != null}
				<div style="float: right;"><img src="image/ico09.gif" />
					<a href="" id="${application.id}" class="download_post_1">下载申请评审书</a>
			...
			<sec:authorize ifAllGranted='ROLE_PROJECT_POST_APPLICATION_APPLY_UPLOAD'>
	...
	<div class="p_box_body">
		<table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" style="border-collapse:collapse;">
			...
			<tr class="table_tr7">
				<td class="key">关键字：</td>
				<td class="value" colspan="3">${application.keywords}</td>
			</tr>
			<tr class="table_tr7">
				<td class="key">摘要：</td>
			...
			<div class="p_box_body">
				<table width="100%" border="0" cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" style="border-collapse:collapse;">
					<tr height="36">
						...
						<td align="right">
							{if isDirector == 1}
								<sec:authorize ifAllGranted="ROLE_PROJECT_POST_APPLICATION_APPLY_MODIFY">
									<input type="button" value="修改" class="btn1 j_modifyAppApply"/>&nbsp;
								</sec:authorize>
								<sec:authorize ifAllGranted="ROLE_PROJECT_POST_APPLICATION_APPLY_ADD">
									<input type="button" value="提交" class="btn1 j_submitAppApply"/>&nbsp;
								</sec:authorize>
								<sec:authorize ifAllGranted="ROLE_PROJECT_POST_APPLICATION_APPLY_DELETE">
	...
	<s:elseif test="#session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@EXPERT)<0 ">
		<sec:authorize ifAllGranted='ROLE_PROJECT_POST_APPLICATION_APPLYAUDIT_VIEW'>
			<div class="p_box_body">
				<table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" style="border-collapse:collapse;">
					<thead>
						<tr class="head_title">
							...
							<td>
								...
								{if application.createMode == 0 && (accountType == "DEPARTMENT" || accountType == "INSTITUTE") && application.status == 2 && application.finalAuditStatus != 3}
									{if application.deptInstAuditStatus == 0}
										<sec:authorize ifAllGranted='ROLE_PROJECT_POST_APPLICATION_APPLYAUDIT_ADD'>
									...
									{elseif application.deptInstAuditStatus < 3}
										...
										<sec:authorize ifAllGranted='ROLE_PROJECT_POST_APPLICATION_APPLYAUDIT_MODIFY'>
	...
	<!-- 专家评审信息-->
	<sec:authorize ifAllGranted='ROLE_PROJECT_POST_APPLICATION_REVIEW_VIEW'>
								...
								{if application.createMode == 0 && application.status == 6 && application.finalAuditStatus != 3 && applicationReview.submitStatus != 3}
									{if appPassAlready != 1}
										{if applicationReview.submitStatus==0}
											<sec:authorize ifAllGranted='ROLE_PROJECT_POST_APPLICATION_REVIEW_ADD'>
												<input class="btn1" appId="${application.id}" type="button" value="评审" />
											</sec:authorize>
										{elseif applicationReview.submitStatus < 3}
											...
											<sec:authorize ifAllGranted='ROLE_PROJECT_POST_APPLICATION_REVIEW_MODIFY'>
												<input class="btn1 j_modifyAppReviewPop" appId="${application.id}" appRevId="${applicationReview.id}" type="button" value="修改" />
											</sec:authorize>
											<sec:authorize ifAllGranted='ROLE_PROJECT_POST_APPLICATION_REVIEW_ADD'>
												<input class="btn1 j_submitAppReview" appId="${application.id}" type="button" value="提交" />
											</sec:authorize>
										{/if}
									{/if}
								{else}
					...
					<tbody>
						...
						<tr class="table_tr4">
							...
							<td>
								...
								{if application.createMode == 0 && (accountType =="EXPERT" || accountType == "TEACHER") && application.status == 6 && application.finalAuditStatus != 3 && allAppReviewSubmit != -1 && reviewNum == 1 }
									{if appPassAlready != 1}
										{if application.reviewStatus == 0}
											<sec:authorize ifAllGranted='ROLE_PROJECT_POST_APPLICATION_REVIEW_ADD'>
												<input class="btn1 j_addAppGroupReviewPop" appId="${application.id}" type="button" value="评审" />
											</sec:authorize>
								...
								{elseif reviewflagFromApp == 22 && accountType == "MINISTRY" && application.status > 5}
									<sec:authorize ifAllGranted='ROLE_PROJECT_POST_APPLICATION_REVIEW_MODIFY'>
										<span><input class="btn2 j_modifyAppReviewResultPop" appId="${application.id}" type="button" value="修改" /></span>
									</sec:authorize>
									<sec:authorize ifAllGranted='ROLE_PROJECT_POST_APPLICATION_REVIEW_ADD'>
										<span><input class="btn2 j_submitAppReviewResult" appId="${application.id}" type="button" value="提交" /></span>
									</sec:authorize>
									<span class="ico_txt2"><a class="j_viewAppGroupReviewPop" appId="${application.id}" style="cursor: pointer;">查看详情</a></span>
								{/if}
							</td>
					...
					<tr class="table_tr4">
						...
						<td>
							...
							{if application.createMode == 0 && accountType == "MINISTRY" && application.status == 7 && application.finalAuditStatus != 3}
								<sec:authorize ifAllGranted='ROLE_PROJECT_POST_APPLICATION_REVIEWAUDIT_VIEW'>
									{if appPassAlready != 1}
										{if application.finalAuditStatus==0}
											<sec:authorize ifAllGranted='ROLE_PROJECT_POST_APPLICATION_REVIEWAUDIT_ADD'>
												<input class="btn1 j_addAppReviewAuditPop" appId="${application.id}" type="button" value="审核" />
											</sec:authorize>
										{elseif application.finalAuditStatus < 3}
											...
											<sec:authorize ifAllGranted='ROLE_PROJECT_POST_APPLICATION_REVIEWAUDIT_MODIFY'>
												<input class="btn1 j_modifyAppReviewAuditPop" appId="${application.id}" type="button" value="修改" />
											</sec:authorize>
											<sec:authorize ifAllGranted='ROLE_PROJECT_POST_APPLICATION_REVIEWAUDIT_ADD'>
												<input class="btn1 j_submitAppReviewAuditResult" appId="${application.id}" type="button" value="提交" />
											</sec:authorize>
										{/if}
									{/if}
								</sec:authorize>
	...
	{if application.finalAuditStatus == 3}
		<div class="p_box_body">
			<div class="p_box_body_t">
				<table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" style="border-collapse:collapse;">
					<tr class="table_tr7">
						<td class="key" width="117"><span>审核意见：<br/></span><span>（反馈给负责人）</span></td>
