...
//			$(["annimage", "endimage", "varimage"]).each(function(){
//				image(this);
//			});
			$("#entityId").attr("value", result.application.id);//刷新申请id
			$("#view_base").html(TrimPath.processDOMTemplate("view_base_template", result));
			$("#view_apply").html(TrimPath.processDOMTemplate("view_apply_template", result));
			$("#view_related").html(TrimPath.processDOMTemplate("view_related_template", result));
			if(result.applicationResult !== undefined) $("#view_publishStatus").html(TrimPath.processDOMTemplate("view_publishStatus_template", result));//刷新发布状态信息
			setOddEvenLine("list_post_member", 0);
			if (result.memberFlag == 1 && (typeof(result.memberList) == "undefined" || result.memberList == 0)) {
				setColspan("list_post_member");
			}
			if (!(result.application.finalAuditStatus == 3 && result.application.finalAuditResult == 2)) {//未立项
				...
				$("#reviewPending").attr("value",result.reviewPending);
				$("#reviewPassAlready").attr("value",result.reviewPassAlready);
				...
				if(result.varList.length>0){
					for(var i = 0; i < result.varList.length; i++){
						setOddEvenLine("list_post_variation"+i, 0);
					}
				}
				...
				//年检、结项成果
				$([result.annProdInfo, result.endProdInfo]).each(function(index1){
					var ids = ["list_table_ann_", "list_table_end_"]
...
//				});
			...
			$("#view_established").show();
			$("#view_related").show();
			if(result.applicationResult !== undefined) $("#view_publishStatus").show();
			$("#tabcontent").show();
		}else {
...
//				});
	...
	/**
	 * 初始化后期资助项目查看页面及按钮，主要包括：查看、添加、修改、删除、上条、下条、弹出层查看详情,下载。
	 * @param {string} nameSpace
	 */
	exports.init = function() {
		viewProject.add(projectType);
		viewProject.modify(projectType);
		viewProject.del(projectType, "此操作将删除该项目所有信息包括申请、立项、年检、中检、结项、变更等。您确定要删除吗？");
