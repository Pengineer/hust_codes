...
public class ProjectStatisticAction extends BaseStatisticAction{
	...
	public String toCommon(){
		try {		
			...
			mdxQuery.setMdx(mdx);
			String commonTitle =(null == title) ? null : new String(title.getBytes("ISO8859-1"),"UTF-8");//指定高校
	...
	public String titleCreate(){
		...
		if(type.equals("所有项目") && discipline == "" && university == "" && subType == "" && area == "" && productType == "" && province == "" && evaluType == "" && universityType == "" && organizer == "" && endStatus == ""){
			...
			title = years + "针对" + type + university + discipline + subType + area + productType + province + evaluType + universityType + organizer + endStatus + "关于" + data + "的统计";			
		}
	...
	public String customStatistic() throws Exception {
		...
		if(statistic_data.contains("[Measures].[中检率]")) {
			mdxBuffer.append(" member [Measures].[中检率] as 'IIf((" + grantedCount + " > 0.0), (" + passMidCount + " / " + grantedCount + "), 0.0)', FORMAT_STRING = \"0.00%\" ");
		}
		...
		if(statistic_data.contains("[Measures].[申请数]")) {
		...
		if(statistic_data.contains("[Measures].[中检数]")) {
			mdxBuffer.append(" member [Measures].[中检数] as " + passMidCount);
		}
