...
//		session.put("FilePath", baseDir + "/" + DatetimeTool.getDatetimeString("yyyyMM"));
	...
	public String add() throws Exception {
		...
		String dfsIds = newsService.uploadToDmss(news);
		news.setDfs(dfsIds);
		return SUCCESS;
	...
	public String toModify() {
		...
		if (news.getAttachmentName() != null && news.getAttachment() != null) {
			...
			for (int i = 0; i < tempFileRealpath.length; i++) {
				...
				} else if(news.getDfs() != null && !news.getDfs().isEmpty() && dmssService.getStatus()) {//如果文件在本地服务器不存在，则从云存储中获取临时文件至服务器
					try {
	...
	public String modify() throws Exception {
		...
		String dfsIds = newsService.uploadToDmss(orgNews);
		orgNews.setDfs(dfsIds);
		dao.modify(orgNews);
		return SUCCESS;
	}
	...
	public String view() throws IOException {
		...
		if (news.getAttachmentName() != null) {// 处理附件名称用于显示，去掉最后一个"; "
			...
			for (int i = 0; i < attachPath.length; i++) {
				...
				} else if(news.getDfs() != null && !news.getDfs().isEmpty() && dmssService.getStatus()) {
...
//				request.setAttribute(GlobalInfo.ERROR_INFO, GlobalInfo.ERROR_ATTACH_NULL);
				...
				String[] dfsIds = news.getDfs().split("; ");//封转云存储位置为数组
		...
		if (null == entityId || entityId.trim().isEmpty()) {//通知id不能为空
			...
			if(null == news){//通知不能为空
				...
				if( targetFile == null ){
					...
					} else if (null != news.getDfs() && !dmssService.getStatus()) {
						jsonMap.put(GlobalInfo.ERROR_INFO, GlobalInfo.ERROR_ATTACH_NULL);
					}
				}
			}
