...
<textarea id="view_apply_template" style="display:none;">
	...
	<div class="p_box_b">
			...
			{if application.createMode==0}
				...
				{if application.finalAuditStatus != 3}
					{if reviewflagFromApp != 22 && reviewflagFromApp != 23 && reviewflagFromApp != 32 && reviewflagFromApp != 33 && reviewflagFromApp != 42 && reviewflagFromApp != 43 && accountType == "MINISTRY" && application.status == 6}
						<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_APPLICATION_REVIEW_ADD'>
		...
		<div class="p_box_b_2">
		<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_APPLICATION_APPLY_VIEW'>
			<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_APPLICATION_APPLY_DOWNLOAD'>
			{if application.file != null}
				<div style="float: right;"><img src="image/ico09.gif" />
				<a href="" id="${application.id}" class="download_key_1">下载申请评审书</a>
			...
			<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_APPLICATION_APPLY_UPLOAD'>
	...
	<div class="p_box_body">
		<table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" style="border-collapse:collapse;">
			...
			<tr class="table_tr7">
				...
				<td class="value" colspan='3'><s:hidden id="directors" name="${application.applicantName}" value="${application.applicantId}" cssClass="directors"/></td>
			</tr>
			<tr class="table_tr7">
	...
	<s:elseif test="#session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@EXPERT)<0 ">
		<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_APPLICATION_APPLYAUDIT_VIEW'>
			<div class="p_box_body">
				<table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" style="border-collapse:collapse;">
					<thead>
						<tr class="head_title">
							...
							<td>
								...
								{if application.createMode == 0 && (accountType == "DEPARTMENT" || accountType == "INSTITUTE") && application.status == 2 && application.finalAuditStatus != 3}
									{if application.deptInstAuditStatus == 0}
										<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_APPLICATION_APPLYAUDIT_ADD'>
									...
									{elseif application.deptInstAuditStatus < 3}
										...
										<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_APPLICATION_APPLYAUDIT_MODIFY'>
	...
	<!-- 专家评审信息-->
	<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_APPLICATION_REVIEW_VIEW'>
								...
								{if application.createMode == 0 && application.status == 6 && application.finalAuditStatus != 3 && applicationReview.submitStatus != 3}
									{if appPassAlready != 1}
										{if applicationReview.submitStatus==0}
											<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_APPLICATION_REVIEW_ADD'>
												<input class="btn1 j_addAppReviewPop" appId="${application.id}" type="button" value="评审" />
											</sec:authorize>
										{elseif applicationReview.submitStatus < 3}
											...
											<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_APPLICATION_REVIEW_MODIFY'>
							...
							<td>
								...
								{if application.createMode == 0 && (accountType =="EXPERT" || accountType == "TEACHER") && application.status == 6 && application.finalAuditStatus != 3 && allAppReviewSubmit != -1 && reviewNum == 1}
									{if appPassAlready != 1}
										...
										{/if}
									{/if}
								{elseif accountType == "MINISTRY" && reviewflagFromApp == 22 && application.status > 5}
									<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_APPLICATION_REVIEW_MODIFY'>
					...
					<tr class="table_tr4">
						...
						<td>
							...
							{if application.createMode == 0 && accountType == "MINISTRY" && application.status == 7 && application.finalAuditStatus != 3}
								<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_APPLICATION_REVIEWAUDIT_VIEW'>
									{if appPassAlready != 1}
										{if application.finalAuditStatus==0}
											<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_APPLICATION_REVIEWAUDIT_ADD'>
												<input class="btn1 j_addAppReviewAuditPop" appId="${application.id}" type="button" value="审核" />
											</sec:authorize>
										{elseif application.finalAuditStatus < 3}
											<span class="ico_txt2"><a class="viewAppReviewAuditPop" appId="${application.id}" style="cursor: pointer;">查看详情</a></span>
											<sec:authorize ifAllGranted='ROLE_PROJECT_KEY_APPLICATION_REVIEWAUDIT_MODIFY'>
