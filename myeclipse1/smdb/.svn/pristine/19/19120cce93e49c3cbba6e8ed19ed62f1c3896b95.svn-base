			...
			<table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" style="border-collapse:collapse;">
				<tr class="table_tr7">
					...
					<td class="value" width="140">
							<a id="${item[1]}" class="linkA" href="">${item[0].agencyName}</a>
					</td>
					<td class="key" width="70">所在{if item[2] != null}院系{else}基地{/if}:</td>
					<td class="value" width="140">
						{if item[2] != null}
							<a id="${item[2]}" class="linkD" href="">${item[0].divisionName}</a>
						{else}
							<a id="${item[3]}" class="linkI" href="">${item[0].divisionName}</a>
						{/if}
					</td>
