...
public abstract class ApplicationGrantedAction extends ProjectBaseAction {
    ...
    protected String projectNumber,projectName,researchType,projectSubtype,dtypeNames,applicant,university,projectTopic,divisionName,provinceName,memberName,discipline;//高级检索条件
	...
	**/
	...
	public String toModifyGra(){
		...
		if (granted.getFile() != null) {
			...
			for (int i = 0; i < tempFileRealpath.length; i++) {
				...
				} else if(granted.getDfs() != null && !granted.getDfs().isEmpty() && dmssService.getStatus()) {//如果文件在本地服务器不存在，则从云存储中获取临时文件至服务器
					try {
						InputStream downloadStream = dmssService.download(granted.getDfs());
						String sessionId = ServletActionContext.getRequest().getSession().getId();
						File dir = new File(ApplicationContainer.sc.getRealPath("temp" + "/" + sessionId.replaceAll("\\W+", "")));
						dir.mkdirs();
						String fileName = granted.getFile().substring(granted.getFile().lastIndexOf("/") + 1);
	...
	public String deleteGra(){
		ProjectGranted projectGranted = dao.query(ProjectGranted.class, projectid);
		String dfsId = projectGranted.getDfs();
		projectService.doWithToAdd(projectGranted);
		dao.modify(projectGranted);
...
//		}
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public ProjectGranted doWithAddOrMdoify(int type ,ProjectGranted projectGranted) throws Exception{
		 //保存上传的文件
		String groupId = null;
		if (type == 1) {
			groupId = "file_add";
		...
		String orgFile = projectGranted.getFile();
		...
		//DMSS同步 
		if(orgFile != projectGranted.getFile() && dmssService.getStatus()){ //文件已修改
			if(null != orgFile && !orgFile.isEmpty()){//原来有文件
				if(null != projectGranted.getFile()){ //现在有文件
					projectService.checkInToDmss(projectGranted);
				}else{ //现在没文件
					dmssService.deleteFile(projectGranted.getDfs());
					projectGranted.setDfs(null);
					dao.modify(projectGranted);
				}
			}else{ //原来没有文件
				if(projectGranted.getFile()!=null){ //现在有文件
					String dfsId = projectService.uploadToDmss(projectGranted);
					projectGranted.setDfs(dfsId);
					dao.modify(projectGranted);
				}
			}
		}
	...
	public ProjectGranted doWithAddResult(ProjectGranted projectGranted) throws Exception{
		 //保存上传的文件
		String groupId = "file_add";
		List<String> files = new ArrayList<String>();
		for (FileRecord fileRecord : uploadService.getGroupFiles(groupId)) {
	...
	public String toModifyResult(){
		...
		if (projectGranted.getFile() != null) {
			...
			for (int i = 0; i < tempFileRealpath.length; i++) {
				...
				if (fileRealpath != null && new File(fileRealpath).exists()) {
					uploadService.addFile(groupId, new File(fileRealpath));
				} else if(projectGranted.getDfs() != null && !projectGranted.getDfs().isEmpty() && dmssService.getStatus()) {//如果文件在本地服务器不存在，则从云存储中获取临时文件至服务器
					try {
						InputStream downloadStream = dmssService.download(projectGranted.getDfs());
						String sessionId = ServletActionContext.getRequest().getSession().getId();
						File dir = new File(ApplicationContainer.sc.getRealPath("temp" + "/" + sessionId.replaceAll("\\W+", "")));
						dir.mkdirs();
						String fileName = projectGranted.getFile().substring(projectGranted.getFile().lastIndexOf("/") + 1);
						File downloadFile = new File(dir, fileName);
						FileUtils.copyInputStreamToFile(downloadStream, downloadFile);
						uploadService.addFile(groupId, downloadFile);
					} catch (IOException e) {
	...
	public String modifyResult() throws Exception{
		...
		String orgFile = granted.getFile();
		List<String> files = new ArrayList<String>();
		for (FileRecord fileRecord : uploadService.getGroupFiles(groupId)) {
			//只能上传一个，在js中作了限制
			File curFile = fileRecord.getOriginal();
		...
		AuditInfo auditInfo = this.projectService.getAuditInfo(loginer, graResult, graImportedStatus, null);
		granted.setUpdateDate(new Date());
		...
		uploadService.flush(groupId);
		//DMSS同步 
		if(orgFile != granted.getFile() && dmssService.getStatus()){ //文件已修改
			if(null != orgFile){//原来有文件
				if(null != granted.getFile()){ //现在有文件
					projectService.checkInToDmss(granted);
				}else{ //现在没文件
					dmssService.deleteFile(granted.getDfs());
					granted.setDfs(null);
					dao.modify(granted);
				}
			}else{ //原来没有文件
				if(granted.getFile()!=null){ //现在有文件
	...
	public InputStream getTargetFile() throws Exception{
		...
		if (projectGranted == null) {
			projectGranted = (ProjectGranted) dao.queryUnique("select pg from ProjectGranted pg where pg.applicationId =?", entityId);
		...
		if(savePath!=null && savePath.length()!=0){
			...
			if(ApplicationContainer.sc.getResourceAsStream(filename) != null) {
				...
				downloadStream = dmssService.download(projectGranted.getDfs());
			}
		 }
		...
		if (null == entityId || entityId.trim().isEmpty()) {//id不能为空
			...
			if(null == projectGranted){
				...
				if(null == ApplicationContainer.sc.getResourceAsStream(filename)){
					if(null == projectGranted.getDfs()) {
						jsonMap.put(GlobalInfo.ERROR_INFO, ProjectInfo.ERROR_FILE_NOT_EXIST);
					} else if (null != projectGranted.getDfs() && !dmssService.getStatus()) {
						jsonMap.put(GlobalInfo.ERROR_INFO, ProjectInfo.ERROR_FILE_NOT_EXIST);
					}
				}
			}
