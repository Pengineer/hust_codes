...
public class ProjectMemberFlushListener implements FlushEntityEventListener {
	...
	public void onFlushEntity(FlushEntityEvent event) throws HibernateException {
		...
		if (entity instanceof GeneralMember) {
			...
			member.setApplicationId(member.getApplication().getId());
		} else if (entity instanceof DevrptMember) {
			DevrptMember member = (DevrptMember) entity;
			member.setApplicationId(member.getApplication().getId());
		} else if (entity instanceof SpecialMember) {
			SpecialMember member = (SpecialMember) entity;
			member.setApplicationId(member.getApplication().getId());
		} else {
