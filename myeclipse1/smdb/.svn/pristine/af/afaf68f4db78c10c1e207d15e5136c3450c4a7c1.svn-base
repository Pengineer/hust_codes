...
public abstract class MidinspectionApplyAuditAction extends ProjectBaseAction{
	...
	protected String unAuditLevel;
	protected Date midStartDate;
	protected Date midEndDate;
...
//			}
	...
	public String toMothball(){
		return SUCCESS;
	}
	public String mothball(){
		StringBuffer mothballSql = new StringBuffer();
		mothballSql.append("select mid from ProjectMidinspection mid where mid.projectType = ");
		mothballSql.append("'"+projectType()+"' ");
		Map mothballMap = new HashMap();
		String[] objects =unAuditLevel.split(",");
		mothballSql.append("and ( 1=0 ");
		for (int i = 0; i < objects.length; i++) {
			int auditLevel =Integer.parseInt(objects[i].trim());
			switch (auditLevel) {
				case 1:
					mothballSql.append("or (mid.status=5 and mid.finalAuditResult=0) ");
					break;
				case 2:
					mothballSql.append("or (mid.status=4 and mid.finalAuditResult=0) ");
					break;
				case 3:
					mothballSql.append("or (mid.status=3 and mid.finalAuditResult=0) ");
					break;
				case 4:
					mothballSql.append("or (mid.status=2 and mid.finalAuditResult=0) ");
					break;
				default:
					break;
			}
		...
		if (midStartDate != null) {
			mothballSql.append("and mid.applicantSubmitDate >=:midStartDate ");
			mothballMap.put("midStartDate", midStartDate);
		}
		if (midEndDate!=null) {
			mothballSql.append("and mid.applicantSubmitDate <=:midEndDate ");
			mothballMap.put("midEndDate", midEndDate);
		}
		
		List<ProjectMidinspection> projectMidinspections = dao.query(mothballSql.toString(), mothballMap);
		for (ProjectMidinspection projectMidinspection : projectMidinspections) {
			if (midAuditDate != null) {
				projectMidinspection.setFinalAuditDate(midAuditDate);
			}else {
				projectMidinspection.setFinalAuditDate(new Date());
			}
			projectMidinspection.setFinalAuditOpinion(midAuditOpinion);
			projectMidinspection.setFinalAuditOpinionFeedback(midAuditOpinionFeedback);
			projectMidinspection.setFinalAuditorName("系统管理员");
			projectMidinspection.setFinalAuditStatus(3);
			projectMidinspection.setFinalAuditResult(midAuditResult);
	...
	public String getUnAuditLevel() {
		return unAuditLevel;
	}
	public void setUnAuditLevel(String unAuditLevel) {
		this.unAuditLevel = unAuditLevel;
	}
