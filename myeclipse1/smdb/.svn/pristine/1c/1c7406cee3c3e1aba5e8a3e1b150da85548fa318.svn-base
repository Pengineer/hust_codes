...
<div class="edit_info" id="product">
	<table width="100%" border="0" cellspacing="2" cellpadding="2">
		<tr class="table_tr2">
			...
			<td class="table_td3">
				<s:radio name="addExistOrNot" list="#{'1':'是','0':'否'}" value="1" cssClass="input_css_radio" />
	...
	<div id="exist_product" >
		<table width="100%" border="0" cellspacing="2" cellpadding="0">
			...
			<tr class="table_tr2">
				...
				<td class="table_td3">
					<s:if test="#session.get('addOrModify')==1">
						<s:select cssClass="select j_selectDtype" id="product1" name="proId"  list="{}" headerKey="-1" headerValue="--%{'请选择'}--" />
					</s:if>
					<s:else>
						<s:select cssClass="select j_selectDtype" id="product1" name="proId"  list="%{#session.productmap}" headerKey="-1" headerValue="--%{'请选择'}--"/>
					</s:else>
				</td>
				<td class="table_td4"></td>
			</tr>
			<tr class="table_tr2">
				...
				<td class="table_td3">
				<s:if test="#session.get('addOrModify')==1">
						<s:select cssClass="select" id="dtype" name="awardApplication.disciplineType" list="{}" headerKey="-1" headerValue="--%{'请选择'}--"/>
					</s:if>
					<s:else>
						<s:select cssClass="select" id="dtype" name="awardApplication.disciplineType" list="%{#session.dtypemap}"  headerKey="-1"  headerValue="--%{'请选择'}--"/>
					</s:else>
				</td>
				<td class="table_td4"></td>
			</tr>
	...
	<table width="100%" border="0" cellspacing="2" cellpadding="0">
		<tr class="table_tr2">
			...
			<td class="table_td3"><s:select cssClass="select" name="awardApplication.session" list="{}" id="session1" headerKey="-1" headerValue="--%{'请选择'}--" /></td>
			<td class="table_td4"></td>
		</tr>
		<tr class="table_tr2">
			<td class="table_td2"><span class="table_title5">所在单位：</span></td>
			<td class="table_td3"><s:select cssClass="select" name="unitId" list="%{#session.unitDetails}" headerKey="-1" headerValue="--%{'请选择'}--"/></td>
			<td class="table_td4"></td>
		</tr>
		<tr class="table_tr2">
