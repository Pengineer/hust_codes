...
//			});
			...
			if (!(result.application.finalAuditStatus == 3 && result.application.finalAuditResult == 2)) {//未立项
				...
				$("#view_established").html(TrimPath.processDOMTemplate("view_established_template", result));
				...
				if(result.varList.length>0){
					for(var i = 0; i < result.varList.length; i++){
						setOddEvenLine("list_instp_variation"+i, 0);
					}
				}
...
//				});
			...
			view.inittabs($("#selectedTab").val(), 1);
			project.director();//在页面加载前生成a标签，用于显示负责人信息
			project.varDirector();//在页面加载前生成a标签，用于显示变更负责人信息
			$("#view_apply").show();
			$("#view_established").show();
			$("#view_related").show();
			$("#view_base").show();
	...
	/**
	 * 初始化一般项目查看页面及按钮，主要包括：查看、添加、修改、删除、上条、下条、弹出层查看详情,下载。
