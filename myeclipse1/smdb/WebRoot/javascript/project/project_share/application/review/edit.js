...
define(function(require, exports, module) {
	...
	require('pop-init');
	require('form');
	var appRevValid = require('javascript/project/project_share/application/review/validate');
	...
	//添加专家评审
	//data：2保存；3提交
	var submitOrSaveAddAppReview = function(data, layer){
		submitOrSaveAppReview(data, 1, layer);
	};
	
	//修改专家评审
	//data：2保存；3提交
	var submitOrSaveModifyAppReview = function(data, layer){
		submitOrSaveAppReview(data, 2, layer);
	};
	...
	//type：1添加；2修改
	//id：结项id
	var submitOrSaveAppReview = function(data, type, layer){
		if(!$("#app_review").valid()){
			return;
		}
		if(data == 3) {
		...
		layer.callBack(dis, type, entityId);
		layer.destroy();
	};
	...
	/******************************以下方法用于小组评审*************************************/
	//组评审时查看本组专家所有意见
	var alertGroupOpinion = function(entityId, projectType){
		popProjectOperation({
			title : "查看结项小组所有专家的评审意见",
			src : "project/" + projectType + "/application/review/viewGroupOpinion.action?entityId=" + entityId
		});
	};
	...
	var initGroupOpinion = function(projectType){
		$(".j_groupOpinion").live("click", function() {
			alertGroupOpinion($("#entityId").val(), projectType);
			return false;
		});
	};
	...
	//添加小组评审
	//data：2保存；3提交
	var submitOrSaveAddAppGroupReview = function(data, layer){
		submitOrSaveAppGroupReview(data, 1, layer);
	};
	
	//修改小组评审
	//data：2保存；3提交
	var submitOrSaveModifyAppGroupReview = function(data, layer){
		submitOrSaveAppGroupReview(data, 2, layer);
	};
	
	//组评审是否提交
	//data：2保存；3提交
	//type：1添加；2修改
	var submitOrSaveAppGroupReview = function(data, type, layer){
		if(!$("#app_review_group").valid()){
			return;
		}
		if(data==3) {
			if( !confirm('提交后无法修改，是否确认提交？'))
	...
	module.exports = {
		 init: function(){init()}, 
		 submitOrSaveAddAppReview: function(data, layer){submitOrSaveAddAppReview(data, layer)},
		 submitOrSaveModifyAppReview: function(data, layer){submitOrSaveModifyAppReview(data, layer)},
		 alertGroupOpinion: function(entityId, projectType){alertGroupOpinion(entityId, projectType)}, 
		 initGroupOpinion: function(projectType){initGroupOpinion(projectType)},
		 submitOrSaveAddAppGroupReview: function(data, layer){submitOrSaveAddAppGroupReview(data, layer)},
		 submitOrSaveModifyAppGroupReview: function(data, layer){submitOrSaveModifyAppGroupReview(data, layer)},
	};
});
