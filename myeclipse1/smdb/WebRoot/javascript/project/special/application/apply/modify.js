define(function(require, exports, module) {
	...
	require('dwr/util');
	require('dwr/interface/baseService');
	require('uploadify');
	...
	var modifyApplication = require('javascript/project/project_share/application/apply/modify');
	
	var projectType = "special";
	
	//初始化onclick事件
	exports.init = function() {
		modifyApplication.init(projectType);
		
		$(function() {
			var appId = $("#appId").val();
			$("#file_" +　appId).uploadifyExt({
		...
		$("#modifyApplyFee").live("click",function() {
...
//			};
			popAddApplyFee({
				title : "修改申请经费概算明细",
				src : "project/special/application/apply/toAddApplyFee.action?applyFee=" + $("#applyFee").val() + "&otherFee=" + $("#otherFee").val() + 
...
//					var url = basePath + "project/special/application/apply/addApplyFee.action" + getPara(result);
//					location.href = url;
					this.destroy();
				}
			});
		...
		$("select[name=application1.subtype.id]").live("change",function(){
			var subType = $("select[name=application1.subtype.id]").val();
			var map,subSubType;
			$("select[name=application1.subSubType.id]").hide(500);
			$("select[name=application1.subSubType.id]").empty();
			function call_back_getSystemOptionMap(_map) {
				map = _map;
			}
			DWREngine.setAsync(false);
			baseService.getChildrenMapByParentId(subType, call_back_getSystemOptionMap);
			DWREngine.setAsync(true);
			subSubType += "<option value='-1'>--请选择--</option>";
			if(map){
				for(var key in map){
				   subSubType += "<option value='"+key+"'>"+map[key]+"</option>";
				} 
				$("select[name=application1.subSubType.id]").append(subSubType);
				$("select[name=application1.subSubType.id]").show(500);
			} 
		});
		...
		$("#addGrantedFee").live("click",function() {
			popAddGrantedFee({
				title : "添加立项经费预算明细",
				src : "project/special/application/apply/toAddGrantedFee.action?approveFee=" + $("#approveFee").val() +  
