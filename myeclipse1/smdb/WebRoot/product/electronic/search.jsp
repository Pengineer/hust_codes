				...
				<td align="right"><span class="choose_bar">
					...
					<s:if test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@EXPERT, @csdc.tool.bean.AccountType@STUDENT)">
						<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{'不限'}--"
							list="#{'1': '出版物名称', '2': '出版物类型', '4': '所属单位', '5': '出版单位'}"/>
					</s:if>
					<!-- 校级管理人员 -->
					<s:elseif test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@MINISTRY_UNIVERSITY, @csdc.tool.bean.AccountType@LOCAL_UNIVERSITY)">
						<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{'不限'}--"
							list="#{'1': '出版物名称', '2': '出版物类型', '3': '第一作者', '6': '所属部门', '5': '出版单位'}"/>
					</s:elseif>
					<!-- 其它人员 -->
					<s:else>
						<s:select cssClass="select" name="searchType" value="%{searchQuery.searchType}" headerKey="-1" headerValue="--%{'不限'}--"
							list="#{'1': '出版物名称', '2': '出版物类型', '3': '第一作者', '4': '所属单位', '5': '出版单位'}"/>
					</s:else>
				</span>
			...
			<table class="adv_table" width="100%" border="0" cellspacing="2" cellpadding="0">
				...
				<s:if test=" #session.loginer.currentType.within(@csdc.tool.bean.AccountType@EXPERT, @csdc.tool.bean.AccountType@STUDENT)">
					<tr class="adv_tr">
						...
						<td class="adv_td2">
							<s:select cssClass="select" name="electronic.submitStatus" value="%{searchQuery.submitStatus}" list="#{'2': '保存', '3': '已提交'}"
				...
				<s:else>
					<tr class="adv_tr">
						...
						<td class="adv_td1">
							<s:select cssClass="select" name="electronic.auditResult" value="%{searchQuery.auditResult}" list="#{'0':'待审','1':'不同意','2':'同意'}"
								headerKey="-1" headerValue="--%{'不限'}--"/></td>
						<td class="adv_td1"></td>
