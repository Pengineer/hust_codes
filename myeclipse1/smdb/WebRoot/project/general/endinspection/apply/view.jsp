				...
				<div class="p_box_b_2">
					...
					{if endinspection.file != null}
						<div style="float: right;"><img src="image/ico09.gif" />
							<a href="" id="${endinspection.id}" class="download_general_3">下载结项申请书</a>
						...
						<tbody>
							<tr class="table_tr7">
								...
								<td colspan="4" style="text-align:center">
									...
									<a href="" id="${projectDatas[endinspection_index].id}" name="${projectDatas[endinspection_index].file}" class="download_general_5">下载研究数据包</span></a>
					...
					<s:if test="#session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@PROVINCE)<0"><!-- 部级及以上管理人员 -->
						...
						<tbody>
							<tr class="table_tr3">
								...
								<td>提交状态</td>
								<td width="250">查看详情</td>
								<sec:authorize ifAllGranted="ROLE_PROJECT_GENERAL_ENDINSPECTION_APPLY_PUBLISH_SWITCH">
									<td width="120">发布情况</td>
								</sec:authorize>
							</tr>
							<tr class="table_tr4">
								...
								<sec:authorize ifAllGranted="ROLE_PROJECT_GENERAL_ENDINSPECTION_APPLY_PUBLISH_SWITCH">
								<td>
								{if endinspection.finalAuditResult !=0 }
									<input type="hidden" name="endinspection.finalAuditResultPublish" value="${ endinspection.finalAuditResultPublish}" />
									<input type="hidden" name="endinspection.id" value="${endinspection.id}" />
									{if endinspection.finalAuditResultPublish == 0}
									<span id = "endinspection_finalAuditResultPublishStatus">未发布 </span><input class="btn1 j_switchEndPublish" type="button" value="发布">
									{elseif}
									<span id = "endinspection_finalAuditResultPublishStatus">已发布 </span> <input class="btn1 j_switchEndPublish" type="button" value="取消发布">
									{/if}
								{elseif}
								<span id = "endinspection_finalAuditResultPublishStatus">未审核，不能发布</span>
								{/if}
								</td>
								</sec:authorize>
							</tr>
							<tr class="table_tr4">
								<td colspan="{if endinspection.isApplyNoevaluation == 1 && endinspection.isApplyExcellent == 1}7{elseif endinspection.isApplyNoevaluation == 1 || endinspection.isApplyExcellent == 1}6{else}5{/if}">结项证书编号：${endinspection.certificate}</td>
							</tr>
						</tbody>
					</s:if>
					<s:else><!-- 部级以下所有人员 -->
