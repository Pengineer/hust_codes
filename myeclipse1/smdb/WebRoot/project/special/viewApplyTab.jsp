...
<textarea id="view_apply_template" style="display:none;">
	...
	<div class="p_box_b">
			...
			{if application.createMode==0}
				...
				{if application.finalAuditStatus != 3}
					{if reviewflagFromApp != 22 && reviewflagFromApp != 23 && reviewflagFromApp != 32 && reviewflagFromApp != 33 && reviewflagFromApp != 42 && reviewflagFromApp != 43 && accountType == "MINISTRY" && application.status == 6}
						<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_APPLICATION_REVIEW_ADD'>
		...
		<div class="p_box_b_2">
			<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_APPLICATION_APPLY_VIEW'>
			<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_APPLICATION_APPLY_DOWNLOAD'>
				{if application.file != null}
					<div style="float: right;"><img src="image/ico09.gif" />
						<a href="" id="${application.id}" class="download_special_1">下载申请评审书</a>
			...
			<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_APPLICATION_APPLY_UPLOAD'>
				<div style="float: right;"><img src="image/ico10.gif" /><a href="javascript:void(0);" onclick="uploadAppPop();">上传申请评审书</a>&nbsp;&nbsp;</div>
			</sec:authorize>
			</s:if>
			{elseif application.finalAuditStatus==3 && granted != null }
			<s:if test="#session.loginer.currentType.equals(@csdc.tool.bean.AccountType@MINISTRY)"><!-- 部级 -->
			<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_APPLICATION_APPLY_UPLOAD'>
				<div style="float: right;"><img src="image/ico10.gif" /><a href="javascript:void(0);" onclick="uploadAppPop();">上传申请评审书</a>&nbsp;&nbsp;</div>
			</sec:authorize>
	...
	<div class="p_box_body">
		<table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" style="border-collapse:collapse;">
			...
			<tr class="table_tr7">
				...
				<td class="value">
					${subTypeNameOld} &nbsp;&nbsp; ${subSubTypeNameOld}
				</td>
			</tr>
			<tr class="table_tr7">
			...
			<div class="p_box_body">
				<table width="100%" border="0" cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" style="border-collapse:collapse;">
					<tr height="36">
						...
						<td align="right">
							{if isDirector == 1}
								<sec:authorize ifAllGranted="ROLE_PROJECT_SPECIAL_APPLICATION_APPLY_MODIFY">
									<input type="button" value="修改" class="btn1 j_modifyAppApply"/>&nbsp;
								</sec:authorize>
								<sec:authorize ifAllGranted="ROLE_PROJECT_SPECIAL_APPLICATION_APPLY_ADD">
	...
	<s:elseif test="#session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@EXPERT)<0 ">
		<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_APPLICATION_APPLYAUDIT_VIEW'>
			<div class="p_box_body">
				<table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" style="border-collapse:collapse;">
					<thead>
						<tr class="head_title">
							...
							<td>
								...
								{if application.createMode == 0 && (accountType == "DEPARTMENT" || accountType == "INSTITUTE") && application.status == 2 && application.finalAuditStatus != 3}
									{if application.deptInstAuditStatus == 0}
										<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_APPLICATION_APPLYAUDIT_ADD'>
									...
									{elseif application.deptInstAuditStatus < 3}
										...
										<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_APPLICATION_APPLYAUDIT_MODIFY'>
								...
								{if application.createMode == 0 && application.status == 6 && application.finalAuditStatus != 3 && applicationReview.submitStatus != 3}
									{if appPassAlready != 1}
										...
										{elseif applicationReview.submitStatus < 3}
											...
											<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_APPLICATION_REVIEW_MODIFY'>
					...
					<tr class="table_tr4">
						...
						<td>
							...
							{if application.createMode == 0 && accountType == "MINISTRY" && application.status == 7 && application.finalAuditStatus != 3}
								<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_APPLICATION_REVIEWAUDIT_VIEW'>
									{if appPassAlready != 1}
										{if application.finalAuditStatus==0}
											<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_APPLICATION_REVIEWAUDIT_ADD'>
										...
										{elseif application.finalAuditStatus < 3}
											...
											<sec:authorize ifAllGranted='ROLE_PROJECT_SPECIAL_APPLICATION_REVIEWAUDIT_MODIFY'>
