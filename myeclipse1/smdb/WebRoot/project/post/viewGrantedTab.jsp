...
<textarea id="view_established_template" style="display:none;">
	...
	{else}
		{if application.finalAuditResult == 2}
			{if granted.createMode == 1 || granted.createMode == 2}
				<div class="p_box_b">
					<div class="p_box_b_1">
			...
			<div class="p_box_body">
				<table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" style="border-collapse:collapse;">
					...
					{if granted.status == 1 || granted.status == 5}
						<tr class="table_tr7">
							...
							<td class="value" colspan="5">
								...
								<s:if test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@ADMINISTRATOR, @csdc.tool.bean.AccountType@MINISTRY)"><!-- 系统管理员或部级 -->
									<sec:authorize ifAllGranted='ROLE_PROJECT_POST_APPLICATION_GRANTED_ADD'>
			...
			<div class="p_box_body">
				<table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" style="border-collapse:collapse;">
					<thead>
						<tr height="36">
							<td colspan="3" height="15" align="center">立项经费预算明细表</td>
						</tr>
					</thead>
					<tbody>
						<tr class="table_tr3">
	...
	<div class="p_box_t" style="padding: 0 0 10px 0;">
		<div class="p_box_t_t" >立项计划信息</div>
	</div>
	<div>
		{if granted.file != null}
			<div style="float: right; padding: 0 0 10px 0;"><img src="image/ico09.gif" />
				<a href="" id="${granted.id}" class="download_general_8">下载立项计划书</a>
	...
	<s:if test="#session.loginer.currentType.within(@csdc.tool.bean.AccountType@EXPERT, @csdc.tool.bean.AccountType@STUDENT)"><!-- 研究人员-->
		{if isDirector == 1 && granted != null && granted.auditstatus ==0 && granted.status == 1 && graPassAlready != 1 && graPending !=1 || (granted.finalAuditStatus == 3 && granted.finalAuditResult == 1)}
			<sec:authorize ifAllGranted="ROLE_PROJECT_POST_APPLICATION_GRANTED_APPLY_ADD">
				<div class="p_box_b_1" id="applyMid">
					<input type="button" class="btn1 j_toAddGraApplyPop" value="申请" />
				</div>
			</sec:authorize>
		{elseif granted.auditstatus == 1 &&　granted.applicantSubmitStatus == 2}
			<sec:authorize ifAllGranted="ROLE_PROJECT_POST_APPLICATION_GRANTED_APPLY_MODIFY">
				<input type="button" value="修改" class="btn1 j_toModifyGraApplyPop"/>&nbsp;
			</sec:authorize>
			<sec:authorize ifAllGranted="ROLE_PROJECT_POST_APPLICATION_GRANTED_APPLY_ADD">
				<input type="button" value="提交" class="btn1 j_submitGraApply"/>&nbsp;
			</sec:authorize>
			<sec:authorize ifAllGranted="ROLE_PROJECT_POST_APPLICATION_GRANTED_APPLY_DELETE">
	...
	<s:elseif test="#session.loginer.currentType.equals(@csdc.tool.bean.AccountType@MINISTRY)"><!-- 部级 -->
		<sec:authorize ifAllGranted='ROLE_PROJECT_POST_POST_PLAN_DATA_ADD'><!-- 录入立项计划 -->
			...
			{if granted.auditType == 1 && granted.finalAuditStatus != 3}
				{if granted.finalAuditStatus < 3}
					<sec:authorize ifAllGranted='ROLE_PROJECT_POST_POST_PLAN_DATA_ADD'>
					...
					<tbody>
						...
						<tr class="table_tr4">
							...
							<td>
								...
								{if granted.createMode == 0 && (accountType == "DEPARTMENT" || accountType == "INSTITUTE") && granted.status == 2 && granted.finalAuditStatus != 3}
									{if endPassAlready != 1 && granted != null && granted.status == 1}
										{if granted.deptInstAuditStatus == 0}
											<sec:authorize ifAllGranted='ROLE_PROJECT_POST_PLAN_APPLYAUDIT_ADD'>
										...
										{elseif granted.deptInstAuditStatus < 3}
											...
											<sec:authorize ifAllGranted='ROLE_PROJECT_POST_PLAN_APPLYAUDIT_MODIFY'>
						...
						<s:if test="#session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@DEPARTMENT)<0"><!-- 校级及以上管理人员 -->
							<tr class="table_tr4">
								...
								<td>
									...
									{if granted.createMode  != 0 && (accountType =="MINISTRY_UNIVERSITY" || accountType == "LOCAL_UNIVERSITY") && granted.auditstatus == 3 && granted.finalAuditStatus != 3}
										{if endPassAlready != 1 && granted != null && granted.status == 1}
							...
							<s:if test="#session.loginer.currentType.compareTo(@csdc.tool.bean.AccountType@MINISTRY_UNIVERSITY)<0"><!-- 省厅及以上管理人员 -->
								<tr class="table_tr4">
									...
									<td>
										...
										{if granted.createMode == 0 && accountType == "PROVINCE" && granted.status == 4 && granted.finalAuditStatus != 3}
											{if endPassAlready != 1 && granted != null && granted.status == 1}
			...
			<table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" style="border-collapse:collapse;">
							...
							<td>
								...
								{if accountType == "MINISTRY" && granted.auditstatus == 5 && granted.finalAuditStatus != 3 && granted.auditType == 0}
										{if granted.finalAuditStatus == 0}
											<sec:authorize ifAllGranted='ROLE_PROJECT_POST_MIDINSPECTION_APPLYAUDIT_ADD'>
												<input class="btn1 j_addGraAuditPop" type="button" value="审核"/>
												<input class="btn1 j_backGraApply" type="button" value="退回"/>
											</sec:authorize>
										{elseif granted.finalAuditStatus < 3}
											<sec:authorize ifAllGranted='ROLE_PROJECT_POST_MIDINSPECTION_APPLYAUDIT_MODIFY'>
												<input class="btn1 j_modifyGraAuditPop" type="button" value="修改"/>
											</sec:authorize>
											<sec:authorize ifAllGranted='ROLE_PROJECT_POST_MIDINSPECTION_APPLYAUDIT_ADD'>
												<input class="btn1 j_submitGraAudit" type="button" value="提交"/>
										...
										{/if}
								{else}
									<sec:authorize ifAllGranted='ROLE_PROJECT_POST_MIDINSPECTION_APPLYAUDIT_VIEW'>
				...
				<s:else><!-- 部级以下所有人员 -->
					{if granted.applicantSubmitStatus == 3}
						<thead>
							<tr class="head_title">
								<td colspan="2">最终审核结果</td>
							</tr>
						</thead>
						<tbody>
							<tr class="table_tr3">
								<td width="50%">审核时间</td>
								<td width="50%">最终审核结果</td>
							</tr>
							<tr class="table_tr4">
