...
public class ProjectAnalysisAction extends BaseAction{
	...
	/**
	 * firstYear（如2012）年结项人员在secondYear（如2013）年的项目申请情况
	 * @return
	 */
	...
	/**
	 * firstYear（例如：2012）年结项人员在secondYear（例如：2013）年的项目申请情况
	 * @return
	...
	public InputStream getDownloadFile() throws UnsupportedEncodingException{
		...
		for (Object[] ends : listEnd) {
			...
			if(ids2EndMap.containsKey(applicantId)){
				projects = (List<Object[]>)ids2EndMap.get(applicantId);
			}
			projects.add(ends);
			ids2EndMap.put(applicantId, projects);
		}
		//secondYear（如2013）年申请
...
//		}
		...
		for (Object[] applys : listApply) {
			...
			if (ids2EndMap.containsKey(applicantId)) {//只考虑结项项目中的负责人
				List<Object[]> projects = new ArrayList<Object[]>();
				if(ids2ApplyMap.containsKey(applicantId)){
					projects = (List<Object[]>)ids2ApplyMap.get(applicantId);
				}
				projects.add(applys);
		...
		for (Entry<Object, List<Object[]>> entry: ids2ApplyMap.entrySet()) {
			...
			List<Object[]> applyProjects = entry.getValue();//申请项目
		...
		//调用导出接口
		String header = firstYear + "年结项人员在" + secondYear + "年的项目申请情况";
		String[] title = {"负责人姓名", secondYear + "年申请项目名称", firstYear + "年结项项目名称", firstYear + "年项目批准号", firstYear + "年结项时间"};
	 ...
	 * 待核算项目年度为2014：则申请、立项起始年度为2011-2013，中检起始年度为2008-2010，结项起始年度为2007-2009
	 * 待核算项目年度为2015：则申请、立项起始年度为2012-2014，中检起始年度为2010-2012，结项起始年度为2008-2010
	 * 待核算项目年度为2016：则申请、立项起始年度为2013-2015，中检起始年度为2011-2013，结项起始年度为2010-2012
	 * 待核算项目年度为2017：则申请、立项起始年度为2014-2016，中检起始年度为2012-2014，结项起始年度为2011-2013
	 * 以后逐年加1
	 * @return
	 * @throws UnsupportedEncodingException
	 */
...
//			datas[1] = (personMap.containsKey(univId)) ? personMap.get(univId) : 0;//研究人员数量
			datas[4] = (appMap.containsKey(univId)) ? appMap.get(univId) : 0;//申请数量
