...
//		jsonMap.put("organization", organization);
		...
		if (paper.getFile() != null) {
			...
			for (String path : attachPath) {
				...
				} else if(paper.getDfs() != null && !paper.getDfs().isEmpty() && dmssService.getStatus()) {
					long fileSize = dmssService.accquireFileSize(paper.getDfs());
...
//		validateMust(); validateEdit();//校验
	...
	public String toModify(){
		...
		if (paper.getFile() != null) {
			...
			} else if(paper.getDfs() != null && !paper.getDfs().isEmpty() && dmssService.getStatus()) {//如果文件在本地服务器不存在，则从云存储中获取临时文件至服务器
				try {
					InputStream downloadStream = dmssService.download(paper.getDfs());
					String sessionId = ServletActionContext.getRequest().getSession().getId();
					File dir = new File(ApplicationContainer.sc.getRealPath("temp" + "/" + sessionId.replaceAll("\\W+", "")));
					dir.mkdirs();
					String fileName = paper.getFile().substring(paper.getFile().lastIndexOf("/") + 1);
	...
	public String modify() throws Exception{
		...
		for (FileRecord fileRecord : uploadService.getGroupFiles(groupId)) {
	 	...
	 	prePaper.setUpdateDate(new Date());
	 	dao.modify(prePaper);
	 	//将文件组(groupId)中文件的更新操作（删除、移动）在磁盘中实施
		uploadService.flush(groupId);
		//DMSS同步 
