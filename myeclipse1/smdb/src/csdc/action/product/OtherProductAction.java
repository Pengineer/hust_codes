...
//		jsonMap.put("organization", organization);
		...
		if (otherProduct.getFile() != null) {//显示下载文件的大小
			...
			for (String path : attachPath) {
				...
				} else if(otherProduct.getDfs() != null && !otherProduct.getDfs().isEmpty() && dmssService.getStatus()) {
					long fileSize = dmssService.accquireFileSize(otherProduct.getDfs());
	...
	public String add() throws Exception {
		...
		otherProduct.setCreateDate(new Date());
		otherProduct.setCreateMode(1);
...
//		}
		...
		if(null != viewType && viewType != 0){
			...
			String dfsId = productService.uploadToDmss(otherProduct);
			otherProduct.setDfs(dfsId);
			return "finish";
		}
		uploadService.flush(groupId);
		String dfsId = productService.uploadToDmss(otherProduct);
...
//		}
	...
	public String toModify(){
		...
		if (otherProduct.getFile() != null) {
			...
			} else if(otherProduct.getDfs() != null && !otherProduct.getDfs().isEmpty() && dmssService.getStatus()) {//如果文件在本地服务器不存在，则从云存储中获取临时文件至服务器
				try {
					InputStream downloadStream = dmssService.download(otherProduct.getDfs());
					String sessionId = ServletActionContext.getRequest().getSession().getId();
					File dir = new File(ApplicationContainer.sc.getRealPath("temp" + "/" + sessionId.replaceAll("\\W+", "")));
					dir.mkdirs();
					String fileName = otherProduct.getFile().substring(otherProduct.getFile().lastIndexOf("/") + 1);
	...
	public String modify()throws Exception{
		...
		String orignFile = preOtherProduct.getFile();
		...
		for (FileRecord fileRecord : uploadService.getGroupFiles(groupId)) {
	 	...
	 	//更新其他成果对象
	 	preOtherProduct.setUpdateDate(new Date());
		dao.modify(preOtherProduct);
		//将文件组(groupId)中文件的更新操作（删除、移动）在磁盘中实施
		uploadService.flush(groupId);
		//DMSS同步 
		if(orignFile != preOtherProduct.getFile() && dmssService.getStatus()){ //文件已修改
			if(null != orignFile){//原来有文件
				if(null != preOtherProduct.getFile()){ //现在有文件
					productService.checkInToDmss(preOtherProduct);
				}else{ //现在没文件
					dmssService.deleteFile(preOtherProduct.getDfs());
					preOtherProduct.setDfs(null);
					dao.modify(preOtherProduct);
				}
			}else{ //原来没有文件
				if(preOtherProduct.getFile()!=null){ //现在有文件
					String dfsId = productService.uploadToDmss(preOtherProduct);
					preOtherProduct.setDfs(dfsId);
					dao.modify(preOtherProduct);
				}
			}
		}
