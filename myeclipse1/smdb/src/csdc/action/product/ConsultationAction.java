...
//		jsonMap.put("organization", organization);
		...
		if (consultation.getFile() != null) {
			...
			for (String path : attachPath) {
				...
				} else if(consultation.getDfs() != null && !consultation.getDfs().isEmpty() && dmssService.getStatus()) {
					long fileSize = dmssService.accquireFileSize(consultation.getDfs());
...
//		}
		...
		if(null != viewType && viewType != 0){
			...
			String dfsId = productService.uploadToDmss(consultation);
			consultation.setDfs(dfsId);
			return "finish";
		}
...
//		}
	...
	public String toModify(){
		...
		if (consultation.getFile() != null) {
			...
			} else if(consultation.getDfs() != null && !consultation.getDfs().isEmpty() && dmssService.getStatus()) {//如果文件在本地服务器不存在，则从云存储中获取临时文件至服务器
				try {
					InputStream downloadStream = dmssService.download(consultation.getDfs());
					String sessionId = ServletActionContext.getRequest().getSession().getId();
					File dir = new File(ApplicationContainer.sc.getRealPath("temp" + "/" + sessionId.replaceAll("\\W+", "")));
					dir.mkdirs();
					String fileName = consultation.getFile().substring(consultation.getFile().lastIndexOf("/") + 1);
	...
	public String modify()throws Exception{
		...
		String orignFile = preConsultation.getFile();
		//基本信息
		...
		for (FileRecord fileRecord : uploadService.getGroupFiles(groupId)) {
	 	...
	 	preConsultation.setUpdateDate(new Date());
		dao.modify(preConsultation);
		//将文件组(groupId)中文件的更新操作（删除、移动）在磁盘中实施
		uploadService.flush(groupId);
		//DMSS同步 
		if(orignFile != preConsultation.getFile() && dmssService.getStatus()){ //文件已修改
			if(null != orignFile){//原来有文件
				if(null != preConsultation.getFile()){ //现在有文件
					productService.checkInToDmss(preConsultation);
				}else{ //现在没文件
					dmssService.deleteFile(preConsultation.getDfs());
					preConsultation.setDfs(null);
					dao.modify(preConsultation);
				}
			}else{ //原来没有文件
				if(preConsultation.getFile()!=null){ //现在有文件
					String dfsId = productService.uploadToDmss(preConsultation);
					preConsultation.setDfs(dfsId);
					dao.modify(preConsultation);
				}
			}
		}
