...
public class MobileProductAction extends MobileAction{
	...
	private static String HQL4PAPER = "select p.chineseName, p.authorName,uni.name, p.id,p.publication,p.auditResult,so1.name from Paper p left join p.university uni left join p.department dep left join p.type so1 " +
			"left join p.institute ins left join p.type s left join p.author aut where 1 = 1";
	private static String HQL4BOOK = "select p.chineseName, p.authorName,uni.name, p.id,p.publishUnit,p.auditResult from Book p left join p.university uni left join p.department dep " +
			"left join p.institute ins left join p.type s left join p.author aut where 1 = 1";
	private static String HQL4CONSULTATION = "select p.chineseName, p.authorName,uni.name, p.id,p.useUnit,p.auditResult from Consultation p left join p.university uni left join p.department dep " +
			"left join p.institute ins left join p.author aut where 1 = 1";
	private static String HQL4ELECTRONIC = "select p.chineseName, p.authorName,uni.name, p.id,p.publishUnit,p.auditResult from Electronic p left join p.university uni left join p.department dep " +
			"left join p.institute ins left join p.type s left join p.author aut where 1 = 1";
	private static String HQL4PATENT = "select p.chineseName, p.authorName,uni.name, p.id,p.inventorName,p.auditResult from Patent p left join p.university uni left join p.department dep " +
			"left join p.institute ins left join p.author aut where 1 = 1";
	private static String HQL4OTHERPRODUCT = "select p.chineseName, p.authorName,uni.name, p.id,p.publishUnit,p.auditResult from OtherProduct p left join p.university uni left join p.department dep " +
			"left join p.institute ins left join p.author aut where 1 = 1";
	...
	private Product product;//其他成果对象
		...
		if("Product".equalsIgnoreCase(productType)){
			...
			case 1://论文
				...
				if (null != paper) {
					...
					dataMap.put("paperNote", (null != paper.getNote())? paper.getNote() : "");//学科门类
					//项目相关信息
					try{			
						dataMap.put("relProjectInfos", productService.getRelProjectInfos(entityId));
					}catch(Exception e) {
						System.out.println(e);
					}
					switch(paper.getAuditResult()){
					case 0: status = "未审核";break;
					case 1:status = "不同意";break;
					case 2:status = "同意";break;
					}
					dataMap.put("statusResult", status);
					...
					/*paper = (Paper)dao.query(Paper.class, entityId);
					dataMap.put("productType", paper.getProductType());
					dataMap.put("accountType", loginer.getCurrentType());
					dataMap.put("paper", paper);
					dataMap.put("paperType", (null != paper.getType()) ? paper.getType().getName() : null);//论文类型
					dataMap.put("form", (null != paper.getForm()) ? paper.getForm().getName() : null);//论文形态
					dataMap.put("publicationScope", (null != paper.getPublicationScope()) ? paper.getPublicationScope().getName() : null);//发表范围
					dataMap.put("universityid", (null != paper.getUniversity()) ? paper.getUniversity().getId() : null);//高校id
					dataMap.put("authorid", (null != paper.getAuthor()) ? paper.getAuthor().getId() : null);//作者id
*/				}
				break;
			case 2://著作
			...
			case 3://研究咨询报告
				...
				switch(consultation.getAuditResult()){
				case 0: status = "未审核";break;
				case 1:status = "不同意";break;
				case 2:status = "同意";break;
				}
			...
			case 4://电子出版物
				...
				if (null != electronic) {
					...
					switch(electronic.getAuditResult()){
					case 0: status = "未审核";break;
					case 1:status = "不同意";break;
					case 2:status = "同意";break;
					}
			...
			case 5://专利
				...
				if(null != patent){
					...
					dataMap.put("categotyType", (null != patent.getCategotyType()) ? patent.getCategotyType() : "");
					dataMap.put("patNote", (null != patent.getNote()) ? patent.getNote() : "");
					/*dataMap.put("patAppNumber", (null != patent.getApplicationNumber()) ? patent.getApplicationNumber() : "");
					dataMap.put("patAppDate", (null != patent.getApplicationDate()) ? new SimpleDateFormat("yyyy-MM-dd").format(patent.getApplicationDate()) : "");
					dataMap.put("patPubDate", (null != patent.getPublicDate()) ? new SimpleDateFormat("yyyy-MM-dd").format(patent.getPublicDate()) : "");
					dataMap.put("patPubNumber", (null != patent.getPublicNumber()) ? patent.getPublicNumber() : "");*/
					dataMap.put("independentClaims", (null != patent.getIndependentClaims()) ? patent.getIndependentClaims() : "");

					
					switch(patent.getAuditResult()){
					case 0: status = "未审核";break;
					case 1:status = "不同意";break;
					case 2:status = "同意";break;
					}
			...
			case 6://其他成果
				...
				if (null != otherProduct) {
					...
					switch(otherProduct.getAuditResult()){
					case 0: status = "未审核";break;
					case 1:status = "不同意";break;
					case 2:status = "同意";break;
					}
		...
		if(relProjectInfos.size() > 0){//是项目成果
			...
			/* b = (String)projectMap.get("isEndProduct");
			/*String isMidProduct = (String) projectMap.get("isMidProduct")=="true"?"是":"否";
			String isEndProduct = (String) projectMap.get("isEndProduct")=="true"?"是":"否";
			String isAnnProduct = (String) projectMap.get("isAnnProduct")=="true"?"是":"否";
			String isMarkMoeSupport = (String) projectMap.get("isMarkMoeSupport")=="true"?"是":"否";*/


			dataMap.put("projectName", projectName);//所属项目
			dataMap.put("projectType", projectTypeName );//项目类型
			/*dataMap.put("isMidProduct", isMidProduct );

			dataMap.put("isEndProduct", isEndProduct );
			dataMap.put("isAnnProduct", isAnnProduct );
			dataMap.put("isEndProduct", isEndProduct );
			dataMap.put("isMarkMoeSupport", isMarkMoeSupport );//项目类型
*/
