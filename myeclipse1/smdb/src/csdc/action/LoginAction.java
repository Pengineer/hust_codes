...
//			this.addFieldError(GlobalInfo.ERROR_INFO, "用户名错误！");
	...
	public String switchAccount() {
		...
		if(account != null) {
			...
			session.put("serverPath", serverPath);
			return SUCCESS;
		} else {
			this.addFieldError(GlobalInfo.ERROR_INFO, "您选择的账号不合法");
			return INPUT;
	...
	private void generateAuth(Account account, Passport passport) {
		...
		if(account != null) {
			...
			while (iterator.hasNext()) {// 遍历用户权限，生成security需要的权限对象GrantedAuthority
				...
				i++;
		//		grantedAuthorityList.add(new GrantedAuthorityImpl(((String) iterator.next()).toUpperCase()));
			}
		} else {
		...
		//Authentication newAuth = new UsernamePasswordAuthenticationToken(userDetails, auth.getCredentials(), grantedAuthorityList);
	...
	public String toSelectServer() {
		...
		ActionContext.getContext().getSession().put("serverPath", serverPath);
		return SUCCESS;
	}
	...
	static {
		...
		serverPathMap.put("management", "/server/management/main.jsp");
	}
	...
	private List<String[]> serverInfo(String username) {
		...
		if (rights.contains(new GrantedAuthorityImpl("ROLE_ACCESS_FUND"))) {
			serverList.add(SERVER_INFO[7]);
		}
		if (rights.contains(new GrantedAuthorityImpl("ROLE_ACCESS_MANAGEMENT"))) {
			serverList.add(SERVER_INFO[8]);
		}
...
//	}
	...
	public String basisRight() {
		...
		homeToDo();
		homeRemind();
		ActionContext.getContext().getSession().put("serverPath", "/server/basis/right.jsp");
		return SUCCESS;
	}
	...
	public String expertRight() {
		homeNews();
		homeNotice();
		ActionContext.getContext().getSession().put("serverPath", "/server/expert/right.jsp");
		return SUCCESS;
	}
	...
	public String statisticRight() {
		homeNews();
		homeNotice();
		ActionContext.getContext().getSession().put("serverPath", "/server/statistic/right.jsp");
		return SUCCESS;
	}
...
//		}
		homeNews();
		homeNotice();
		ActionContext.getContext().getSession().put("serverPath", "/server/award/right.jsp");
		return SUCCESS;
	}
...
//		}
		homeNews();
		homeNotice();
		ActionContext.getContext().getSession().put("serverPath", "/server/fee/right.jsp");
		return SUCCESS;
	}
	...
	public String ucenterRight() {
		...
		homeChat();
		homeApply();
		ActionContext.getContext().getSession().put("serverPath", "/server/ucenter/right.jsp");
		return SUCCESS;
	}
	...
	public String scenterRight() {
		homeNews();
		homeNotice();
		ActionContext.getContext().getSession().put("serverPath", "/server/scenter/right.jsp");
		return SUCCESS;
	}
	 ...
	 * 内部管理系统，right页面
	 */
	public String managementRight() {
		homeNews();
		homeNotice();
		String hql = "select app.id, app.name, j.name, app.applicantDate, app.status from Applicant app left join app.job j order by app.applicantDate desc";
		homeApply = dao.query(hql.toString(), 0, 5);
		ActionContext.getContext().getSession().put("serverPath", "/server/management/right.jsp");
		return SUCCESS;
	}
	...
	public String projectRight() {
		...
		ActionContext.getContext().getSession().put("serverPath", "/server/project/right.jsp");
		return SUCCESS;
	}
