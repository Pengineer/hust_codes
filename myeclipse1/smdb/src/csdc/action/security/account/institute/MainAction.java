...
public class MainAction extends AccountAction {
	...
	private static final String HQL = "select a.id, pp.name, i.id, i.name, a.startDate, a.expireDate, a.status, a.lastLoginDate from Account a left join a.passport pp, Institute i left join i.type sys left join i.subjection u where a.institute.id = i.id and a.type = 'INSTITUTE' ";
	...
	private static final String PAGE_NAME = "accountInstituteMainPage";
	...
	public Object[] simpleSearchCondition() {
		...
		hql.append(HQL);
		hql.append(" and ");
		if (loginer.getCurrentType().equals(AccountType.ADMINISTRATOR) || loginer.getCurrentType().equals(AccountType.MINISTRY)) {// 系统管理员或部级账号查询所有基地主账号
			hql.append(" 1=1 ");
		} else if (loginer.getCurrentType().equals(AccountType.PROVINCE)) {// 省级账号查询本省地方高校所有基地主账号或省属及省部共建基地主账号
			hql.append(" ((sys.code = '02' or sys.code = '03') and u.type = 3 or u.type = 4) and u.subjection.id = :currentUnitId ");
			map.put("currentUnitId", loginer.getCurrentBelongUnitId());
		} else if (loginer.getCurrentType().equals(AccountType.MINISTRY_UNIVERSITY) || loginer.getCurrentType().equals(AccountType.LOCAL_UNIVERSITY)) {// 校级账号查询本校所有基地主账号
		...
		if (searchType == 1) {// 按账号名检索
			...
			hql.append(" (LOWER(pp.name) like :keyword or LOWER(u.name) like :keyword or LOWER(i.name) like :keyword) ");
			map.put("keyword", "%" + keyword + "%");
		}
		return new Object[] {
			hql.toString(),
...
//		return SUCCESS;
	...
	public Object[] advSearchCondition() {
		...
		if (belongUnitName != null && !belongUnitName.isEmpty()) {// 按高校名或基地名检索
			...
			hql.append(" and (LOWER(i.name) like :belongUnitName or LOWER(u.name) like :belongUnitName) ");
