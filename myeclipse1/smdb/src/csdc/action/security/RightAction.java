...
public class RightAction extends BaseAction {
	...
	public Object[] simpleSearchCondition() {
		...
		session.put("rightHql", hql);
		session.put("rightMap", map);
		return new Object[]{
			hql.toString(),
			map,
			3,
	...
	@Transactional
	public String importRightByExcel(){
		File excel = new File("C:\\Users\\wangming\\Desktop\\right.xls");
		Sheet sheet = null;
		try {
			Workbook workbook = Workbook.getWorkbook(new FileInputStream(excel));
...
//			throw new RuntimeException(e);
		...
		String[][] content = new String[sheet.getRows()][sheet.getColumns()];
		System.out.print("total:");
		System.out.println(content.length);
		StringBuffer errorRight = new StringBuffer();
		try {
			for (int i = 2; i < sheet.getRows(); i++) {
				...
				//r.id, r.name, r.description, r.code, r.nodevalue
				temp.setName(item[1]);
				temp.setDescription(item[2]);
				temp.setCode(item[3]);
				temp.setNodevalue(item[4]);
				if (rightService.checkRightName(temp.getName())||rightService.checkRightCode(temp.getCode())||rightService.checkRightNode(temp.getNodevalue())) {
					errorRight.append(item[1]+";");
					continue;					
				}
				rightService.addRight(temp);// 添加权限
			}
		} catch (Exception e) {
			e.printStackTrace();
			throw new RuntimeException();
		}
		jsonMap.put("msg", "import success! total:"+content.length);
		jsonMap.put("error", errorRight);
		return SUCCESS;
	}
	...
	public InputStream getDownloadFile() throws UnsupportedEncodingException {
		...
		String hql = session.get("rightHql").toString();
		Map map = (Map) session.get("rightMap");
		List dataList = dao.query(hql, map);
