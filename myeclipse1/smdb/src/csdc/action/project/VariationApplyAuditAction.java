...
//			varCanApproveItem = this.projectService.getVarCanApproveItem(variation.getDeptInstAuditResultDetail());
	...
	public String add(){
        ...
        if(variation.getFinalAuditStatus() != 3 && ((accountType.equals(AccountType.MINISTRY) && varType >= 2) || (accountType.equals(AccountType.PROVINCE) && varType == 4) || ((accountType.equals(AccountType.MINISTRY_UNIVERSITY) || accountType.equals(AccountType.LOCAL_UNIVERSITY)) && varType == 3) || ((accountType.equals(AccountType.DEPARTMENT) || accountType.equals(AccountType.INSTITUTE)) && varType == 2))){
        	...
        	if(accountType.equals(AccountType.MINISTRY)&&varType!=5) {
        		variation.setStatus(5);
        	}
        	variation.edit(auditMap);//保存操作结果
        	if(varAuditDate!=null) {
        		variation.setFinalAuditDate(this.projectService.setDateHhmmss(varAuditDate));
        	} else {
        		variation.setFinalAuditDate(new Date());
        	}
