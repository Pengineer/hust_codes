...
public abstract class ProjectBaseAction extends BaseAction {
	...
	protected Integer listType;//项目列表类型(1.申请列表；2.立项列表；3.中检列表；4.结项列表；5.变更列表；6项目管理系统首页进入；7我的项目列表；8评审项目列表；9用户信息中心首页进入；10评审项目申请列表；11年检列表);
	...
	protected int isAppReviewer;//是否是项目申请评审人，0.不是；1.是评审人但不是评审组长；2.是评审人且是评审组长
	protected String projectType;//项目类型
	protected String deadline;//业务截止时间 
	protected Integer appStatus;//申请业务设置状态
	...
	protected KeyTopic topicSelection;//选题申请对象
	...
	protected ProjectFee projectFeeApply,projectFeeGranted,projectFeeAnn,projectFeeMid,projectFeeEnd,newFee;//项目申请经费对象，项目立项经费对象，项目年检经费对象，项目中检经费对象
...
//			ProjectFee projectFeeApply = null;
			...
			if (application.getFile() != null) {
				...
				for (String path : attachPath) {
					...
					} else if(application.getDfs() != null && !application.getDfs().isEmpty() && dmssService.getStatus()){
						long fileSize = dmssService.accquireFileSize(application.getDfs());
						appFileSizeList.add(baseService.accquireFileSize(fileSize));
					} else {// 附件不存在
...
//			ProjectFee projectFeeGranted = null;
			...
			if (null != granted) {
				if (granted.getFile() != null) {
					...
					for (String path : attachPath) {
						...
						} else if(granted.getDfs() != null && !granted.getDfs().isEmpty() && dmssService.getStatus()) {
							long fileSize = dmssService.accquireFileSize(granted.getDfs());
							graFileSizeList.add(baseService.accquireFileSize(fileSize));
						} else {// 附件不存在
							graFileSizeList.add(null);
						}
					}
				} else {
					graFileSizeList.add(null);
			...
			//申请各级别截止时间业务信息
			deadline = this.projectService.checkIfTimeValidate(accountType, businessType());//用于申请
			...
			//判断项目申请是否有评审记录-1无评审记录，1有评审记录且是专家评审，22教育部录入暂存，23教育部录入提交，32省厅录入暂存，33省厅录入提交，42高校录入暂存，43高校录入提交
			...
			int allAppReviewSubmit = 0;//对于当前申请，是否所有评审专家都提交了个人评审
			...
			//判断当前申请是否所有评审专家都提交了个人评审 0所有专家已提交；-1还有专家未提交
			...
			String subSubTypeNameOld = null;
			String subsubtypeIdOld = (String) dao.queryUnique("select so.id from ProjectApplication pa left join pa.subsubtype so where pa.id =?",application.getId());
			SystemOption subsubtypeOld = dao.query(SystemOption.class, subsubtypeIdOld!=null?subsubtypeIdOld:"");
//			subSubTypeNameOld = (application.getSubsubtype() != null) ? application.getSubsubtype().getName() : "";//原始项目子类名称
			subSubTypeNameOld = (subsubtypeOld != null) ? subsubtypeOld.getName() : "";//原始项目子子类名称
			...
			if(application.getFinalAuditStatus() == 3 && application.getFinalAuditResult() == 2){//已立项
				...
				String subsubtypeIdNew = (String) dao.queryUnique("select so.id from ProjectGranted pg left join pg.subsubtype so where pg.applicationId =?",application.getId());
				SystemOption subsubtypeNew = dao.query(SystemOption.class, subsubtypeIdNew!=null?subsubtypeIdNew:"");
				String subSubTypeNameNew = (subsubtypeNew != null) ? subsubtypeNew.getName() : "";//最新项目子类名称
//				String subSubTypeNameNew = (granted.getSubsubtype() != null) ? granted.getSubsubtype().getName() : "";//最新项目子类名称
				...
				for (ProjectEndinspection pendi : endList) {
					if (pendi.getFile() != null) {
						...
						for (String path : attachPath) {
							...
							} else if(pendi.getDfs() != null && !pendi.getDfs().isEmpty() && dmssService.getStatus()) {
								long fileSize = dmssService.accquireFileSize(pendi.getDfs());
								endiFileSizeList.add(baseService.accquireFileSize(fileSize));
							} else {// 附件不存在
					...
					if (null != resData) {
						if (null != resData.getFile()) {
							...
							for (String path : attachPath) {
								...
								} else if(resData.getDfs() != null && !resData.getDfs().isEmpty() && dmssService.getStatus()) {
									long fileSize = dmssService.accquireFileSize(resData.getDfs());
									resFileSizeList.add(baseService.accquireFileSize(fileSize));
								} else {// 附件不存在
				...
				for (ProjectVariation var : varList) {
					if (var.getFile() != null) {
						...
						for (String path : attachPath) {
							...
							} else if(var.getDfs() != null && !var.getDfs().isEmpty() && dmssService.getStatus()) {
								long fileSize = dmssService.accquireFileSize(var.getDfs());
								varFileSizeList.add(baseService.accquireFileSize(fileSize));
							} else {// 附件不存在
					...
					if (var.getPostponementPlanFile() != null) {
						...
						for (String path : attachPath) {
							...
							} else if(var.getPostponementPlanDfs() != null && !var.getPostponementPlanDfs().isEmpty() && dmssService.getStatus()) {
								long fileSize = dmssService.accquireFileSize(var.getPostponementPlanDfs());
								postponementPlanFileSizeList.add(baseService.accquireFileSize(fileSize));
							} else {// 附件不存在
...
//				jsonMap.put("projectFunding", projectFunding);//拨款信息
				...
				jsonMap.put("subSubTypeNameNew", subSubTypeNameNew);//最新项目子类名称
					...
					for(ProjectMidinspection gm : midList) {
						...
						if (gm.getFile() != null) {
							...
							for (String path : attachPath) {
								...
								} else if(gm.getDfs() != null && !gm.getDfs().isEmpty() && dmssService.getStatus()) {
									long fileSize = dmssService.accquireFileSize(gm.getDfs());
									midFileSizeList.add(baseService.accquireFileSize(fileSize));
								} else {// 附件不存在
						...
						if (gm.getFile() != null) {
							...
							for (String path : attachPath) {
								...
								if (null != is) {
									...
									long fileSize = dmssService.accquireFileSize(gm.getDfs());
									annFileSizeList.add(baseService.accquireFileSize(fileSize));
								} else {// 附件不存在
			...
			//存放json部分代码
			jsonMap.put("application", application);//项目申请
			jsonMap.put("projectFeeApply", projectFeeApply);	//经费概算
			jsonMap.put("projectFeeGranted", projectFeeGranted);	//经费预算
			jsonMap.put("appAuditorInfo", appAuditorInfo);//申请各级审核人和审核人所在机构信息
			jsonMap.put("appInfo", appInfo);//申请截止时间业务信息
			jsonMap.put("appPassAlready", appPassAlready);//申请是否通过
			jsonMap.put("reviewflagFromApp", reviewflagFromApp);//判断项目申请是否有评审记录 -1无评审记录，1有评审记录且是专家评审，22教育部录入暂存，23教育部录入提交，32省厅录入暂存，33省厅录入提交，42高校录入暂存，43高校录入提交
			jsonMap.put("isAppReviewer", isAppReviewer);//是否结项评审人
			jsonMap.put("reviewNum", reviewNum);
			
			jsonMap.put("appRevInfo", appRevInfo);//申请评审截止时间业务信息
			...
			jsonMap.put("subSubTypeNameOld", subSubTypeNameOld);//原始项目子类名称
			...
			//项目数据发布情况(目前只查询一般项目和基地项目的所有接口下数据发布情况)
			if(accountType.equals(AccountType.ADMINISTRATOR)&&("general".equals(application.getType())||"instp".equals(application.getType()))) {
				Map syncStatusMap = projectService.getSynStatus(entityId); 
				//查询项目申请审核结果数据发布情况
				if(syncStatusMap.containsKey(SinossInterface.APP_RESULT)) {
					jsonMap.put("applicationResult", ((SyncStatus)syncStatusMap.get(SinossInterface.APP_RESULT)).getStatus());
				} else {
					jsonMap.put("applicationResult", 0);
				}
				//查询项目中检审核结果数据发布情况
				if(syncStatusMap.containsKey(SinossInterface.MID_RESULT)) {
					jsonMap.put("midinspectionResult", ((SyncStatus)syncStatusMap.get(SinossInterface.APP_RESULT)).getStatus());
				} else {
					jsonMap.put("midinspectionResult", 0);
				}
				//查询项目变更审核结果数据发布情况
				List<Integer> varStatus =  new ArrayList<Integer>();
				if(syncStatusMap.containsKey(SinossInterface.VAR_RESULT)) {
					List<SyncStatus> varSyncStatus = (List<SyncStatus>)syncStatusMap.get(SinossInterface.VAR_RESULT);
					for(SyncStatus sync: varSyncStatus) {
						varStatus.add(sync.getStatus());
					}
				}
				jsonMap.put("variationResult", varStatus);
				//查询项目结项审核结果数据发布情况
				if(syncStatusMap.containsKey(SinossInterface.END_RESULT)) {
					jsonMap.put("endinspectionResult", ((SyncStatus)syncStatusMap.get(SinossInterface.END_RESULT)).getStatus());
				} else {
					jsonMap.put("endinspectionResult", 0);
				}
				//查询项目需要中检数据发布情况
				if(syncStatusMap.containsKey(SinossInterface.MID_REQUIRED)) {
					jsonMap.put("midinspectionRequired", ((SyncStatus)syncStatusMap.get(SinossInterface.MID_REQUIRED)).getStatus());
				} else {
					jsonMap.put("midinspectionRequired", 0);
				}
				//查询项目中检延期数据发布情况
				if(syncStatusMap.containsKey(SinossInterface.MID_DEFER)) {
					jsonMap.put("midinspecitonDefer", ((SyncStatus)syncStatusMap.get(SinossInterface.MID_DEFER)).getStatus());
				} else {
					jsonMap.put("midinspecitonDefer", 0);
				}
			}
		}
		...
		topicSelection = (KeyTopic)dao.query(KeyTopic.class, topsId);
		if(null != topicSelection){//重大攻关项目选题
			...
			jsonMap.put("topsInfo", topsInfo);//选题申请各级别截止时间业务信息
		}
...
//			toFundFee = projectGranted.getApproveFee()-fundedFee;
	...
	public void setTopicSelection(KeyTopic topicSelection) {
		this.topicSelection = topicSelection;
	}
	public KeyTopic getTopicSelection() {
