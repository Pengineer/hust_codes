...
/**
 * 项目申请审核父类管理
 * 定义了子类需要实现的抽象方法并实现了所有项目申请审核共用的相关方法
 * @author 肖雅
 */
public abstract class ApplicationApplyAuditAction extends ProjectBaseAction {
	...
	protected int appAuditStatus;//项目申请状态
	protected String appAuditOpinion;//项目申请意见
	protected String appAuditOpinionFeedback;//申请意见（反馈给项目负责人）
	protected int appAuditResult;//项目申请结果
	protected String entityId;//申请id
	protected String appAuditPerson;//项目申请审核人
	protected Date appAuditDate;//项目申请审核时间
	...
	public String add() {
		...
		for(int i = 0; i < appIds.length; i++) {
			...
			if(application.getFinalAuditStatus() != 3 && ((accountType.equals(AccountType.MINISTRY) && appType == 5) || (accountType.equals(AccountType.PROVINCE) && appType == 4) || ((accountType.equals(AccountType.MINISTRY_UNIVERSITY) || accountType.equals(AccountType.LOCAL_UNIVERSITY)) && appType == 3) || ((accountType.equals(AccountType.DEPARTMENT) || accountType.equals(AccountType.INSTITUTE)) && appType==2))){
				...
				if(application.getStatus() == 6 && application.getCreateMode() != 1 && application.getReviewStatus() == 3 ){
