...
public class InstpProjectFinder {
	...
	/**
	 * [项目年度+负责人姓名+审核状态码+项目年度] -> [application.id]
	 */
	private Map<String, String> pYearStatusMap;
	
	/**
	 * [项目年度+负责人姓名+项目年度] -> [application.id]
	 */
	private Map<String, String> pNameYearMap;
	...
	public InstpApplication findApplication(String projectName, String applicantName) {
		if (pNameMap == null || pYearMap == null || pYearStatusMap == null || pNameYearMap == null) {
			initApplication();
		}
	 ...
	 * @return
	 */
	public InstpApplication findApplication(String projectName, String applicantName, int year, int status) {
		if (pNameMap == null || pYearMap == null || pYearStatusMap == null || pNameYearMap == null) {
			initApplication();
		}
		String key = StringTool.fix(projectName + applicantName.replaceAll("\\d+", "") + status + year);
		String applicationId = pYearStatusMap.get(key);
		return (InstpApplication) (applicationId == null ? null : dao.query(InstpApplication.class, applicationId));
	}
	 ...
	 * @return
	 */
	public InstpApplication findApplication(String projectName, String applicantName, int year) {
		if (pNameMap == null || pYearMap == null || pYearStatusMap == null || pNameYearMap == null) {
			initApplication();
		}
		String key = StringTool.fix(projectName + applicantName.replaceAll("\\d+", "") + year);
		String applicationId = pNameYearMap.get(key);
		return (InstpApplication) (applicationId == null ? null : dao.query(InstpApplication.class, applicationId));
	}
	...
	private void initApplication() {
		...
		pYearStatusMap = new HashMap<String, String>();
		pNameYearMap = new HashMap<String, String>();
		List<Object[]> list = dao.query("select application.name, application.year, mem.member.name, application.id, application.status from InstpApplication application left join application.instpMember mem where mem.isDirector = 1 order by application.year asc, application.finalAuditResult asc");
		list.addAll(dao.query("select granted.name, application.year, mem.member.name, application.id, application.status from InstpGranted granted, InstpApplication application left join application.instpMember mem where granted.application.id = application.id and granted.name <> application.name and mem.isDirector = 1 order by application.year asc, application.finalAuditResult asc"));
		for (Object[] o : list) {
			...
			pYearStatusMap.put(projectName + applicantName + status + year, applicationId);
			pNameYearMap.put(projectName + applicantName + year, applicationId);
		}
