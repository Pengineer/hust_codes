...
public class InstpMidInspectionAutoImporter extends ProjectMidInspectionAutoImporter {
	...
	public void work() throws Throwable {
		...
		count = 0;
		totalImportNum = 0;
		List<SinossProjectMidinspection> spMidinspections = dao.query("from SinossProjectMidinspection spm where spm.projectType = 'base' and spm.isAdded = 1");		
		...
		SystemOption 国内 = (SystemOption) dao.queryUnique("from SystemOption so where so.standard = 'publicationScope' and so.name = '国内'");
		SystemOption 国外 = (SystemOption) dao.queryUnique("from SystemOption so where so.standard = 'publicationScope' and so.name = '国外'");
		SystemOption 港澳台 = (SystemOption) dao.queryUnique("from SystemOption so where so.standard = 'publicationScope' and so.name = '港澳台'");
		...
		for (SinossProjectMidinspection spm: spMidinspections) {
			...
			System.out.println(spm.getName() + "----base:" + (++count) + "/" + spMidinspections.size());
			...
			if (grantedId == null) {
				...
				spm.setIsAdded(0);
				//spm.setNote("2015年录入：基地");
				if (null != spm.getDeferReason()) {//录入变更				
					InstpVariation iVariation = new InstpVariation();
					granted.addVariation(iVariation);
					iVariation.setCreateDate(new Date());
					iVariation.setCreateMode(0);
