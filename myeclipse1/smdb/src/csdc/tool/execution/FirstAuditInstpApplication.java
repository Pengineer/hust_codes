 ...
 * （2）职称审核：项目申请者必须是全国普通高等学校具有正高级专业技术职务的在编在岗教师。
 * （3）撤项项目审核：项目申请人应不具有三年内撤项的教育部人文社会科学研究一般项目、专项任务项目、重大攻关项目、后期资助项目、基地项目（暂未包含）；
 */
...
//		year = (Integer) ActionContext.getContext().getSession().get("defaultYear");	
		...
		//查出当前年的项目
		List<InstpApplication> projects = dao.query("select ia from InstpApplication ia left join ia.university u where ia.year = ? " +
				" and ((ia.provinceAuditStatus = 3 and ia.provinceAuditResult = 2) " +
				" or ( ia.provinceAuditStatus = 0 and ia.provinceAuditResult = 0 and ia.universityAuditStatus = 3 and ia.universityAuditResult = 2 ) " +
				" or ( ia.universityAuditStatus = 0 and ia.universityAuditResult = 0 and ia.provinceAuditStatus = 0 and ia.provinceAuditResult = 0 ) ) ", year);

		for (InstpApplication project : projects) {
			//1、预处理：查重前先清空原有初审结果
			...
			InstpMember director =  (InstpMember) dao.query("select mem from InstpMember mem left join mem.application app where app.id =:appId and mem.isDirector = 1 and mem.memberName =:applicantName",map).get(0);
			//负责人职称检查
			if(!正高级.contains(director.getSpecialistTitle())){
				firstAuditResult += "项目负责人（" + director.getSpecialistTitle() + "）不是正高级员工";
			}
			//2、项目查重
			String[] keys = getKeys(director.getUniversity().getName(), director.getMemberName(), true);//[高校代码+项目负责人]
			for (String key : keys) {
		...
		System.out.println("当前年申请的基地项目负责人所属高校未找到高校代码的共：" + notFindUniversityCodes.size() + "条");
	...
	private void initMap() {
		...
		//国家社科重大项目
		List<String[]> nssfs = jdbcDao.query("select c_applicant_new, c_university, c_number, c_name, c_applicant from T_NSSF where c_university is not null and C_IS_DUP_CHECK_INSTP = 1");
