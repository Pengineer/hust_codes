		...
		while (true) {
			...
			for (String[] row : rows) {
				...
				if (project.getName() != null && project.getNumber() != null) {
					if (tool.getDate(row[15]) != null && !("".equals(tool.getDate(row[15])))) {
						beanFieldUtils.setField(project, "status", 2, BuiltinMergeStrategies.SUPPLY, true);
					} else {
						beanFieldUtils.setField(project, "status", 0, BuiltinMergeStrategies.SUPPLY, true);
					}
