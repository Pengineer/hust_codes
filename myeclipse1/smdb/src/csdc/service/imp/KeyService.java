...
public class KeyService extends ProjectService implements IKeyService  {
	...
	//选题申请相关查询语句
	/**
	 * 获得当前登陆者的初级检索项目选题申请的条件语句
	 * @param searchType 检索条件
	 * @return 初级检索项目选题申请的条件语句
	 */
	...
	/**
	 * 获得当前登陆者的初级检索项目选题申请的补充条件查询语句
	 * @param accountType 账号类型
	 * @return 初级检索选题申请的条件语句
	 */
	public String getKeyTopicSimpleSearchHQLAdd(AccountType accountType){
		String hql = "";
		if(accountType.within(AccountType.EXPERT, AccountType.STUDENT)) {
			hql = " and (tops.status >= 1 or tops.createMode=1 or tops.createMode=2) ";
		}
		else if(accountType.equals(AccountType.DEPARTMENT) || accountType.equals(AccountType.INSTITUTE)) {
			hql = " and (tops.status >= 2 or tops.createMode=1 or tops.createMode=2) ";
		}
		else if(accountType.equals(AccountType.MINISTRY_UNIVERSITY) || accountType.equals(AccountType.LOCAL_UNIVERSITY)) {
			hql = " and (tops.status >= 3 or tops.createMode=1 or tops.createMode=2) ";
		}
		else if(accountType.equals(AccountType.PROVINCE) ) {
			hql = " and (tops.status >= 4 or tops.createMode=1 or tops.createMode=2) ";
		}
		else if(accountType.equals(AccountType.MINISTRY)) {
			hql = " and (tops.status >= 5 or tops.createMode=1 or tops.createMode=2) ";
		}
		return hql;
	}
	...
	public KeyTopic setApplicantIdFromTopicSelection(KeyTopic oldTopicSelection, KeyTopic NewTopicSelection){
		//人员信息
	 ...
	 */
	@SuppressWarnings("unchecked")
	public KeyTopic setApplicantIdFromPerson(KeyTopic topicSelection){
		if(topicSelection.getApplicantId() != null){//有申请者id信息
	...
	public int isSubordinateUniversityTopicSelection(String topsId){
		...
		int utype = 0;
		KeyTopic topicSelection = (KeyTopic) dao.query(KeyTopic.class, topsId);
	 ...
	 * @return 信息list
	 */
	public List getTopsAuditorInfo(KeyTopic topicSelection){
