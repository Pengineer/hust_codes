...
//		MailTask mailTask = scheduledMailTasks.get(mailId);
		/*
		 * 由于scheduledMailTasks中只包含状态为“准备发送”的邮件，故mailTask非空即可说明其状态为“准备发送”
		 */
		final Mail mail = (Mail) dao.query(Mail.class, mailId);
		dao.evict(mail);
...
//		每批邮件发送，限制数量
//		count ++;
//		int total = Integer.parseInt(((SystemConfig) dao.query(SystemConfig.class, "sysconfig00021")).getValue());
//		int waitTime = Integer.parseInt(((SystemConfig) dao.query(SystemConfig.class, "sysconfig00023")).getValue());
//    	if (count < total) {
