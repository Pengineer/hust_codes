...
public class MatcherBeanTransformer {
	 ...
	 * @param dao
	 */
	public MatcherBeanTransformer(List<ProjectApplication> projects, List<Expert> experts, List<ProjectApplicationReview> matches, HibernateBaseDao dao) {
	...
	private void process() {
		...
		//TODO延迟加载
		for (ProjectApplicationReview projectReviewer : matches) {
...
//		if (expert instanceof HibernateProxy || expert.getRating() == null || expert.getUniversityCode() == null) {
////			dao.refresh(expert);
...
//		}
	...
	private void getOtherBurden(){
		...
		ProjectApplication aProject = projects.get(0);
		if (aProject.getType().equals("general")) {
			List<Object[]> otherBurden = dao.query("select expert.id,count(ir.id) from Expert expert,ProjectApplicationReview ir where ir.reviewer.id = expert.id and ir.year = '" + defaultYear + "' and ir.type != 'general' group by expert.id");
			for (Object[] objects : otherBurden) {
				String expertId = objects[0].toString();
				Integer expertInstpBurden = Integer.parseInt(objects[1].toString());
				otherBurdenMap.put(expertId, expertInstpBurden);
			}
			//查询其他非general类型的评审项目burden
		}
		if (aProject.getType().equals("instp")) {
			List<Object[]> otherBurden = dao.query("select expert.id,count(gr.id) from Expert expert,ProjectApplicationReview gr where gr.reviewer.id = expert.id and gr.year = '" + defaultYear + "' and gr.type != 'instp' group by expert.id");
			for (Object[] objects : otherBurden) {
				String expertId = objects[0].toString();
				Integer expertGeneralBurden = Integer.parseInt(objects[1].toString());
				otherBurdenMap.put(expertId, expertGeneralBurden);
			}
			//查询其他非instp类型的评审项目burden
		}
